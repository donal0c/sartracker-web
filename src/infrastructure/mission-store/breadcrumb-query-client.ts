...
export function createBreadcrumbRecordDecoder(accept: (record: { kind: unknown; value: ScalarRow }) => void) {
    ...
    acceptLine(line: string) {
      ...
      if (record.kind === 'rowStart') {
        ...
        if (typeof record.key !== 'string' || record.key.length > 1_024 || !Number.isSafeInteger(record.length)
          || (record.length as number) < 0) throw new Error('Invalid breadcrumb string field.')
        assign(record.key, '')
        ...
        if (row === null || stringKey === null || typeof record.value !== 'string' || record.value.length > 1_024
          || record.value.length === 0 || record.offset !== String(row[stringKey]).length
          || String(row[stringKey]).length + record.value.length > stringLength) {
          throw new Error('Invalid breadcrumb string fragment.')
        }
        row[stringKey] = String(row[stringKey]) + record.value
  ...
  return {
    ...
    async listBreadcrumbPositions(missionId, perDeviceLimit, suppliedId) {
      ...
      try {
        ...
        listeners.get(requestId)?.({ receivedPositions: 0, totalPositions: counts[0]! })
        let kindIndex = 0
        let pending = ''
        const decoder = createBreadcrumbRecordDecoder((record) => {
          ...
          if (kindIndex === 0 && record.value.mission_id !== missionId) throw new Error('Breadcrumb query snapshot belongs to another mission.')
        ...
        for (let sequence = 0; ; sequence += 1) {
          ...
          if (rows[0]!.length !== reportedPositions) {
            ...
            listeners.get(requestId)?.({ receivedPositions: reportedPositions, totalPositions: counts[0]! })
          }
          if (frame.done) break
    ...
    async cancelBreadcrumbQuery(requestId) {
      ...
      return raw.cancelBreadcrumbQuery({ requestId })
    },
  }
}
