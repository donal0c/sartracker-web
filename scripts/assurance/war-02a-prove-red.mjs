...
for (const entry of cases) {
  const args = [path.join(root, 'node_modules/vitest/vitest.mjs'), 'run',
    `tests/unit/assurance/war-02a/${entry.file}`, '-t', entry.name, '--no-file-parallelism', '--reporter=json']
  for (const disabled of [false, true]) {
    ...
    const named = executed.length === 1 && executed[0].title.startsWith(entry.name)
    const valid = !result.error && result.signal === null && (disabled
      ? named && result.status === 1 && report.numFailedTests === 1 && report.numPassedTests === 0
        && executed[0].status === 'failed'
        && executed[0].failureMessages.some((message) => message.includes(`AssertionError: ${entry.oracle}`))
      : named && result.status === 0 && report.success === true && report.numPassedTests === 1 && report.numFailedTests === 0)
    if (!valid) {
