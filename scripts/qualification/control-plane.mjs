...
export function compileCoverageRegistry(input) {
  ...
  if (!Array.isArray(input.contracts) || !Array.isArray(input.releaseCriticalHazards)) {
    throw new Error('Qualification registry contracts and release-critical hazards are required.')
  }
  ...
  const criticalHazards = uniqueStrings(input.releaseCriticalHazards, 'releaseCriticalHazards')
...
export function evaluateCandidate({ registry, mode, identities, contractResults, blockers = [], judgeResults = [] }) {
  ...
  const releaseEligible = mode === 'candidate'
  ...
  return Object.freeze({
    ...
    mode,
    verdict: mode === 'dry-run'
      ? 'DRY_RUN_ONLY'
      : releaseEligible ? 'PASS' : 'HOLD',
    releaseEligible,
    identities,
