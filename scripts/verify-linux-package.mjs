...
import { loadPackageToolchain, appImageToolsetVersion } from '../build/linux-package-toolchain.js'
...
async function main() {
  ...
  const { getAppImageTools, x64 } = loadPackageToolchain(JSON.parse(readFileSync(path.join(projectRoot, 'package.json'), 'utf8')))
  const toolsetVersion = appImageToolsetVersion(JSON.parse(readFileSync(path.join(projectRoot, 'electron-builder.json'), 'utf8')))
  const sourceArgument = process.argv.indexOf('--source-before')
  // Standalone reinspection has no pre-build proof; preserve its current dirtiness.
  const sourceBefore = sourceArgument < 0 ? capturePackageSource(projectRoot) : JSON.parse(process.argv[sourceArgument + 1])
  ...
  try {
    ...
    const appImageTools = await getAppImageTools(toolsetVersion, x64)
...
betterSqlite3: require(${JSON.stringify(path.join(appDir, 'resources/app.asar/node_modules/better-sqlite3/package.json'))}).version,
...
db.close(); console.log(JSON.stringify(result));`
    ...
    assertPackagedNativeRuntime(lock, native)
    const receipt = {
      schema: 'sartracker-linux-package-safety-v2',
      ...packageSourceProvenance(sourceBefore, capturePackageSource(projectRoot), { preBuildSourceCaptured: sourceArgument >= 0 }),
      ...
      launcher, squashfsOffset, toolsetVersion, native, inventories, debFiles, debControl, launcherLibraries,
