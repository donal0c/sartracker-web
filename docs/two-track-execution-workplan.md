...
and complete five independent exact-head reviews before the PR opens.
...
| Active — five exact-head reviews | PR-3: decide and deliver complete mission coverage | Shared Tracking / S2 Electron / Coverage / Verification | `DON-273` / `DON-276` / `DON-275` | Candidate B and G3's combined default posture are ratified; the v9→v10 migration proof stands. Review-wave mission/source/worker/style/filter-attestation gaps are red-first remediated through `4b740f2`. Candidate B's affected G2 rows remain within unchanged budgets under the explicit standing rule. The corrected `df61a02` production driver proves exact 960k/2M totals plus non-empty decoded geometry for every period before/after activation; the single packaged CI soak remains valid because no ingest/write path changed. The final unit/static/Dots/build/full Chromium/coverage visual/macOS+Ubuntu package gates are green. Complete five independent exact-head reviews, then open one PR to `master` if clean. |
