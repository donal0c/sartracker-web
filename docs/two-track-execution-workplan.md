...
| Active | Exact post-PR6 WAR-04B refresh | Assurance / Release integrity | `DON-146` / `DON-254` / `DON-255` | Master `3cdf555d`: both audits/live controls refreshed; 265 focused tests and local native/inventory pass; local lifecycle rejects 203 ms continuity and same-head retry is refused. Independent Linux `34338004244` passes all workflow gates; final review/PR-head CI binding is recorded on the review PR. HOLD; no dependency remediation/publication. PR #12 also integrates current master and corrects temporary CI profiling; its current-head checks are recorded on the PR. [Report](assurance/findings/WAR-04B.md). |
