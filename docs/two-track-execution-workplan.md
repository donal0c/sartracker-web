...
| Remediation gate | PR-1: trustworthy ingest and live safety | Shared Tracking / S2 Electron / Evidence | `DON-266` / `DON-267` / `DON-268` / `DON-269` | [PR #1](https://github.com/donal0c/sartracker-web/pull/1) remains unmerged while the requested-change pass is proved. Mission-scoped rejection delivery, durable failure/evidence-loss markers, replay, Windows safety, bounded aggregate/page reads, all-invalid live handling, conservative stationary tolerance/accuracy/outlier behavior, restart identity, and the remaining main-isolate Review health scan are fixed with red-green coverage. Unit `1,684`, backend, Chromium `146`, tracking visual/review, BCP-10, lint/build, and packaging pass; exact committed-head large-store and packaged-soak reruns remain before push. The cold audit worker remains about `7.5 s`; indexing/retention stays open in `DON-251`, and the 200 ms gate is not waived. |
