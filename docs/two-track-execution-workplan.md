...
| In Review | Fix breadcrumb history truncation across multi-device missions | S2 Electron / Shared Tracking | `DON-159` / `DON-25` | Implemented locally. Live breadcrumbs are now retained with a per-device, full-window render budget that preserves each device's first/last points and route coverage instead of one global last-20k slice. Diagnostics/support reports include per-device retained/observed/truncated counts, and Electron persistence remains uncapped by default with a guard comment for future caps. Verified: focused DON-159 unit set, full `npm run test` (149/854), `npm run lint`, `npm run build`, targeted diagnostics E2E, full Chromium E2E (107), and `npm run test:backend` (47 passed / 1 ignored). Awaiting team retest in the next Electron beta. |
