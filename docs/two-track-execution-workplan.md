...
| Done locally | Fix field-scale 36-hour breadcrumb catch-up, restart reliability, and route fidelity | S2 Electron / Shared Tracking / Verification | `DON-260` | Reopened from 2026-08-09 field evidence: a 36-hour mission took about nine minutes for one tracker and appeared in blocks; comparison with Traccar also showed missing visible route detail. The correction separates initial catch-up from live polling, preserves two-hour/max-eight request bounds, checkpoints durable per-device frontiers atomically, paces anti-entropy separately, prioritizes active mission devices, bounds renderer/query memory, and uses deterministic geometry-aware representatives with truthful achieved-error metadata. The final frozen package passed the 33-device resilience proof: current fix `91 ms`, first breadcrumb `310 ms`, exact `279,936`-row persistence in `36.112 s`, injected-503 recovery, SIGKILL/frontier resume, and exact `104,274`-coordinate render parity after restart. The unchanged 14-day soak passed `1,935,384` exact rows, two restarts, integrity/WAL/responsiveness gates, and a fail-closed Darwin process-tree peak of `1,464,614,912` bytes, `682,868,736` below 2 GiB. Exact CI-built beta qualification remains before publication. |
