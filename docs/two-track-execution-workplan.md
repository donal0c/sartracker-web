...
| In Progress | Make Breadcrumb Dots source-exact and independently release-gated | S2 Electron / Shared Tracking / Verification | `DON-260` | Beta.12.9 persisted exact history but rendered Dots from the bounded Line projection, producing pairs/groups with long cadence gaps. The replacement uses a mission-start-bounded exact SQLite page query, dedicated Dots source, <=10k automatic display, and exact Earlier/Later pages; Line alone remains simplified. Independent macOS packaged proofs cover 279,936 fixes/28 rendered pages and 1,935,384 fixes/194 source pages with zero gaps/dupes, recovery/restarts, exact Line totals, clean operator ownership, 56.329 s outward traversal, and 1.737 GB peak RSS. Full unit `195/1,548` and Playwright `179/179` are green. Next: cut beta.12.10 draft, then exact CI AppImage + installed `.deb` Ubuntu 36h/14d, package-safety, and live provider-to-map equality gates. Do not hand over beta.12.9 as the corrected Dots build. |
