...
| Final five-review gate — PR #3 open | PR-3: decide and deliver complete mission coverage | Shared Tracking / S2 Electron / Coverage / Verification | `DON-273` / `DON-276` / `DON-275` | Candidate B and G3 remain ratified; migration/scale proof stands for unchanged geometry/schema surfaces. Broad review of `848bfb2` found that fatal main exit bypassed the renderer-evidence coordinator. The remediation makes fatal relaunch await durable loss blockers and adds an atomic active-session marker, so restart after fatal exit, SIGKILL, or power loss blocks every unfinalized mission before an operator window opens. Real SQLite restart health is critical. Final full unit rerun 272/2,208, lint, TypeScript, build/budgets, backend 51/51 plus one intentional ignore, final Chromium rerun 13/13 and selected critical visual/review 2/2 pass. Earlier package proof is invalidated; commit/push, replacement exact-head macOS/Linux package/soak proof, then five fresh PR-3 reviews from zero remain. The prospective PR-4 review amendment does not relax PR-3. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and unmerged. Do not merge or release from this task. |
