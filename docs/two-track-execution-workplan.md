...
| Release candidate | Fix field-scale 36-hour breadcrumb catch-up, restart reliability, and route fidelity | S2 Electron / Shared Tracking / Verification | `DON-260` | Reopened from field evidence: 36-hour history took about nine minutes and arrived in blocks. Eamonn's 2026-08-10 follow-up added slow-speed clusters and larger gaps at 120–145 km/h. The correction separates initial catch-up from live polling, preserves two-hour/max-eight bounds, checkpoints durable frontiers, paces anti-entropy, prioritizes active devices, bounds renderer/query memory, and uses deterministic geometry-aware representatives. Mac/local proof is exact, but Ubuntu correctly rejected beta.12.6 at the unchanged 60-second gate: per-chunk FULL SQLite transactions left `268,275/279,936` rows and incomplete coverage. `4896ecd` now atomically persists each fetched wave and its checkpoints, with batch/fetch failure isolation and publication-after-durability locked by tests. Beta.12.7 release preparation is active; complete no-skip, tag CI, exact CI AppImage/real `.deb` Ubuntu 36-hour, long-soak, lifecycle, diagnostics, safety, and live-provider qualification remain before publication. |
