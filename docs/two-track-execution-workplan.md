...
| Recovery in flight; final exact-head proof pending | [PR #10](https://github.com/donal0c/sartracker-web/pull/10): PR-6 archive lifecycle | S2 Electron / Archive / Mission Review / Verification | `DON-248` / `DON-252` / `DON-253` | Rejected diagnostics now include the earlier caf9/495/81/74/6a/231/d91/7e heads, cadence-failing pushed `b75f8689…`, proof-indeterminate local `b7793753…`, and cleanup-failing local `30061c2d…`; none will be rerun unchanged. B75 passed macOS/browser/visual/kill gates but Linux run `33940959449` proved a real 240 ms Review cadence breach. B779 repaired current-publication/evidence/cache cadence but exposed a named-operation proof race. 30061 repaired that proof boundary, then its sole packaged lifecycle reproduced an immediate cleanup failure whose focused causal regression identifies as a deferred WAL read snapshot invalidated by an independent live-mission commit and hidden from the busy retry. The red-first successor owns the writer slot before boundary reads and carries only bounded inventory-derived failure attribution through real worker/IPC/Playwright receipt boundaries. Exact mission/device/archive custody, source/continuity/main/renderer/cleanup gates, finite retries, and strict `<200 ms` deadlines remain unchanged. Successor pre-freeze gates are green at focused `6/110`, archive `20/412`, full serial `377/3,806`, full ESLint, production build/budgets, backend `58 + 1 ignored`, Node/diff checks, and a clean focused review. Replacement exact-head package/lifecycle, browser/visual, physical kill matrix, Linux, four final reviews, and exactly one fresh controlled greater-than-2-GiB qualifier remain. Pre-merge proof is not release or field acceptance. |
