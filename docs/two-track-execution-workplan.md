...
1. Execute the **Breadcrumb and Mission-History Programme** under the locked ADR and `docs/breadcrumb-programme-execution-policy.md`. PR-1 through PR-5 are merged. Breadcrumb programme PR-6—the archive-lifecycle stage owned by `DON-248`/`DON-252`/`DON-253` and distinct from merged WAR-01 GitHub PR #6—is being recovered on [PR #10](https://github.com/donal0c/sartracker-web/pull/10). Exact head `caf9e5e480fcd02cc44d68c8397efcd6ae78f2cd` is rejected: its Ubuntu receipt verified a greater-than-2-GiB archive but ended with a misleading teardown classification and independently recorded multi-second liveness/durable-write breaches. The causes are roughly 9.7 million retained telemetry events plus repeated finalization/history scans. The bounded recovery adds deterministic finalization lookup, mission-scoped logical cleanup with telemetry-only `mission_events` deletion, constant-space diagnostics, and a process-faithful packaged current-fix watchdog gate. All final exact-head local/package/browser/visual/Linux gates and four independent reviews remain pending, followed by exactly one fresh controlled field qualification. This remains pre-merge engineering work, not release or field acceptance. Donal retains approval/merge authority. After the exact merged head exists, run WAR-04B's narrow refresh before any release decision; BCP-17 must qualify one final candidate before `DON-255` may publish it.
...
| Recovery in flight; final exact-head proof pending | [PR #10](https://github.com/donal0c/sartracker-web/pull/10): PR-6 archive lifecycle | S2 Electron / Archive / Mission Review / Verification | `DON-248` / `DON-252` / `DON-253` | Head `caf9e5e480fcd02cc44d68c8397efcd6ae78f2cd` is rejected. Its Ubuntu receipt verified a 5,243,848,931-byte archive but ended when an unbounded `Math.max(...samples)` diagnostics aggregation overflowed; independent measurements also breached the strict liveness gate. Roughly 9.7 million retained telemetry events and repeated finalization/history scans are the diagnosed scale causes. Recovery uses deterministic boundary lookup, mission-scoped logical cleanup with telemetry-only `mission_events` deletion and retained audit/custody events, constant-space diagnostics, and a packaged Traccar→main/preload→React→MapLibre external watchdog. Final local, package, browser/visual, Linux, four-review, and exactly-one fresh controlled greater-than-2-GiB gates remain pending. Pre-merge proof is not release or field acceptance. |
