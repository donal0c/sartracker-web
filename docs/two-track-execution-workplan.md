...
1. Execute the **Breadcrumb and Mission-History Programme** under the locked ADR and `docs/breadcrumb-programme-execution-policy.md`. PR-1 through PR-4 are merged. PR-5 mission evidence/replay (`DON-274`, `DON-277`, `DON-278`, `DON-279`) remains one review unit on `codex/breadcrumb-pr5-evidence-replay` from exact merged PR-4 base `80309c995a18eeb190cce4310c9a46b0f46d5263`. The restarted review wave invalidated `ad21f1a8`: prior-candidate cursor semantics could fail open, finalized idempotent archive repair was absent, and a 2,000,000-row quarantine made a clean Finish block main for 2,061.05 ms. Executable fix `b9c47e0d` adds durable cursor-format recapture, mission-first indexed quarantine custody, and fenced/audited finalized archive replacement. Strict red-green proof is green through full unit 2,498/2,498, focused 193/193, 500,000-entry bounded lookup, backend, lint/build, fresh indexed 960k and unsigned macOS arm64 package/soak. Exact binding head `c5447500` passed Linux run `33226833222`, including the same deterministic gate, production build/budgets, native x64 packages, indexed 960k qualification, packaged 8,664-position soak and AppImage launch. Restarted broad/persistence/concurrency/renderer reviews on one final code-and-documentation head remain the task-owned completion gate. PR-6 archive lifecycle follows only after Donal accepts PR-5.
