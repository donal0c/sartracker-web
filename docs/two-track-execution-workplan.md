...
| Final evidence-lifecycle remediation — PR #3 open | PR-3: decide and deliver complete mission coverage | Shared Tracking / S2 Electron / Coverage / Verification | `DON-273` / `DON-276` / `DON-275` | Candidate B/G3 and unchanged migration/scale proof stand. The Opus deadline-inversion P1 produced durable provisional uncertainty at `42f1694`. The bounded exact-`80cc941` recheck then found three related seam cases, all centrally retraced: accepted mission writes could settle after persistence rejection; negative/crash promotion reused a stale scope snapshot; and partial provisional staging lacked compensation. One red-first repair now requires a durable `mission_persistence_failed` blocker before settlement, keeps current positions independent, makes Finish retry/fail closed, refreshes mission scope at promotion, and retracts earlier staged scopes on partial failure. GREEN is focused 9 / 250, full 273 / 2,249, lint, TypeScript, CJS syntax, diff, build/budgets, Chromium 16/16, and selected critical visual 2/2 with two uncached Opus PASS reviews. Commit/push, repeat exact-head macOS/Linux package and the one Ubuntu CI-scale soak, obtain green expanded CI, then run the amended broad, persistence, concurrency, and renderer-containment reviews once. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and **not merge-ready**; do not merge or release. |
