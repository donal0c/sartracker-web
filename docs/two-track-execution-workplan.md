...
1. Execute the **Breadcrumb and Mission-History Programme** under the locked ADR and `docs/breadcrumb-programme-execution-policy.md`. PR-1 through PR-4 are merged. PR-5 mission evidence/replay (`DON-274`, `DON-277`, `DON-278`, `DON-279`) is implemented as one review unit on `codex/breadcrumb-pr5-evidence-replay` from exact merged PR-4 base `80309c995a18eeb190cce4310c9a46b0f46d5263`. Pushed candidate `1e9b6acc` is rejected by the Linux 500,000-row startup gate and renderer/input review. Indexed startup and the complete Replay/GPX containment seam are red-green in the replacement tree; commit/bind/push, package/Linux proof and a restarted broad/persistence/concurrency/renderer exact-head wave remain the task-owned completion gates. PR-6 archive lifecycle follows only after Donal accepts PR-5.
