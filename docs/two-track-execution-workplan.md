...
1. Execute the **Breadcrumb and Mission-History Programme** under the locked ADR and `docs/breadcrumb-programme-execution-policy.md`. PR-1 through PR-4 are merged. PR-5 mission evidence/replay (`DON-274`, `DON-277`, `DON-278`, `DON-279`) remains one review unit on `codex/breadcrumb-pr5-evidence-replay` from exact merged PR-4 base `80309c995a18eeb190cce4310c9a46b0f46d5263`. Exact head `782167e8` passed Linux but all four independent reviewers rejected it with four unique P2s. Executable fix `d98d7907` removes the populated legacy event index from synchronous upgrade, byte-bounds worker writer turns with explicit oversized-row quarantine, applies event-provenance readiness to every archive/finalize path, and closes marker/drawing mutation bounds across preload, main, browser harness and operator inputs. Full serial unit 2,492/2,492, Chromium 167/167, backend, lint/build, fresh indexed 960k, unsigned macOS arm64 package/soak and exact branch-head Linux run `33222361222` are green. The restarted broad/persistence/concurrency/renderer wave on one final code-and-documentation head remains the task-owned completion gate. PR-6 archive lifecycle follows only after Donal accepts PR-5.
