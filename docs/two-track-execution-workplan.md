...
| In Review | Fix Linux keyring decrypt failure blocking Electron startup | S2 Electron / Runtime | `DON-175` / `DON-25` | Fixed locally. Undecryptable stored Traccar secrets now disable tracking with an operator warning instead of throwing out of runtime bootstrap; explicit `trackingConfig: null` no longer falls through to env config. Added `npm run electron:smoke:bad-secret`, beta handoff/runbook/manual updates, and unit regressions. Verified: focused tests, `node --check` for the smoke script, `npm run lint`, `npm run build`, full `npm run test` (150/925). Awaiting packaged Ubuntu bad-secret smoke before release/close. |
| Todo | Decide/fix Review workspace blocking active mission controls | S3 Web App / UI | `DON-176` / `DON-5` | New field-report follow-up. Separate from completed DON-148 large-event freeze: current `WorkspaceOverlay` consumes clicks outside the Review panel, making map tools and Pause/Finish feel frozen while Review is open. Needs product call: non-blocking/docked Review vs explicit modal blocking, then browser-backed regression. |
