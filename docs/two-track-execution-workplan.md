...
1. Execute the **Breadcrumb and Mission-History Programme** under the locked ADR and `docs/breadcrumb-programme-execution-policy.md`. PR-1 through PR-5 are merged. Breadcrumb programme PR-6—the archive-lifecycle stage owned by `DON-248`/`DON-252`/`DON-253` and distinct from merged WAR-01 GitHub PR #6—is being recovered on [PR #10](https://github.com/donal0c/sartracker-web/pull/10). Exact heads `caf9e5e480fcd02cc44d68c8397efcd6ae78f2cd`, `49523dc8b460a2080c5fbbd3bc11c961296f481d`, local `81e47973714ff5cbbd908329559009c281b352fe`, and local `74bdd95ca3bbd09f775ba111d53f6313e76769d6` are rejected diagnostics: caf9 crossed the greater-than-2-GiB archive boundary but failed teardown and real liveness; 495 passed Linux through tracking soak before its packaged lifecycle exposed phase-accounting ambiguity without writing a failure receipt; 81 emitted the new failure receipt and isolated a negative first renderer-frame sample before any archive operation to mixed RAF/performance clocks; 74 recorded healthy sub-34-ms liveness but rejected a normal in-flight source created between non-atomic renderer and source-ledger cuts before the operation began. The bounded recovery retains deterministic finalization lookup, mission-scoped logical cleanup with telemetry-only `mission_events` deletion, and constant-space diagnostics. Its red-first replacement preserves exact Traccar→main/preload→React→MapLibre identity plus main/renderer continuity across active phase handoffs, drains the operation ledger before close, excludes terminal harness closeout, keeps process/profile cleanup fail-closed, emits one atomic sanitized terminal artifact, and measures renderer frames on one monotonic clock. Causal exclusive-start/inclusive-end source fences now separate continuously polled global evidence from finite operation-fresh evidence without pausing polling, resetting continuity, or extending any source's original deadline; renderer collection and source correlation are serialized across explicit and watchdog drains. Secondary cleanup failures retain bounded sanitized per-step attribution. Repeated-correction lineage uses deterministic archive-owned unlock events and exact existing-index/rowid point reads, without a startup index, migration, history scan, or sort. The strict `<200 ms` gate is unchanged. A replacement commit plus all final exact-head package/browser/visual/Linux gates and four independent reviews remain pending, followed by exactly one fresh controlled field qualification. This remains pre-merge engineering work, not release or field acceptance. Donal retains approval/merge authority. After the exact merged head exists, run WAR-04B's narrow refresh before any release decision; BCP-17 must qualify one final candidate before `DON-255` may publish it.
...
The unmerged `DON-248`/`DON-252`/`DON-253` archive successor is in exact-head
qualification and review on PR #10. Its frozen implementation candidate and
proof remain pre-merge; `RPL-004` therefore stays `rewrite-pending` until merge.
That successor is the prerequisite for streamed, encrypted/locked archive and
restore-and-replay resilience. The WAR task
...
| Recovery in flight; final exact-head proof pending | [PR #10](https://github.com/donal0c/sartracker-web/pull/10): PR-6 archive lifecycle | S2 Electron / Archive / Mission Review / Verification | `DON-248` / `DON-252` / `DON-253` | Heads `caf9e5e8…`, `49523dc8…`, local `81e47973…`, and local `74bdd95…` are rejected diagnostics. Caf9 crossed 5.24 GB but failed teardown and liveness; 495 passed Linux through packaged tracking soak before ambiguous phase accounting and no failure receipt; 81's new receipt isolated a mixed-clock negative first renderer frame before any archive operation; 74 measured healthy sub-34-ms liveness but rejected a normal continuously polled source between non-atomic renderer/source cuts before operation start. Recovery retains deterministic boundary lookup, mission-scoped telemetry-only cleanup with audit/custody retention, and constant-space diagnostics. The local red-first replacement preserves exact-fix, main, and renderer continuity across phase changes; drains operation evidence before close; ends before unrelated closeout; keeps process/profile cleanup fail-closed; emits exactly one atomic sanitized terminal artifact; and measures renderer-frame endpoints on one monotonic clock. Causal source-sequence fences and serialized renderer/source correlation exclude pre-start and post-end straddlers from operation-fresh proof while retaining their global 200-ms liveness duty; finite in-window sources settle only to their original deadline, and secondary cleanup failures retain bounded sanitized per-step attribution. Repeated-correction lineage uses deterministic archive-owned unlock events and existing unique-id/rowid point reads, with no new startup scan. The strict `<200 ms` gate is unchanged. Replacement exact-head package, browser/visual, Linux, four-review, and exactly-one fresh controlled greater-than-2-GiB gates remain pending. Pre-merge proof is not release or field acceptance. |
...
| In review (programme PR-6) | Decide archive encryption, authenticity, custody, and emergency access | S2 Electron / Archive / Governance | `DON-248` | BCP-14 is implemented on PR #10 under the security decision and candidate evidence record. Human custody roles remain the non-blocking tabletop and are not guessed. Donal's approval/merge remains; no release proof is claimed. |
...
| In review (programme PR-6) | Create streamed encrypted mission archives with restore-and-replay proof | S2 Electron / Archive | `DON-252` | BCP-15 is implemented on PR #10 with bounded mission-only streaming and sealed-file exhaustive restore/verification. Candidate proof remains pre-merge and is not release proof; Donal's approval/merge remains. |
| In review (programme PR-6) | Add archive-backed review, visible revisions, and indefinite retention | S2 Electron / Governance | `DON-253` | BCP-16 is implemented on PR #10 with read-only archive Review, visible immutable supplements, and eligibility-gated logical cleanup. Resume is available only when an intact journal proves cleanup is in progress; invalid recovery state fails closed, and live-row membership drift requires re-finalization. Cleanup may remove archived mission rows, rebuildable derived projections, four explicitly settled operational tables, and only the allowed high-volume telemetry event types from `mission_events`. It retains the mission stub, archive/supplement registry, every non-telemetry mission audit event, and unknown future audit event types. Candidate proof remains pre-merge and is not release proof; Donal's approval/merge remains. |
