...
1. Execute the **Breadcrumb and Mission-History Programme** under the locked ADR and `docs/breadcrumb-programme-execution-policy.md`. PR-1 through PR-4 are merged. PR-5 mission evidence/replay (`DON-274`, `DON-277`, `DON-278`, `DON-279`) remains one review unit on `codex/breadcrumb-pr5-evidence-replay` from exact merged PR-4 base `80309c995a18eeb190cce4310c9a46b0f46d5263`. Exact head `a584b2aa` passed Linux run `33207406645` but all four reviewers rejected it with five accepted P2s; fix `12532025` closed those findings. Documentation-bound `d051e4c9` was then rejected before review because simultaneous exact-head Linux runs conflicted: `33213448567` passed, while slower-runner `33213464281` breached the 200 ms current-write gate and fixed settlement windows. Margin fix `7f14d518` reduces legacy-event turns to 250, lengthens GPX writer yields, removes multi-megabyte source duplication from the active projection while retaining the exact immutable revision, and extends only harness settlement time. Affected liveness tests passed three consecutive runs; full unit, lint/build, indexed 960k and rebuilt packaged soak are green. Documentation binding, push, isolated exact-head Linux and a restarted broad/persistence/concurrency/renderer wave remain task-owned completion gates. PR-6 archive lifecycle follows only after Donal accepts PR-5.
