...
1. Execute the **Breadcrumb and Mission-History Programme** under the locked ADR and `docs/breadcrumb-programme-execution-policy.md`. PR-1 through PR-5 are merged. Breadcrumb programme PR-6—the archive-lifecycle stage owned by `DON-248`/`DON-252`/`DON-253` and distinct from merged WAR-01 GitHub PR #6—is in review as [PR #10](https://github.com/donal0c/sartracker-web/pull/10). The current implementation/qualification head is `661305694d43682a6e4aa0e0fafeeb962affc7ea` / tree `0854dfd72b0b1c04c3e6fda0827ca9761060c6f5`; local single-worker deterministic proof is `359/359` files and `3,431/3,431` tests, with focused archive/lifecycle/qualification checks `71/71`, lint, TypeScript, Node syntax and diff checks green. The cleanup/live-ingest fix uses deferred read-then-write pages, finite transient `SQLITE_BUSY*` boundary retries and a 500-position default page without weakening the 200 ms heartbeat/current gate. Earlier browser/visual/package/kill/Linux evidence remains carry-forward from 3885 until exact candidate reruns. Ubuntu greater-than-2 GiB proof and the five exact-head reviews remain pending; after those results, freeze the final evidence/manual/handoff/Linear records and run final documentation-head rechecks. This is pre-merge engineering proof, not release or field acceptance. Donal retains approval/merge authority. After the exact merged head exists, run WAR-04B's narrow refresh before any release decision. BCP-17 must qualify one exact final candidate before `DON-255` may publish it.
...
| Frozen code head; final documentation/review wave pending | [PR #10](https://github.com/donal0c/sartracker-web/pull/10): PR-6 archive lifecycle | S2 Electron / Archive / Mission Review / Verification | `DON-248` / `DON-252` / `DON-253` | Code/qualification head `661305694d43682a6e4aa0e0fafeeb962affc7ea` (tree `0854dfd72b0b1c04c3e6fda0827ca9761060c6f5`) is pushed. Local serial deterministic gate `359/3,431`; focused archive/lifecycle/qualification checks `71/71`; lint, TypeScript, Node syntax and diff checks are green. The cleanup/live-ingest fix retains the hard 200 ms gate while using deferred boundaries, finite transient busy retries and 500-position pages. Earlier browser/visual/package/kill/Linux evidence is carry-forward from 3885 pending exact candidate reruns. Ubuntu greater-than-2 GiB qualification and all required exact-head reviews remain pending; after those results, commit the evidence/manual/handoff/Linear freeze and run final documentation-head rechecks. Pre-merge proof is not release or field acceptance. |
