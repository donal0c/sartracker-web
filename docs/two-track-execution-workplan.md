...
| In progress | Prepare Electron beta handoff release and Discovery map loading instructions | S2 Electron / S1 Maps | `DON-142` / `DON-25` | Active handoff runbook added at `docs/electron-beta-handoff.md`; local interim Linux/macOS artifact bundle generated under `tmp/don142-electron-handoff/`; release docs now warn that the old Tauri workflow is not the current Electron handoff path. Immediate route is private/direct artifact sharing for Mac/Linux; durable GitHub Release automation is split to `DON-143`; Windows remains `DON-141`. |
| Todo | Migrate GitHub release workflow to Electron desktop artifacts | S2 Electron / Release | `DON-143` / `DON-142` | Create or migrate an Electron-specific GitHub draft/prerelease workflow for app binaries and `SHA256SUMS` only. This is the durable download-link path; it is not required before privately sharing the current Mac/Linux validation bundle. |
