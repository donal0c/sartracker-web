...
1. Execute the **Breadcrumb and Mission-History Programme** under the locked ADR and `docs/breadcrumb-programme-execution-policy.md`. PR-1 through PR-4 are merged. PR-5 mission evidence/replay (`DON-274`, `DON-277`, `DON-278`, `DON-279`) remains one review unit on `codex/breadcrumb-pr5-evidence-replay` from exact merged PR-4 base `80309c995a18eeb190cce4310c9a46b0f46d5263`. Exact head `a584b2aa` passed Linux but all four reviewers rejected it with five accepted P2s; fix `12532025` closed them. `d051e4c9` was rejected on conflicting Linux evidence, and `84702866` was rejected before review when isolated run `33215754562` breached the object-backfill heartbeat gate and timed out event settlement. Executable fix `e9466e54` moves restartable legacy event/object/GPX reconstruction out of Electron main into a joined worker while keeping current writes prioritized and evidence/replay/finalization fail-closed. Three repeated five-test scale/liveness sets, full unit, lint/build, backend, fresh indexed 960k and rebuilt packaged soak are green. Documentation binding, push, isolated exact-head Linux and a restarted broad/persistence/concurrency/renderer wave remain task-owned completion gates. PR-6 archive lifecycle follows only after Donal accepts PR-5.
