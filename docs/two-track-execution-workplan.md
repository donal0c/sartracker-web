...
1. Execute the **Breadcrumb and Mission-History Programme** under the locked ADR and `docs/breadcrumb-programme-execution-policy.md`. PR-1 through PR-4 are merged. PR-5 mission evidence/replay (`DON-274`, `DON-277`, `DON-278`, `DON-279`) remains one review unit on `codex/breadcrumb-pr5-evidence-replay` from exact merged PR-4 base `80309c995a18eeb190cce4310c9a46b0f46d5263`. Exact head `356a9e03` passed Linux run `33198210110` but was rejected by all four independent reviewers. Executable remediation `9a1a54f8` closes those five findings; Linux then rejected `52465dfc` because a local Replay preload import could not load inside Electron's sandbox. Exact package reproduction preceded sandbox-safe fix `09845b8b`; its rebuilt two-launch/8,664-position soak is green. Linux next rejected `b59c54e1` when a 50-object legacy baseline turn breached the 200 ms event-loop gate; fix `fcf55f88` reduces the durable turn to 25 and is green through five focused 50k runs plus the full 2,480-test gate. Run `33206596556` showed the old 15-second harness loop ending at 42,500/50,000; test correction `84eb2245` retains complete 50k equality and the unchanged `<200 ms` gate while allowing slower runners 30 seconds. Documentation binding, push, fresh exact-head Linux and a restarted broad/persistence/concurrency/renderer wave remain task-owned completion gates. PR-6 archive lifecycle follows only after Donal accepts PR-5.
