...
| --- | --- | --- | --- | --- |
| Active | Deep breadcrumb correctness and cross-installation equivalence hotfix | S2 Electron / Shared Tracking / Verification | `DON-260` | `0.1.0-beta.12.1` release blocker. Core TDD implementation and local proof are complete: stable Traccar identity, correction audit, deterministic whole-route render selection, worker-backed restart reconstruction, bounded historical chunks with repeat reconciliation, strict malformed-row handling, five-minute stale semantics, stopped/replacement-runtime guards, explicit persistence/cache failure states, no-skip beta verification, multi-day packaged soaks, and same-session Fable pass. Runs `30402564688`, `30426564770`, and `30427996046` stopped before release/artifact promotion while successively exposing Linux WebGL blocklisting, desktop-session throttling, and an Azure SwiftShader/Vulkan fallback failure; each retained exact position truth, and the latter two completed 4/4 healthy trusted interactions. The red-to-green Linux CI harness now requests ANGLE/OpenGL and foreground-equivalent throttling guards without weakening the 1,000 ms gate. Its exact wrapper passes Ubuntu/Xvfb at 8,664 exact, 4/4 healthy, main 186.0 ms, renderer 166.6 ms / median 16.7 ms, and action 689.1 ms. Full committed-state beta verification passed 8/8 with no skips, and independent adversarial review found no P1/P2. Remaining closeout is replacement tag-driven CI, exact artifact qualification on Ubuntu, and controlled prerelease publication. |
