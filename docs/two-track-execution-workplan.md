...
1. Execute the **Breadcrumb and Mission-History Programme** under the locked ADR and `docs/breadcrumb-programme-execution-policy.md`. PR-1 through PR-5 are merged. Breadcrumb programme PR-6—the archive-lifecycle stage owned by `DON-248`/`DON-252`/`DON-253` and distinct from merged WAR-01 GitHub PR #6—is in review as [PR #10](https://github.com/donal0c/sartracker-web/pull/10). The prior candidate proofs and initial reviews are superseded by red-first qualification-harness remediation pushed at `6f911998`; durable probe writes now use a worker connection while the unchanged 200 ms current/heartbeat gate remains hard. Targeted qualification tests, lint and TypeScript are green; Ubuntu greater-than-2 GiB qualification is rerunning against this exact head, with the required five-charter review/recheck ledger still pending. This is pre-merge engineering proof, not release or field acceptance. Donal retains approval/merge authority. After the exact merged head exists, run WAR-04B's narrow refresh before any release decision. BCP-17 must qualify one exact final candidate before `DON-255` may publish it.
...
| In qualification rerun | [PR #10](https://github.com/donal0c/sartracker-web/pull/10): PR-6 archive lifecycle | S2 Electron / Archive / Mission Review / Verification | `DON-248` / `DON-252` / `DON-253` | Qualification-harness remediation is pushed at `6f911998`: durable probe writes use a worker connection and the 200 ms current/heartbeat gate is unchanged. Targeted qualification tests, lint and TypeScript are green; Ubuntu >2 GiB qualification and the five exact-head reviews remain pending. Prior browser/package/Linux proof is carry-forward evidence only until exact-head rerun. Pre-merge proof is not release or field acceptance. |
