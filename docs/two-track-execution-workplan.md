...
| Release candidate | Fix field-scale 36-hour breadcrumb catch-up, restart reliability, and route fidelity | S2 Electron / Shared Tracking / Verification | `DON-260` | Reopened from field evidence: 36-hour history took about nine minutes and arrived in blocks. Eamonn's 2026-08-10 follow-up added slow-speed clusters and larger gaps at 120–145 km/h. The correction separates initial catch-up from live polling, preserves two-hour/max-eight bounds, checkpoints durable frontiers, paces anti-entropy, prioritizes active devices, bounds renderer/query memory, and uses deterministic geometry-aware representatives. The proof drives one 12 km/h then 120–145 km/h vehicle and requires exact packaged dot-mode parity: both phases retain `720/720` source fixes with zero gap inflation. The frozen 33-device fault/SIGKILL/restart run passed current `99 ms`, first breadcrumb `345 ms`, exact `279,936`-row persistence in `37.148 s`, and exact `103,614` line/dot coordinates; the unchanged 14-day soak passed `1,935,384` exact rows, two restarts, integrity/WAL/responsiveness, zero crashes, and `1,533,214,720`-byte peak RSS. Automated beta gates, Chromium `142/142`, visual `37/37`, and independent review `43/43` are green. `electron-v0.1.0-beta.12.6` release preparation is active; exact CI AppImage/`.deb` Ubuntu qualification remains before publication. |
