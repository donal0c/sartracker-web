...
| Qualified draft | Fix field-scale 36-hour breadcrumb catch-up, restart reliability, and route fidelity | S2 Electron / Shared Tracking / Verification | `DON-260` | Immutable beta.12.9 commit/tag `32a4326442b9` / `electron-v0.1.0-beta.12.9` and workflow `31414363788` are green. Exact Ubuntu CI AppImage persisted all `279,936` rows in `46.287 s`, recovered from 503/SIGKILL, matched the canonical render, and restarted exactly at `5.667/5.557/5.649 s`; exact five-/fourteen-day soaks and the operator matrix pass. The real installed `.deb` is file-clean, independently persisted all rows in `46.447 s`, restarted exactly at `5.703/5.786/5.787 s`, and passed lifecycle/archive, coordinate, duplicate-launch, diagnostics, corrupt-secret, and live-provider gates. Apt status 100 is isolated to three pre-existing broken NVIDIA/kernel packages after successful SAR Tracker installation. Linear is updated. Only guarded publication, fresh-public checksums, and final public AppImage Ubuntu lifecycle smoke remain. |
