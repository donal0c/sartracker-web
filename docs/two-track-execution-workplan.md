...
| In Progress | Prevent multiple Electron instances sharing one runtime profile | S2 Electron / Runtime | `DON-180` / `DON-25` | Team beta.7 PCLinux report reproduced by clicking the launcher while SAR Tracker is already opening/running: two windows can exist and the second can hit the startup fault. Root cause: Electron main did not claim `app.requestSingleInstanceLock()`. Local fix adds the lock before runtime/window setup, quits duplicate processes before `whenReady()`, and restores/focuses the existing window on `second-instance`. Verified locally: red→green `electron-main-startup` regressions, focused runtime tests (4 files / 19), `node --check electron/main.cjs`, lint, build. Remaining gate: packaged Linux/AppImage smoke before beta.8 promotion. |
