...
| In progress | Prepare Electron beta handoff release and Discovery map loading instructions | S2 Electron / S1 Maps | `DON-142` / `DON-25` | Active handoff runbook added at `docs/electron-beta-handoff.md`; local interim Linux/macOS artifact bundle generated under `tmp/don142-electron-handoff/`; release docs now warn that the old Tauri workflow is not the current Electron handoff path. Decide direct interim sharing vs GitHub draft/prerelease after Windows gate. |
