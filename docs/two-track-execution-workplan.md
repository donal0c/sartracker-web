...
| P1 remediation — PR #3 open | PR-3: decide and deliver complete mission coverage | Shared Tracking / S2 Electron / Coverage / Verification | `DON-273` / `DON-276` / `DON-275` | Candidate B and G3 remain ratified; migration/scale proof stands for unchanged geometry/schema surfaces. Review of `fa863da` found that a marker-write failure could be acknowledged from RAM and that Finish could strand renderer-only evidence after the mission left active status. The red-first remediation makes evidence-loss marking strictly durable and puts Finish behind an all-batch drain/acceptance seal, leaving the mission active on failure and permitting later finalization from the sealed state. Full unit 272/2,194, lint, TypeScript, build/budgets, backend 51/51 plus one intentional ignore, Chromium 13/13, and selected critical visual/review 2/2 pass. Commit/push, unsigned macOS and exact-head Linux package/soak gates, and five fresh independent PR-3 reviews remain. The prospective PR-4 review amendment does not relax PR-3. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and unmerged. Do not merge or release from this task. |
