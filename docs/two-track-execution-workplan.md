...
| Recovery in flight; final exact-head proof pending | [PR #10](https://github.com/donal0c/sartracker-web/pull/10): PR-6 archive lifecycle | S2 Electron / Archive / Mission Review / Verification | `DON-248` / `DON-252` / `DON-253` | Heads `caf9e5e8…`, `49523dc8…`, local `81e47973…`, local `74bdd95…`, local `6a72ae91…`, and local `23161300…` are rejected diagnostics. Caf9 crossed 5.24 GB but failed teardown and liveness; 495 passed Linux through tracking soak before ambiguous phase accounting and no failure receipt; 81 isolated a mixed-clock renderer-frame sample; 74 rejected a normal non-atomic ledger join; 6a rejected a legitimately superseded intermediate latest-state snapshot despite healthy later exact renders; 231 wrote a complete cleanup-successful receipt with a 216 ms restore trigger that is harness-indeterminate because an independent main tick could overtake a timely but not-yet-drained renderer observation. Source retrace separately confirmed that durable snapshot settlement plus a new full interval could create a real current-cadence gap. Recovery retains deterministic boundary lookup, mission-scoped telemetry-only cleanup with audit/custody retention, and constant-space diagnostics. The local red-first replacement preserves exact-fix, main, and renderer continuity across phase changes; drains operation evidence before close; ends before unrelated closeout; keeps process/profile cleanup fail-closed; emits exactly one atomic sanitized terminal artifact; and measures renderer-frame endpoints on one monotonic clock. Causal source-sequence fences and serialized renderer/source correlation keep operation freshness exact. A valid exact renderer sequence may acknowledge only older latest-state snapshots still strictly inside their original deadlines; emission, churn, boundaries, and invalid/late acknowledgements cannot reset or extend the strict `<200 ms` gate, and sequence regression fails closed within and across drains. Current-fix continuity and source expiry are audited through a serialized renderer drain's conservative request-start watermark, while the next success interval consumes time already spent settling the visible snapshot without overlap or early evidence release. Queue acquisition and raw CDP work remain independently bounded; timeout poisons the channel. Renderer-owned handoff partitions fixes at an exact watermark, operation segments use immutable phase bounds, and a fenced pre/post-drain pause preserves terminal evidence plus an idempotent cleanup retry without post-pause freshness credit. Secondary renderer/cleanup failures retain bounded sanitized attribution. Current local gates are `2/145` focused, `10/477` expanded, `375/3,759` full serial, full static/build, backend `58 + 1 ignored`, and clean focused re-audits; these are pre-freeze dirty-tree checks only. Repeated-correction lineage uses deterministic archive-owned unlock events and existing unique-id/rowid point reads, with no new startup scan. Replacement exact-head package, browser/visual, Linux, four-review, and exactly-one fresh controlled greater-than-2-GiB gates remain pending. Pre-merge proof is not release or field acceptance. |
