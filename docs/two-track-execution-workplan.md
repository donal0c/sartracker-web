...
| Todo | Decide/fix Review workspace blocking active mission controls | S3 Web App / UI | `DON-176` / `DON-5` | Characterized on beta.6 (2026-06-16, still open). Browser-backed repro: full-screen `WorkspaceOverlay` backdrop (`src/components/workspace-overlay.tsx`) intercepts clicks outside the Review panel — with an active mission + Review open, mission Finish and the map are unreachable until Review is closed (Esc restores control). Confirms the field report is modal-overlay blocking, not the DON-148 freeze. Separate from DON-175; carried into beta.6 as a documented Known Limitation. Still needs product call (non-blocking/docked Review vs explicit modal blocking) + browser-backed regression. |
