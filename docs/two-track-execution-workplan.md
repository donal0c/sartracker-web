...
| Active — five exact-head reviews | PR-3: decide and deliver complete mission coverage | Shared Tracking / S2 Electron / Coverage / Verification | `DON-273` / `DON-276` / `DON-275` | Candidate B and G3's combined default posture are ratified; the v9→v10 migration proof stands. Review-wave mission/source/worker/style/filter-attestation gaps are red-first remediated through `4b740f2`; the accepted-write outing scan is bounded at `40a713c`. Review #4's missing initial direct-device checkpoint and unenforced open-outing cooldown are fixed red-first through `691775a`, with explicit Retry preserved. Final tool head `69a1096` explicitly budgets the resulting audited backfill completions. Candidate B's affected G2 rows remain within unchanged budgets, and `df61a02` still proves exact 960k/2M totals plus non-empty decoded geometry for every period. Full unit/static/Dots/build, full Chromium 158/158, coverage visual, macOS+Ubuntu packages, and the final replacement packaged soak are green. Complete five independent exact-head reviews, then open one PR to `master` if clean. |
