...
| Active | Qualify beta.12 CI artifact on Ubuntu and original field machine | S2 Electron / Release | `DON-247` | Exact artifact `fb6c4922…` passed the full Ubuntu matrix, was published as an internal prerelease, then was downloaded afresh from the release URL and passed the post-publication Ubuntu smoke. The 2026-07-20 field split is now explicit: PCLinuxOS AppImage stayed responsive at 218 hours, while a Mint `.deb` stopped accepting application actions around 182 hours even though the in-app clock continued. The packaged soak now probes a real, non-forced Devices open/close interaction at mission start, cumulative-load checkpoints, restart recovery, and final load, failing on blocked/slow controls and capturing a screenshot on input failure. A local packaged CI profile passed 4/4 interaction samples, 8,664 exact positions, restart, backup, main/renderer responsiveness, and zero redundant telemetry. Original Mint/profile AppImage-vs-`.deb` confirmation and support-bundle collection remain before closeout. |
