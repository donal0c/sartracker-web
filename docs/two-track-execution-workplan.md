...
| --- | --- | --- | --- | --- |
| Active | Deep breadcrumb correctness and cross-installation equivalence hotfix | S2 Electron / Shared Tracking / Verification | `DON-260` | `0.1.0-beta.12.1` release blocker. Core TDD implementation and local proof are complete: stable Traccar identity, correction audit, deterministic whole-route render selection, worker-backed restart reconstruction, bounded historical chunks with repeat reconciliation, strict malformed-row handling, five-minute stale semantics, stopped/replacement-runtime guards, explicit persistence/cache failure states, no-skip beta verification, multi-day packaged soaks, and same-session Fable pass. Runs `30402564688`, `30426564770`, `30427996046`, and `30429828590` stopped before release/artifact promotion while exposing Linux WebGL blocklisting, desktop throttling, and two Azure SwiftShader/Vulkan fallback failures; all retained exact truth and the latter three completed 4/4 healthy trusted interactions. The final red-to-green harness removes SwiftShader permission, provisions/scopes Mesa llvmpipe to Linux validation, retains ANGLE/OpenGL and foreground-equivalent guards, excludes recorder CDP overhead from click-to-state latency, and separately fails any renderer gap at the unchanged 1,000 ms threshold. Three consecutive Ubuntu/Xvfb rehearsals passed at 8,664 exact positions, 4/4 healthy interactions, 133.4-166.7 ms renderer maxima / 16.7 ms median cadence, and 600.9-776.6 ms action maxima with no Vulkan/SwiftShader errors. Full committed-state beta verification passed 8/8 with no skips from `sha.a852dc198832`, and independent adversarial review found no P1/P2. Remaining closeout is replacement tag-driven CI, exact artifact qualification on Ubuntu, and controlled prerelease publication. |
