...
| Release candidate | Fix field-scale 36-hour breadcrumb catch-up, restart reliability, and route fidelity | S2 Electron / Shared Tracking / Verification | `DON-260` | Reopened from field evidence: 36-hour history took about nine minutes and arrived in blocks, with slow-speed clusters and larger gaps at 120–145 km/h. The correction separates current fixes from catch-up, preserves two-hour/max-eight request bounds, checkpoints durable frontiers, paces anti-entropy, bounds memory, and uses deterministic geometry-aware representatives. Ubuntu rejected beta.12.6 because per-chunk FULL transactions left `268,275/279,936` rows at 60 seconds; `4896ecd` replaces them with one atomic persistence operation per fetched wave and local exact proof is now `28.119 s`. Beta.12.7 was not built because its 198,000-position GeoJSON test exceeded the generic CI five-second timeout at `5.627 s`; `21ba720` records a truthful 15-second budget for that intentionally large deterministic test. Beta.12.8 release preparation is active; fresh no-skip/tag CI and exact CI AppImage/real `.deb` Ubuntu qualification remain mandatory. |
