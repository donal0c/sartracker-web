...
| Recovery in flight; final exact-head proof pending | [PR #10](https://github.com/donal0c/sartracker-web/pull/10): PR-6 archive lifecycle | S2 Electron / Archive / Mission Review / Verification | `DON-248` / `DON-252` / `DON-253` | Rejected diagnostics now include the earlier caf9/495/81/74/6a/231/d91/7e heads, cadence-failing pushed `b75f8689…`, and proof-indeterminate local `b7793753…`; none will be rerun unchanged. B75 passed macOS/browser/visual/kill gates but Linux run `33940959449` proved a real 240 ms Review cadence breach. B779 contains the bounded current-publication/evidence/cache repair and packaged cleanly, but its sole two-launch lifecycle attempt exposed a cumulative-phase/named-operation proof race without any `>=200 ms` failure diagnostics. The red-first successor keeps exact mission/device/archive custody binding, requires resumed restore's own in-fence fix, establishes a new restore baseline before post-cleanup Review, and preserves attributable operation diagnostics before deletion. Existing source, continuity, main, renderer, cleanup, terminal-artifact, audit/custody, and strict `>=200 ms` gates remain unchanged. Successor pre-freeze gates are green at four lifecycle files `191/191`, wider affected `10/399`, full serial `375/3,795`, full ESLint, production build/budgets, backend `58 + 1 ignored`, Node/diff checks, and a clean focused review. Replacement exact-head package/lifecycle, browser/visual, physical kill matrix, Linux, four final reviews, and exactly one fresh controlled greater-than-2-GiB qualifier remain. Pre-merge proof is not release or field acceptance. |
