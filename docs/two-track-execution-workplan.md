...
| Active — final review restart | PR-3: decide and deliver complete mission coverage | Shared Tracking / S2 Electron / Coverage / Verification | `DON-273` / `DON-276` / `DON-275` | Candidate B and G3's combined default posture are ratified. T1 software/package/migration/soak gates passed at `d1e884b`. The failed first review wave's eight blockers are red-first remediated; production-path Ubuntu qualification at `d39732e` passes exact 960k/2M delivery, first-useful, complete, and main-gap gates. Restore the separate final flip, run exact-head gates, then restart all five reviews before opening the PR. |
