...
| --- | --- | --- | --- | --- |
| Active | Deep breadcrumb correctness and cross-installation equivalence hotfix | S2 Electron / Shared Tracking / Verification | `DON-260` | `0.1.0-beta.12.1` release blocker. Core TDD implementation and local proof are complete: stable Traccar identity, correction audit, deterministic whole-route render selection, worker-backed restart reconstruction, bounded historical chunks with repeat reconciliation, strict malformed-row handling, five-minute stale semantics, stopped/replacement-runtime guards, explicit persistence/cache failure states, no-skip beta verification, multi-day packaged soaks, and same-session Fable pass. Run `30402564688` blocked on Linux WebGL blocklisting. Run `30426564770` then rendered the real UI, preserved the exact 8,664-row digest, and completed 4/4 healthy interactions, but its headless window was background/occlusion throttled and one action exceeded the unchanged 1,000 ms gate. Both stopped before release/artifact promotion. A red-to-green foreground-equivalence harness fix now disables Chromium background/renderer/occluded-window throttling; actual Ubuntu/Xvfb proof passes 8,664 exact with 4/4 healthy, main 176.4 ms, renderer 149.9 ms, action 646.2 ms, and no desktop throttle. Remaining closeout is replacement tag-driven CI, exact artifact qualification on Ubuntu, and controlled prerelease publication. |
