...
| Review-ready | PR-1: trustworthy ingest and live safety | Shared Tracking / S2 Electron / Evidence | `DON-266` / `DON-267` / `DON-268` / `DON-269` | [PR #1](https://github.com/donal0c/sartracker-web/pull/1) remains unmerged after the requested-change pass. Mission-scoped rejection delivery, durable failure/evidence-loss markers, replay, Windows safety, bounded aggregate/page reads, all-invalid live handling, conservative stationary tolerance/accuracy/outlier behavior, restart identity, and the remaining main-isolate Review health scan are fixed with red-green coverage. A final packaged soak also caught and red-first fixed placeholder-device audit churn. Unit `1,685`, backend, Chromium `146`, tracking visual/review, BCP-10, lint/build, and packaging pass. Exact packaged code commit `f24906c425e0` preserved the 3.704 GB v7 fixture and 2,040,000-row digest through v8, held startup/Review main maxima to 43.4/20.2 ms while current fixes published during the worker read, returned exact audit/count results, and passed the two-launch 8,664-position soak with 4.5 ms main maximum and zero redundant audit slope. The cold audit worker remains about `7.5 s`; indexing/retention stays open in `DON-251`, and the 200 ms gate is not waived. |
