...
| In Progress | Make Breadcrumb Dots source-exact and independently release-gated | S2 Electron / Shared Tracking / Verification | `DON-260` | Beta.12.9 persisted exact history but rendered Dots from the bounded Line projection, producing pairs/groups with long cadence gaps. The replacement uses a mission-start-bounded exact SQLite page query, dedicated Dots source, <=10k automatic display, and exact Earlier/Later pages; Line alone remains simplified. Beta.12.10 passed Ubuntu 36h exactness but was rejected when its 1,935,384-fix Ubuntu traversal took 81.096 s against 60 s: every page rescanned all positions to resolve the mission roster. Beta.12.11 uses the indexed devices roster; the preserved 194-page query falls from 46.406 s to 22.335 s with the identical union. A clean verifier also caught and fixed non-home app profile paths in exported diagnostics. Combined unit `195/1,559`, TypeScript/lint/build/Node/diff, Playwright `179/179`, and visual review `43/43` are green. Next: rerun clean beta verification, build beta.12.11, then exact CI AppImage + installed `.deb` Ubuntu 36h/14d, package-safety, and live provider-to-map gates. Do not hand over beta.12.9 or beta.12.10 as the corrected build. |
