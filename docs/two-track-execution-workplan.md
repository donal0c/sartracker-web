...
The exact-head T1 migration/soak/software gates still precede G3; G3 still
blocks the separate final default-flip commit.
...
| Active | PR-3: decide and deliver complete mission coverage | Shared Tracking / S2 Electron / Coverage / Verification | `DON-273` / `DON-276` / `DON-275` | G2 selected Candidate B and Donal ratified B plus the unchanged budgets. `DON-273` and BCP-08 / `DON-276` are complete. BCP-09's flag-on UI, filters, honest progress/recovery flow, exact inspection entry, participant exclusion, focused Chromium/visual evidence, and manual are implemented. Exact-head T1 gates are next; G3 still blocks the separate default flip. |
