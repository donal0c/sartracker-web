...
| --- | --- | --- | --- | --- |
| Active | Deep breadcrumb correctness and cross-installation equivalence hotfix | S2 Electron / Shared Tracking / Verification | `DON-260` | `0.1.0-beta.12.1` release blocker. Core TDD implementation and local proof are complete: stable Traccar identity, correction audit, deterministic whole-route render selection, worker-backed restart reconstruction, bounded historical chunks with repeat reconciliation, strict malformed-row handling, five-minute stale semantics, stopped/replacement-runtime guards, explicit persistence/cache failure states, no-skip beta verification, multi-day packaged soaks, and same-session Fable pass. First release run `30402564688` passed the position/digest/restart/performance gates but correctly blocked when its Linux image blocklisted WebGL and the real operator probes could not find the React/map workspace; no release was created. The CI harness now uses the same Linux software-rendering flags as launch smoke, with red-to-green unit coverage and an actual Ubuntu/Xvfb pass at 8,664 exact positions and 4/4 healthy interactions without weakening thresholds. Remaining closeout is replacement tag-driven CI, exact artifact qualification on Ubuntu, and controlled prerelease publication. |
