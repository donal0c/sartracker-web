...
visual and packaged map-surface checks passed. Exact-head PR CI and a fresh
independent review remain pending; local package evidence is not candidate
qualification.
...
breadcrumb transport, are recorded in their terminal receipts. PR23's final
implementation head was `45420208`; terminal CI `34694049815` passed. Its
background-throttling, reload-cleanup, deadline and inactivity-watchdog repairs
are merged. The separate ~239 ms legacy recovery path and unchanged strict
`<200 ms` qualification remain outstanding; release remains HOLD.

This branch carries the WAR-02B bounded property and mutation controls. Its
focused receipt, exact-head CI evidence and remaining qualification boundary
are recorded in the assurance documents below. No new whole-candidate release
...
Use [Testing and review cadence](testing-and-review-cadence.md) for the current
risk-based execution loop and evidence reuse rules (recorded 2026-09-09).
...
unchanged strict `<200 ms` qualification remains later; Release stays HOLD.
That follow-up is now merged in PR23; the current queue does not claim final
...
at `302bdd040976bd370271cf5866549fa2a7e05ff5`. Repair Train B covered
`AUD-01`, `AUD-10`, and `AUD-05` under the GPX evidence-fidelity and
import/outing lifecycle boundary. PR19 is merged at `d20bae5f` from final
implementation head `f203869c`, with terminal CI `34694049815` passed. The
[Train B record](assurance/findings/repair-train-b.md) retains its evidence and
historical failures. WAR-06's merged characterizations remain investigation-only
evidence, with production repairs outside this PR; its hazards are not closed
by Train B.
...
head. The in-repo negative-control receipt and executable-head CI run
...
remain explicit gaps; PR #19 is already merged at `d20bae5f` with terminal CI
`34694049815` passed. Its implementation evidence and historical timing failures
remain in the [Train B record](assurance/findings/repair-train-b.md). The latest
qualification comment retains the 205 ms strict-gate failure. Any DON-254 repair
and qualification remains downstream of this evidence.
...
Focused local evidence is 4 files / 19 tests, full correctness 438 files /
...
Train A subsequently repaired `AUD-13`, `AUD-02`, and `AUD-03`. The six
remaining confirmed findings proceed through Repair Trains C–D: map
interaction/rendering and mission progress/Review. Repair Train B's historical
evidence remains in its linked record.
Prerequisite-satisfied WAR investigation may continue around them when file and
state ownership is disjoint.

BCP-17 and whole-candidate qualification are held until every P1/P2 audit
finding and applicable WAR-04 release-blocking remediation is closed or has an
explicit, policy-valid non-blocking disposition. Qualification evidence created
before those code changes is not final-candidate evidence.
...
1. Breadcrumb PR-1 through PR-6, Team Feedback Batches 1–2, WAR-11A,
   WAR-02A, Repair Train A, PR19, PR22, PR23, WAR-06, PR21 and Repair Train B
   are merged through current `origin/master` at `1fac099a`. Use [Testing and
   review cadence](testing-and-review-cadence.md) for new work. The active
   DON-254 queue is separate follow-up B legacy recovery (~239 ms), then
   unchanged strict `<200 ms` qualification. Repair Trains C/D and applicable
   WAR remediation retain their own scope; BCP-17 final qualification runs only
   after those release-blocking repairs are dispositioned. Release remains
   HOLD; merge does not establish release or field acceptance.
...
`DON-254`/`DON-255`. PR #10's `DON-248`/`DON-252`/`DON-253` archive/restore
qualification is separate pre-merge engineering proof; this shadow-use protocol
does not supply or promote it.

### WAR-01 assurance baseline
...
PR #10 has since merged. The targeted [post-PR6 WAR-04B refresh](assurance/findings/WAR-04B.md)
reclassifies RPL-004 as an evidence-tier gap: implementation is delivered, while
exact-candidate qualification and the fresh local timing rejection remain open. The WAR task
...
[PR #8](https://github.com/donal0c/sartracker-web/pull/8) is the merged
investigation-only WAR-04 result on exact base
`3d0d36b3874947d3d620bdb5262d9cd2d7233fcf`. It merged from final head
`3a2278ee8804a9ded0f2fd26626c4c00743c05a6` at
`341d95add5a7eceb6db506a2afd0ea70cb1fc944` and records nine confirmed local
...
silently absorbed by WAR-04. Two independent final-head reviews were clean and
Electron Linux Validation Build `33302709964` passed. That is branch/package-
integrity evidence only: the isolated probes remain deliberately red, no
...
| Done | [PR #10](https://github.com/donal0c/sartracker-web/pull/10): PR-6 archive lifecycle | S2 Electron / Archive / Mission Review / Verification | `DON-248` / `DON-252` / `DON-253` | Merged at `e0ead688` on 2026-09-09 from tested head `1b1f86ee`. Final Linux CI `34324371898` passed 4,137 tests, build/lint, 960k replay and packaged tracking/archive/AppImage checks; local browser/visual and macOS smoke passed. All 95 findings are dispositioned in the [complete ledger](breadcrumb-pr6-complete-review-ledger.md). Historical failures remain archived. Release qualification remains separate. |
| Done | [PR #6](https://github.com/donal0c/sartracker-web/pull/6): WAR-01 whole-application assurance charter and stable hazard register | Whole application / Assurance / Verification | Existing owners are cited per hazard; no new issue | Documentation/investigation only, merged at `182d077804498f5c10aaf0b8b2c1525f29a31da7`. Covers all 12 hazard domains and the five absolute blockers. Post-PR5 reconciliation is pinned to merge `eec92812b783a795c093f37268b295dd2179a3af`; merged PR5 rows carry inspected control/evidence limits, `RPL-004` remains `rewrite-pending` at that historical snapshot for the then-unmerged `DON-248`/`DON-252`/`DON-253` archive successor, and the required broad/focused exact-head reviews were clean. Archive PR #10 has since merged; WAR-04B's narrow merged-head refresh remains next. |
...
| In Progress | [DON-264](https://linear.app/donal-oc/issue/DON-264): surface persistent overlay synchronization failures in diagnostics and map health | S2 Electron / Shared Map / Diagnostics | `DON-264` | Rebased `codex/don-264-overlay-warning` onto current master `a81dd4a388196d241a48205ad45e961c1ab26c9b`. The quick re-registration regression is red at `a0e17d9f` and green at the streak fix; the independent review's concurrent-warning clipping finding is covered by a bounded keyboard/wheel-scroll region and ten-warning browser regression. Full correctness passed (570 files; 5,832 passed; 25 skipped), lint, Chromium + visual warning/recovery, production build/macOS arm64 `electron:pack`, and local packaged C14 warning/recovery/cleanup (`releaseEligible:false`). Exact-head PR CI and fresh review of the final pushed head remain pending; package evidence is not candidate qualification. |
...
| Done locally | Remove the mission-store backup freeze from periodic autosave | S2 Electron / Persistence | `DON-240` | Full scans removed from autosave; 100-byte O(1) sanity check and worker-thread copy preserve atomicity. Replacement exact-CI Ubuntu qualification is complete; original-team-machine release confirmation remains `DON-247`. |
...
| Done | Decide archive encryption, authenticity, custody, and emergency access | S2 Electron / Archive / Governance | `DON-248` | BCP-14 is implemented on PR #10 under the security decision and candidate evidence record. Human custody roles remain the non-blocking tabletop and are not guessed. Merged in PR #10 on 2026-09-09; no release proof is claimed. |
...
| Backlog | Oversized legacy database assessment and recovery | S2 Electron / Recovery | `DON-250` | Owns mission-state-aware oversized-store recovery and any safe physical-compaction route with `DON-251`; never abandon operational data or run in-process multi-GB `VACUUM`. PR #10 performs logical post-archive cleanup only and does not promise file shrink. |
...
| Done | Add tracking poll-window ledger to diagnostics reports | S2 Electron / Tracking / Diagnostics | `DON-229` | Done 2026-07-12. Support and incident reports now contain a bounded, session-scoped ledger of poll cycles, anonymized breadcrumb cursor/window ranges, and request retry attempts, including safe phase, duration, returned/accepted/duplicate counts, retry delay, recovery timing, and allow-listed failure class. Incident exports explicitly report retained-window coverage. Device identity, coordinates, credentials, provider URLs, and raw error text are excluded. A DON-228 regression proves the next poll records the inclusive previous cursor/window and counts its boundary duplicate; a browser regression takes live tracking from online to HTTP 503/offline and verifies the copied evidence excludes the configured username. Verified with focused units `77/77`, full unit `1150/1150`, lint, build, backend `47 passed / 1 ignored`, Chromium `130/130`, and Electron packaging. |
| Done | Make breadcrumb dots deterministic and faithful across installations | Shared Tracking / Map Runtime | `DON-259` | Done locally 2026-07-20. Field evidence showed grouped breadcrumbs and apparently different visible representatives across two Linux installations. A red regression confirmed dot-mode GeoJSON deliberately dropped closely spaced retained fixes under the completed `DON-126` distance-decimation rule. The correction removes that second omission boundary: every position in the bounded per-device live snapshot now becomes one dot, while raw accepted positions remain complete in SQLite and existing long-mission render budgets remain explicit in diagnostics. Verification: focused red-to-green unit and Chromium regressions; lint; production build/bundle budgets; full unit `164 files / 1148 tests`; backend `47 passed / 1 ignored`; packaged Electron CI soak with 8,664 exact positions and 4/4 operator interactions; final serialized Chromium `132/132`; visual `35/35`; independent critical visual review `40/40`. |
| Done | Harden Traccar breadcrumb cursors and session/speed semantics | S2 Electron / Shared Tracking | `DON-233` / `DON-234` | Done locally 2026-07-06. Breadcrumb cursors now overlap, clamp future timestamps to the completed fetch window, advance from the max timestamp in unsorted batches, avoid truncated fallback after seed failure, canonicalize timestamps, and keep same-second distinct source positions through accumulation and mission persistence. Traccar session 401/403 clears stale cookies and reauthenticates; request timeouts clear on failure; API speed is normalized from Traccar knots to displayed/stored km/h. Verified with red-to-green tracking/client/Electron regressions, lint, build, full unit 152/1028, backend 47 passed / 1 ignored, full Playwright 162/162, visual review critical gate overall PASS, Electron package, packaged launch, packaged bad-secret smoke, and in-app Browser validation with mock Traccar online. Official offline smoke needs a private MBTiles package. |
...
| Done | App-owned official map library copy and package management | S1 Maps / S2 Electron | `DON-110` / `DON-7` | Done 2026-06-06. Electron imports official `.mbtiles` packages into `userData/official-map-packages/`, preflights disk space, dedupes/replaces by map id, validates the copied package on save, removes registrations from Settings, cleans up removed app-owned files, and reports safe package size metadata without exposing paths. Browser mode remains public-map-only. |
| Done | Official map package coverage manifest and readiness certificate | S1 Maps | `DON-111` / `DON-7` | Done 2026-06-06. Display/export safe bounds, zooms, tile count, status, and current-view coverage so operators can prove the map covers the search area. |
...
| Backlog | Admin package preparation workflow for standard and mission-area official maps | S1 Maps | `DON-113` / `DON-7` | Parked after Electron beta handoff. Useful future admin/back-office work for repeatable package generation, but not blocking team testing because operators can import their own private Discovery packages now. |
...
| In Review | Fix breadcrumb history truncation across multi-device missions | S2 Electron / Shared Tracking | `DON-159` / `DON-25` | Implemented locally. Live breadcrumbs are now retained with a per-device, full-window render budget that preserves each device's first/last points and route coverage instead of one global last-20k slice. Diagnostics/support reports include per-device retained/observed/truncated counts, and runtime persistence now uses raw newly fetched breadcrumbs before render budgeting. Verified: focused DON-159 unit set, full `npm run test` (149/856), `npm run lint`, `npm run build`, targeted diagnostics E2E, full Chromium E2E (107), and `npm run test:backend` (47 passed / 1 ignored). Awaiting team retest in the next Electron beta. |
| Done | Make breadcrumb accumulation truly incremental | S2 Electron / Shared Tracking | `DON-165` / `DON-25` | Done 2026-06-20. Replaced the poller hot-path whole-history rebuild with a stateful per-device accumulator that parses and merges only incoming positions on steady-state appends while preserving DON-159 per-device fairness, first/last retention, metadata, and chronological render output. Verified with red-to-green scale regression, focused tracking units, full unit, lint, build, full Chromium E2E, visual tracking coverage, and visual review. |
| Done | Bulk-persist tracking snapshots | S2 Electron / Tracking Persistence | `DON-200` / `DON-25` | Done 2026-06-20. Tracking runtime now dedupes raw breadcrumbs/current fixes into a staged batch and uses an optional mission-store bulk write when available; Electron persists rows in one SQLite transaction with reused statements, duplicate skipping, device `last_seen` updates, and existing `position_recorded` telemetry semantics. Verified with start-runtime and real Electron SQLite regressions, full unit, backend, lint, build, Chromium E2E, and local `electron:pack`. |
| Done | Reuse readonly MBTiles handles in official-map proxy | S2 Electron / S1 Maps | `DON-201` / `DON-7` | Done 2026-06-20. Electron official-map proxy now caches readonly MBTiles readers/statements per ready package path and safe settings fingerprint, invalidates/close handles on package replacement/removal/error/app quit, and preserves local-hit, local-miss online fallback, missing-package, unreadable-package, and credential-redaction behavior. Verified with injected reader cache tests, real test MBTiles coverage, official-map focused units, lint, build, full Chromium/visual map coverage, and local `electron:pack`. Private Discovery package smoke was not run. |
| Done | Count Mission Review positions without loading every row | S3 Web App / S2 Electron | `DON-202` / `DON-5` | Done 2026-06-20. Mission Review now requests `countPositions()` for breadcrumb summary instead of `listPositions()`, while tracking resume/cursor seeding keeps full-position access. Electron implements `SELECT COUNT(*)`, browser harness mirrors the contract, and Tauri falls back through its existing read path. Verified with red-to-green Review runtime and Electron SQLite count tests, Mission Review model updates, full unit, backend, Chromium Mission Review E2E, visual review, lint, and build. |
| Done | Operator workflow hardening batch | Track A / Shared UI / Map Tools / Settings | `DON-203` / `DON-204` / `DON-205` | Done 2026-06-20. Docked Mission Review no longer closes underneath a top dialog/confirmation on the same Escape press; Settings now prompts before discarding unsaved edits; Text Label drag hit-testing uses rendered label hits plus a conservative visible-text fallback, suppresses the post-drag click, and respects workspace/dialog boundaries so overlays cannot move labels underneath them. DON-190 Devices workspace behavior was revalidated without code changes. Manual updated for Settings discard confirmation. Visual marker capture and paused-finish timing assertions were hardened after validation exposed harness timing drift. Verified with red-to-green DON-203/DON-204/DON-205 regressions, focused Devices revalidation, lint, build, full unit (152 files / 976 tests), backend 47 passed / 1 ignored, full Chromium 126/126, visual 34/34, visual review 39/39, Electron syntax checks, and local unsigned `electron:pack`. Web/frontend validation plus local packaging is sufficient because no Electron IPC contract, SQLite schema, filesystem picker, diagnostics export, launch singleton, offline-tile protocol, or direct Traccar network boundary changed. |
| Done | Runtime safety bug sweep fixes | S2 Electron / Tracking / Runtime / Persistence | `DON-206` / `DON-207` / `DON-208` / `DON-209` | Done locally 2026-06-20. Traccar list responses now tolerate malformed rows without dropping valid device/current/breadcrumb data; provider URLs with embedded credentials are rejected, redacted, and blocked from live runtime startup if manually persisted; corrupt tracking/autosave intervals are normalized before runtime scheduling; Electron finalization retries recover from an interrupted archive-success state without duplicating archive-success/finalized events. Manual and handoff updated. Verified with red-to-green focused regressions (7 files / 94 tests), lint, build, full unit (152 files / 991 tests), backend 47 passed / 1 ignored, full Chromium 126/126, visual 34/34, visual review 39/39 after prompt/capture hardening, local unsigned `electron:pack`, packaged bad-secret smoke, and `beta:verify --no-smoke` report `tmp/beta-artifacts/verify-0.1.0-beta.7-sha.cfe1fa36752f-2026-06-20T13-49-43Z.json`. Official private MBTiles smoke and manual beta smoke were not run. |
| Done | Fix Linux keyring decrypt failure blocking Electron startup | S2 Electron / Runtime | `DON-175` / `DON-25` | DONE 2026-06-16. Undecryptable stored Traccar secrets now disable tracking with an operator warning instead of throwing out of runtime bootstrap; explicit `trackingConfig: null` no longer falls through to env config. Added `npm run electron:smoke:bad-secret`, beta handoff/runbook/manual updates, and unit regressions. Verified locally (focused tests, `node --check`, lint, build, `npm run test` 150/925, backend 47) AND packaged on the CI-built `electron-v0.1.0-beta.6` AppImage (run `27596864861`, checksum OK, Ubuntu 24.04.2 real Wayland): bad-secret/keyring gate PASS + core lifecycle, archive, coordinate rejection, sanitized diagnostics, live Traccar online (33 devices / 8 fixes). beta.6 draft prerelease held pending Donal promotion approval. |
| Done | Fix Review workspace blocking active mission controls | S3 Web App / UI | `DON-176` / `DON-5` | DONE 2026-06-16 in beta.7. Non-blocking docked Review (decision locked with Donal): opt-in `docked` mode on `WorkspaceOverlay` drops the full-screen backdrop/focus-trap; Review docks left over the map while active/paused so the Pause/Finish rail and a map band stay live; read-only note; document-level Esc close. Verified: E2E red→green (`mission-review` 5/5) + packaged Ubuntu smoke on CI beta.7 (run `27601812958`) docked Review 6/6. Other workspaces stay modal. |
| Done | Replace keyring-dependent runtime credentials with app-owned local storage | S2 Electron / Runtime | `DON-177` / `DON-25` | DONE 2026-06-16 in beta.7. App-owned `credentials.json` (0600) under userData; presence-authoritative read with no keyring call; one-time migration from legacy `secrets.json` (decryptable→migrate + leave intact (decision (a)); undecryptable→disabled+warn, no throw); Clear = authoritative empty entry; dropped `basic_text` save-block; diagnostics `credential storage: local-file`, secret redacted. Supersedes DON-31 safeStorage design + the DON-175 stop-gap guard. Verified: strict TDD 931 unit tests, lint, build, backend 47; packaged Ubuntu smoke on CI beta.7 — credential matrix 3/3, real-keyring migration via live Traccar (33 devices/8 fixes), bad-secret PASS, diagnostics sanitized. Residual: local plaintext (best-effort 0600), intentional trade for reliability. |
| Done | Prevent multiple Electron instances sharing one runtime profile | S2 Electron / Runtime | `DON-180` / `DON-25` | Done 2026-06-23. Team beta.7 PCLinux report reproduced by clicking the launcher while SAR Tracker is already opening/running: two windows could exist and the second could hit the startup fault. Root cause: Electron main did not claim `app.requestSingleInstanceLock()`. Fix claims the lock before runtime/window setup, quits duplicate processes before `whenReady()`, and restores/focuses the existing window on `second-instance`. Verified locally with red-to-green `electron-main-startup` regressions, focused runtime tests (4 files / 19), `node --check electron/main.cjs`, lint, and build. The beta.8 deep Ubuntu packaged smoke then passed DON-180 duplicate-launch coverage on the CI-built artifact as part of the 43/43 release matrix, and Linear is closed. |
| Done | Install-tests beta.8 bug batch | Track A / Shared / Tracking / Layers | `DON-184`-`DON-189` | Done 2026-06-20 in six separate commits. Fixed app-chrome clicks opening Marker Details, first tracking update camera auto-fit, Layer Tree refresh scroll reset, Search Sector radius draft loss, and sparse breadcrumb line rendering. Verified with per-bug Playwright regressions, focused unit tests where applicable, `npm run lint`, `npm run build`, `npm run test` (150 files / 940 tests), `npm run test:backend` (47 passed / 1 ignored), and relevant Chromium specs 39/39. |
| Done | Devices workspace layout and selected-list behavior | Track A / Tracking UI | `DON-190` | Done 2026-06-20. Devices list tabs now keep selection inside the visible list; search is scoped to the active list/tab; selected-device Zoom action stays visible at constrained desktop width. Manual updated. Verified with red→green unit tests, Devices workspace Chromium 8/8, lint, build, full unit (150 files / 943 tests), backend 47, and full Chromium 117/117 after an unrelated mission timing flake passed in isolation. |
| Done | Map Tools and RHS Tools consolidation | Track A / Map Tools | `DON-191` | Done 2026-06-20. Map Tools now owns Measure details/clear controls and Marker at GR; RHS Tools and Focus Mode Tools no longer duplicate those controls and retain GPX import space. Measurement is one-shot after the second point, uses the operational crosshair cursor, and map labels are larger. Manual and screenshot updated. Verified with red→green unit tests, focused Chromium sweep 33/33, lint, build, full unit (150 files / 945 tests), backend 47, full Chromium 117/117, visual app-shell 5/5, and visual review PASS for `shell-drawing-toolbar`. |
| Done | Mission-control, helicopter, and top-panel layout simplification | Track A / Shared UI | `DON-192` | Done 2026-06-20. Minimized active Mission Control now moves fully into the command mast with a readable Expand affordance; Pause/Finish are absent while minimized and Focus Mode keeps Mission Control expanded. Helicopters moved from Tracking to Tools in standard and Focus Mode sidebars, and the mast was trimmed while keeping the minimized mission cell legible. Manual updated. Verified with red→green unit and Chromium coverage, lint, build, full unit (150 files / 946 tests), backend 47, full Chromium 117/117, visual mission lifecycle 7/7, app-shell visual 5/5, and visual review PASS for `mission-minimized-mast-state` + `shell-idle-state`. |
| Done | Tracking stale-state visibility and map-label readability | Track A / Shared Tracking / Layers | `DON-193` | Done 2026-06-20. Tracking offline/paused-refresh warnings now use prominent red alert treatment near the Tracking System header; device map labels use dark text with a white halo; breadcrumb casing is narrower and less visually dominant; Layer Tree separates `Current Location` from `Breadcrumbs` with independent per-device visibility while Mission Review de-duplicates those device entries. Manual text and tracking screenshot updated. Verified with red→green unit tests, lint, build, full unit (151 files / 951 tests), backend 47, full Chromium 118/118 after one existing paused-mission timing assertion first passed 3/3 in isolation, visual tracking 7/7, and visual review PASS for `tracking-offline-alert`, `tracking-paused-refresh-alert`, `tracking-map-devices`, and `tracking-layer-panel`. Web/frontend validation is sufficient for this chunk because no Electron IPC, SQLite, filesystem, diagnostics export, packaging, launch path, or direct Traccar network boundary changed. |
| Done | Layer Tree and GPX readability controls | Track A / Shared GPX / Layers | `DON-194` | Done 2026-06-20. Imported GPX files now expose individual palette/hex colour controls and persist colour in existing GPX metadata for MapLibre line styling; Layer Tree rows and checkboxes are larger and more readable at smaller desktop sizes; muted Mission Review detail text was lifted one contrast step. Manual updated. Verified with red→green unit and Playwright coverage, focused related units 9 files / 72 tests, focused Chromium GPX/layers/review 15/15, visual GPX/layers Playwright 1/1, visual review PASS for `gpx-colour-controls-small-display` and `layer-tree-readable-small-display`, lint, build, full unit (151 files / 953 tests), backend 47, and full Chromium 119/119. Web/frontend validation is sufficient because no Electron IPC, SQLite schema/migration, filesystem picker, diagnostics export, packaging, launch path, or direct Traccar network boundary changed. |
| Done | Map shell readability and smaller-display scaling | Track A / Shared Map Shell | `DON-195` | Done 2026-06-20. Added a visible high-contrast metric map scale derived from the current MapLibre camera; centred the DD / Irish Grid / DMS coordinate group in the lower map strip while keeping Convert on the right; recorded that Discovery grid lines are package/source dependent rather than app-drawn in this beta. Manual updated. Verified with red→green scale-readout unit coverage, full map Chromium 15/15, normal+small visual app-shell Playwright 2/2, visual review PASS for `shell-idle-state` and `shell-map-scale-small-display`, lint, build, full unit (152 files / 956 tests), backend 47, and full Chromium 120/120. Web/frontend validation is sufficient because no Electron IPC, SQLite, filesystem, diagnostics export, packaging, launch path, offline tile protocol, or direct Traccar network boundary changed. |
| Done | Drawing detail panels and required-field emphasis | Track A / Shared Drawings | `DON-196` | Done 2026-06-20. Text Label removes derived Anchor/Rotation controls; Range Rings remove derived Centre/Mode and highlight required Name/Radius; Search Sector replaces derived Centre/Radius cards with an Irish Grid readout; Search Areas default red and can hide their map label while preserving the drawing name. Search Area manual label-positioning/zoomed-out drift split to `DON-214`. Manual updated. Verified with red→green unit tests, focused DON-196 Chromium and visual Playwright, visual review PASS for four DON-196 screenshots, lint, build, full unit (152 files / 963 tests), backend 47, and final full Chromium 121/121 after unrelated DON-189/DON-193 timing checks passed in isolation and then in the final full run. Web/frontend validation is sufficient because no Electron IPC, SQLite schema/migration, filesystem, diagnostics export, packaging, launch path, offline tile protocol, or direct Traccar network boundary changed. |
| Done | Marker and casualty display terminology improvements | Track A / Shared Markers | `DON-197` | Done 2026-06-20. Casualty marker `Condition` is now `Casualty Status`; status order is Lost, Crag Fast, Medical Emergency, Unknown, Deceased; Evacuation Priority order is Normal, Urgent, Walk-Off, None, Self-Evacuation. Added persisted per-marker `label_size` with casualty default 16 and operator-adjustable Map Label Size, carried through browser harness, Electron SQLite schema 4, Rust/Tauri schema, GeoJSON, and MapLibre labels. Manual updated. Coordinate-field simplification remains `DON-135`. Verified with red→green unit tests, focused casualty Chromium and visual Playwright, visual review PASS for `marker-casualty-dialog` and `marker-casualty-map-label-size`, lint, build, full unit (152 files / 968 tests), backend 47, and full Chromium 121/121. |
| Done | Mission preview, map sharing, and external-resource workflow decisions | Track A / Shared Workflow | `DON-198` | Done 2026-06-20 as a decision/split-ticket item. Recorded `docs/mission-preview-external-resources-decision.md`: future Mission Preview should be read-only and reuse the mission overlay stack; map sharing should be a local export/print workflow rather than WhatsApp-specific sending; beta.8 external resources should stay with imported GPX/layer visibility until a live-resource ownership model is decided; evacuation/gear logs need a separate workflow decision; `DON-100` remains canonical for multi-day grouping. Follow-ups: `DON-215`, `DON-216`, `DON-217`, `DON-218`. No runtime behavior changed. |
| Done | Coordinator and settings access-control decision | Track A / S2 Electron / Settings | `DON-199` | Done 2026-06-20 as a decision/split-ticket item. Recorded `docs/settings-access-control-decision.md`: current Settings/admin roster is an audit/workflow guard, not authentication; future work should protect coordinator/admin roster edits, mission-affecting settings, finalized-mission unlock, and advanced repair actions; beta direction is a lightweight local privileged-settings guard, not full multi-user auth; mission unlock should stop relying only on the live mutable Settings roster. Follow-ups: `DON-219`, `DON-220`, `DON-221`. Manual known gap updated. No runtime behavior changed. |
...
#### Beta.12.3 — breadcrumb safety hotfix (`DON-260`)
...
or archive redesign owned by `DON-248` through `DON-255`.

Locked invariants:

1. Preserve the Traccar position ID separately from the local SQLite row ID.
2. Identical accepted input selects the same ordered visible identities across
   polling batches, response permutations, restart, OS, and profile.
3. Same-time distinct fixes remain distinct; corrected source IDs update one
   authoritative row and append an audit event.
4. Current fixes publish before slow history work. History requests are fixed
   two-hour chunks and repeat mission-wide so late fixes older than the live
   overlap are eventually reconciled.
5. Restart reconstruction may be O(history) in a read-only worker, but must
   never perform O(history) work on Electron's main isolate or transfer an
   unbounded render payload.
6. Full accepted position truth remains in mission SQLite. The map is bounded
   to 5,000 deterministic representatives per device and tells the operator
   when display simplification is active.
7. Stopped, superseded, or cross-mission runtimes cannot publish or persist
   stale work. Replacement runtimes serialize persistence/cache side effects.
8. Release proof includes two-profile identity equality, source coordinates and
   fix times, restart/correction/late-arrival cases, five- and fourteen-day
   packaged soaks, visual review, full `beta:verify`, and exact CI artifact
   smoke on Ubuntu.

Non-goals: full Discovery map loading, destructive storage cleanup, live-store
...
request is unresolved. A completed empty result is healthy, not permanently
...
- `DON-29` — S8d runtime decision checkpoint. Done 2026-06-06; Electron confirmed as production shell.
- `DON-30` — S8e ongoing desktop runtime support plan. Done 2026-06-06; policy at `docs/desktop-runtime-support-policy.md`.
...
- `DON-109` — official map setup wizard and package import UI.
