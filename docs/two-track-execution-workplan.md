...
1. Publish the fully qualified `0.1.0-beta.12.5` draft under `DON-261`, `DON-262`, and `DON-263`. Exact commit `042d77ad6158`, clean no-skip verification, tag CI `30497318233`, exact CI AppImage, and real installed `.deb` have passed the complete Ubuntu matrix. `DON-264` separately tracks non-blocking persistent overlay-sync diagnostic elevation.
