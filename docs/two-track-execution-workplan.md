...
| Active | Fable deep-analysis remediation queue | Shared / S2 Electron / Tracking / Coordinates | `DON-230` | Parent queue created 2026-07-06 from `output/fable-deep-analysis.md`; children `DON-231`-`DON-239` group the findings by shared implementation and verification path. `DON-231` through `DON-238` are complete locally. Continue with remaining child `DON-239`. |
