...
1. Execute the **Breadcrumb and Mission-History Programme** under the locked ADR and `docs/breadcrumb-programme-execution-policy.md`. PR-1 through PR-5 are merged. Breadcrumb programme PR-6—the archive-lifecycle stage owned by `DON-248`/`DON-252`/`DON-253` and distinct from merged WAR-01 GitHub PR #6—is in review as [PR #10](https://github.com/donal0c/sartracker-web/pull/10). The reviewed executable candidate is `6c26a1746bf5ee97648285eab680f6deff7e14ae` / tree `b6678f7b8fd6d333ada58b60b48057e94783bcd1`; correction custody, archive-family serialization, durable correction authorization outside the bounded audit page, browser verifier persistence/current-predecessor enforcement/supplement provenance, post-commit worker-exit classification, cleanup-retry evidence reopening, truthful operator recovery, all-mutation writer fencing, mutable recovery-code handling across archive workers, bounded cleanup attribution, cleanup-diagnostic-first classification, precise SQL-boundary stage attribution, and qualification-safe liveness measurement are red-first tested. Fresh broad, renderer/input, persistence and concurrency exact-head reviews are clean on this executable head; crypto/lifetime checks are `114/114`. The full serial unit gate remains `369/369` files and `3,530/3,530` tests; browser/visual/package results from 3d5 are retained as prior-head evidence only, while the final exact-head reruns are pending. Exact-head Linux workflow `33803457910` is green for 3d5 with clean source binding. The superseded Ubuntu run was stopped before evidence completion; one final >2 GiB qualification is pending after the documentation binding. Final documentation/manual/handoff/Linear rechecks and PR-body reconciliation remain. This is pre-merge engineering proof, not release or field acceptance. Donal retains approval/merge authority. After the exact merged head exists, run WAR-04B's narrow refresh before any release decision. BCP-17 must qualify one exact final candidate before `DON-255` may publish it.
...
| Candidate remediation in flight; final documentation/review wave pending | [PR #10](https://github.com/donal0c/sartracker-web/pull/10): PR-6 archive lifecycle | S2 Electron / Archive / Mission Review / Verification | `DON-248` / `DON-252` / `DON-253` | Reviewed executable head `6c26a1746bf5ee97648285eab680f6deff7e14ae` (tree `b6678f7b8fd6d333ada58b60b48057e94783bcd1`) is pushed. Cleanup worker failures now have sanitized substage/table-cursor/cause/exit attribution; attached cleanup diagnostics take precedence over a colliding raw `SQLITE_BUSY`; and stage markers are assigned before the select/delete/journal/completion/custody operations they describe. The qualifier keeps a pure 50 ms heartbeat separate from 5 s current-position publication. Fresh broad, concurrency/renderer, persistence/completeness and narrow crypto reviews are clean. Local serial unit `369/369` files / `3,530/3,530` tests, affected focused `176/176`, lint, TypeScript and syntax are green. Browser/visual/package and Linux results from 3d5 are prior-head evidence; one final >2 GiB qualification and exact-head package/browser/visual/Linux reruns remain after the documentation binding. Pre-merge proof is not release or field acceptance. |
