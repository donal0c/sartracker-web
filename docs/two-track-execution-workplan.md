...
| In Progress (publication only) | Make Breadcrumb Dots source-exact and independently release-gated | S2 Electron / Shared Tracking / Verification | `DON-260` | Beta.12.11 exact tag `bced8052b85c` and CI run `31482052296` are green. Dots now uses mission-start-bounded exact SQLite pages and a dedicated source; <=10k fixes render automatically, larger missions use exact Earlier/Later pages, and Line alone remains simplified. Ubuntu AppImage gates passed 279,936 fixes/28 pages and 1,935,384 fixes/194 pages (`54.632 s` outward, `67.597 s` return, `1.597 GB` RSS); the real installed `.deb` is `install ok installed`, `dpkg -V` clean, payload-exact, and passed its own 279,936-fix proof. Private GET-only AppImage/installed-`.deb` provider gates passed 10,371/10,184 fixes through SQLite, exact UI pages, GeoJSON and rendered MapLibre with zero representatives. Lifecycle, safety, diagnostics privacy, schema refusal, restart/fault/checkpoint, Line total, Playwright `179/179`, visual `43/43`, and final unit `195/1,565` are green. Next: guarded publication and fresh-public-byte lifecycle/checksum proof, then close DON-260. Beta.12.9/.12.10 are not the correction. |
