...
1. Execute the **Breadcrumb and Mission-History Programme** under the locked ADR and `docs/breadcrumb-programme-execution-policy.md`. PR-1 through PR-5 are merged. Breadcrumb programme PR-6—the archive-lifecycle stage owned by `DON-248`/`DON-252`/`DON-253` and distinct from merged WAR-01 GitHub PR #6—is in review as [PR #10](https://github.com/donal0c/sartracker-web/pull/10). The current source candidate is `0f0723d4b1ec7e78d4f6c166abad049188660ca6` / tree `b53474dc93069930a0c284ed6507510bd6a87d94`; correction custody, archive-family serialization, durable correction authorization outside the bounded audit page, browser verifier persistence/current-predecessor enforcement/supplement provenance, post-commit worker-exit classification, cleanup-retry evidence reopening, truthful operator recovery, all-mutation writer fencing, and mutable recovery-code handling across archive workers are red-first tested. Fresh broad, renderer/input, persistence and concurrency exact-head reviews are clean; crypto/lifetime checks are `39/39`, archive-review/mission-review Chromium is `17/17`, archive visual review is `4/4`, and the exact-head packaged macOS archive-lifecycle smoke passed. Full serial unit, kill matrix, Ubuntu >2 GiB, Linux workflow, and final docs/manual/handoff/Linear rechecks remain pending. This is pre-merge engineering proof, not release or field acceptance. Donal retains approval/merge authority. After the exact merged head exists, run WAR-04B's narrow refresh before any release decision. BCP-17 must qualify one exact final candidate before `DON-255` may publish it.
...
| Candidate pushed; final documentation/review wave pending | [PR #10](https://github.com/donal0c/sartracker-web/pull/10): PR-6 archive lifecycle | S2 Electron / Archive / Mission Review / Verification | `DON-248` / `DON-252` / `DON-253` | Candidate `561ffcb96960ba3bd62dcede1c616b74a79b22a7` (tree `afbd3b3940d9908edea9d661c24078e471bf63a1`) is pushed after red-first fixes for archive-backed correction snapshots without registry tables, completion-to-exit worker handling, attachment-residue-gated recovery fences, correction close auditing, truthful custody recovery state, cleanup-retry evidence reopening, post-commit worker-exit classification, all-mutation writer fencing, bounded-page-independent correction authorization, browser verifier persistence, current-predecessor enforcement, supplement provenance, main-loop-safe cleanup workers, qualification-safe run identities, and cancelled-cleanup physical-exit fencing. Full serial unit `369/369` files / `3,527/3,527` tests and static checks are green. Package/kill/browser/visual runs from b30 are prior-head evidence and are not promoted automatically; Ubuntu >2 GiB and exact-head Linux are running against 561, with final-head reruns and exact-head reviews pending. Pre-merge proof is not release or field acceptance. |
