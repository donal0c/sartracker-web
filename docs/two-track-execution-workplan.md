...
1. Execute the **Breadcrumb and Mission-History Programme** under the locked ADR and `docs/breadcrumb-programme-execution-policy.md`. PR-1 through PR-4 are merged. PR-5 mission evidence/replay (`DON-274`, `DON-277`, `DON-278`, `DON-279`) remains one review unit on `codex/breadcrumb-pr5-evidence-replay` from exact merged PR-4 base `80309c995a18eeb190cce4310c9a46b0f46d5263`. The first review wave on pushed head `75a4ee05` invalidated that candidate: persistence/completeness found an unbounded legacy event identity and mission-global quarantine custody; broad life-safety found incomplete embedded event evidence could survive recoverable Finalize archive reuse. Executable fix `f5f0438e` uses bounded row-ID migration state, retains oversized source rows behind affected-mission-only fail-closed custody, and validates embedded snapshot evidence before recovery reuse. Full serial unit 2,495/2,495, Chromium 167/167, backend, lint/build, fresh indexed 960k and unsigned macOS arm64 package/soak are green. Exact branch-head Linux run `33224611572` is also green on `fb5f9636` through tests, AppImage/`.deb`, 960k, packaged soak and real launch/close. Restarted broad/persistence/concurrency/renderer reviews on one final code-and-documentation head remain the task-owned completion gate. PR-6 archive lifecycle follows only after Donal accepts PR-5.
