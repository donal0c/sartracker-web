...
| In progress | PR-1: trustworthy ingest and live safety; complete candidate integration | Shared Tracking / S2 Electron / Evidence | `DON-266` / `DON-267` / `DON-268` / `DON-269` | Branch `codex/breadcrumb-pr1-ingest-safety`; BCP-10 exact Dots and BCP-01 current-position safety remain green. BCP-02 adds schema v8 provenance, immutable first-accepted-wins conflict evidence, durable rejection outbox, persistent degraded health, and completion guards. BCP-05 adds accuracy-aware stationary attention with additive map/UI presentation and acknowledgement. The 3.7 GB gate exposed pre-existing synchronous Mission Review audit/count scans, materially equivalent at exact base and candidate; the narrow `DON-251` worker isolation is pulled into PR-1 without adding an index or waiving 200 ms. Final exact-head gates, worker-isolated packaged proof, soak, push, and PR closeout remain; no team release comes from this PR. |
