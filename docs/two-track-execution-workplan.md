...
| Review-ready | PR-1: trustworthy ingest and live safety | Shared Tracking / S2 Electron / Evidence | `DON-266` / `DON-267` / `DON-268` / `DON-269` | [PR #1](https://github.com/donal0c/sartracker-web/pull/1) remains unmerged. A second red-first review pass now fences finalization behind renderer evidence flush and the main outbox queue, rejects new finalized-mission projection without losing the staged record, scopes multi-reason health/corruption to one mission, applies 4,096-file/64 MiB backpressure, retries only bounded eight-record batches, drains renderer disposal, preserves explicit fix-time warnings, enforces source-device ownership, settles synchronous Review worker-construction failure, and caches unchanged per-device stationary projection. Corrupt unique evidence is quarantined, never auto-deleted. Proof: unit `1,706`, backend, full Chromium `146` plus final stationary Chromium, touched visual/fresh review, BCP-10, lint/build/Node/diff checks, and packaging green. The earlier exact `f24906c425e0` 3.704 GB migration/Review and 8,664-position soak remain valid because their documented invalidation surfaces did not change. The cold audit worker remains about `7.5 s`; indexing/retention stays open in `DON-251`, and the 200 ms gate is not waived. |
