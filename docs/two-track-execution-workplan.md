...
1. Execute the **Breadcrumb and Mission-History Programme** under the locked ADR and `docs/breadcrumb-programme-execution-policy.md`. PR-1 through PR-4 are merged. PR-5 mission evidence/replay (`DON-274`, `DON-277`, `DON-278`, `DON-279`) is implemented as one review unit on `codex/breadcrumb-pr5-evidence-replay` from exact merged PR-4 base `80309c995a18eeb190cce4310c9a46b0f46d5263`. Pushed candidates `1e9b6acc` and `011a8051` are rejected: the latter fixed indexed startup but Linux run `33185543280` advanced only 450,003 of 500,000 settled rows inside the unchanged liveness deadline. Executable fix `123116e4` / binding `2a2c5ba4` narrows only the inter-turn yield and is locally green through deterministic, browser, visual, indexed-960k, authentic-v11 and macOS packaged-soak proof. Push/Linux proof and a restarted broad/persistence/concurrency/renderer exact-head wave remain task-owned completion gates. PR-6 archive lifecycle follows only after Donal accepts PR-5.
