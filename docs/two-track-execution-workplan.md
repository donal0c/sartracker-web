...
| Release candidate | Fix field-scale 36-hour breadcrumb catch-up, restart reliability, and route fidelity | S2 Electron / Shared Tracking / Verification | `DON-260` | Reopened from field evidence: 36-hour history took about nine minutes and arrived in blocks. Continuous bounded catch-up, atomic wave persistence, durable frontiers, geometry-aware retention, cancellation, and memory bounds are implemented. Ubuntu rejected beta.12.6 on 60-second persistence throughput and beta.12.8 on completed-history restart margin (`10.177 s` fail, `9.892 s` pass against a `10 s` gate). `b3a3826` forces selected-row hydration through the SQLite rowid primary key, reducing the exact Ubuntu 279,936-row canonical query from `4.642 s` to `1.315 s` with identical 103,633 retained positions. Beta.12.9 must pass fresh no-skip/tag CI plus three exact completed-history restarts, the 36-hour/14-day proofs, and the real Ubuntu AppImage/installed `.deb` matrix before publication. |
