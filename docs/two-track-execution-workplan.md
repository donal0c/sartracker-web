...
and hazard register. WAR-01 may proceed while PR5 is in flight, but every
PR5-owned replay/evidence boundary remains `rewrite-pending`; immediately after
PR5 merges, re-open those merged production and test paths and reconcile the
affected hazard rows on the merge SHA. PR6 remains the prerequisite for the
streamed archive/restore resilience slice. WAR-01 does not create speculative
Linear issues: each row cites the closest existing owner.
...
| Active docs | WAR-01 whole-application assurance charter and stable hazard register | Whole application / Assurance / Verification | Existing owners are cited per hazard; no new issue | Pinned-base documentation/investigation only. Covers all 12 hazard domains and the absolute blockers. PR5-owned rows stay `rewrite-pending` until the mandatory short post-merge reconciliation; PR6 remains unimplemented and required before archive/restore resilience work. |
