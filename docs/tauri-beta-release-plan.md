...
- Linux/Windows artifacts must pass native launch smoke in CI before a draft
  beta can be promoted. A real-machine smoke still remains required for
  operator workflows and live network dependencies such as Traccar and map
  tiles.
...
Status: implemented 2026-05-17 (`sartracker-web-y6a`); launch-smoke hardening
added 2026-05-18 after first Linux tester feedback.

Delivered:

- `.github/workflows/release.yml` — tag-driven release workflow (`v*` tags +
  `workflow_dispatch` escape hatch, including `smoke_existing_release` for
  already-attached release assets). Splits into a `gates` job (Linux,
  lint/test/build/version-trio assertion/release-notes existence check), a
  `bundle` matrix (`ubuntu-22.04` + `windows-2022`) using
  `tauri-apps/tauri-action@v0.6.2`, native Linux/Windows launch-smoke jobs,
  a `checksums` job that generates a `SHA256SUMS` sidecar from the uploaded
  assets, and a `summary` job.
- Linux artifacts: AppImage + `.deb`, x86_64.
- Windows artifact: NSIS `.exe` (current-user, no admin), x86_64. MSI is
  deferred because Tauri's MSI bundler rejects the `-beta.N` prerelease suffix.
...
- `docs/releases/TEMPLATE.md` rewritten with per-OS install sections covering
  AppImage, .deb, NSIS-with-SmartScreen, deferred MSI, and macOS Gatekeeper
  paths.
...
- **`fail-fast: false`** on the bundle matrix so Linux artifacts survive a
  ...
  no end-user sees it.
- **Native launch smoke before checksums/promotion.** Linux AppImage and
  Windows NSIS artifacts are downloaded from the draft release, launched on
  native GitHub runners, and screenshotted before `SHA256SUMS` is generated.
  This catches "doesn't even boot" packaging failures before a release is
  shareable. It is not a replacement for real-machine operator workflow smoke.
- **Live beta dependency preflight.** The gates job authenticates against the
  documented Traccar web/API endpoint (`http://kmrtsar.eu:8082`) and checks
  representative OpenTopoMap/OpenStreetMap/ESRI tile URLs before any bundle
  starts. This catches stale endpoint docs and upstream outages early.
- **`SHA256SUMS` sidecar** generated by a follow-up job after all bundle
  uploads and launch-smoke jobs pass. Testers verify with `sha256sum -c`
  (Linux) / `Get-FileHash` (Windows) / `shasum -a 256 -c` (macOS).
...
# When green, review CI launch-smoke screenshots, smoke-test the primary
# platform on a real machine, then:
gh release edit v<v> --repo donal0c/sartracker-web --draft=false
```

Smoke an existing release without rebuilding or re-uploading duplicate assets:

```bash
gh workflow run release.yml --repo donal0c/sartracker-web \
  -f tag=v0.1.0-beta.3 \
  -f smoke_existing_release=true
```
...
- The first published Linux AppImage reached testers before native launch and
  map/tracking smoke had run on Linux hardware.
- Tester feedback showed two failures: Traccar was configured with
  `http://kmrtsar.eu:5055`, which is a device/listener-style port and returns
  `400 Bad Request` for Traccar API session requests; OpenTopoMap also appeared
  degraded on the tester machines and needs packaged Linux evidence rather than
  assuming the macOS/local browser checks are enough.
...
- `workflow_dispatch` can run the same launch-smoke path against an existing
  release by setting `smoke_existing_release=true`, avoiding duplicate bundle
...
- Open the Linux artifact on a real operator-like Linux machine.
- Confirm OpenTopoMap tiles actually render on a normal network.
- Configure Traccar with the web/API URL (`http://kmrtsar.eu:8082` or the
  documented fallback `http://kmrtsar.ddns.net:8082`), not `:5055`.
- Confirm mission start, persistence after restart, tracking connection,
  diagnostics export, and release-note checksum instructions.
