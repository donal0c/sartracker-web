...
- Implementation readiness: implementation, G2/G3, migration, both exact-head review remediations, replacement Candidate-B rows, decoded all-period production qualification, the single replacement packaged soak, and the final software/browser/visual/package repeat are complete; five independent exact-head reviews remain.
...
- Evidence: G2's prescribed 18 serial packaged runs completed on the Ubuntu reference host at measured SHA `8eff87b724ae6`; B alone passed 960k and 2M. The mission/source/worker lifetime remediation reran B's six rows plus two kill probes at `53e38bf`; both scales passed unchanged budgets and remain standing because the latest fixes do not change the measured pipeline. The corrected production driver delivered 959,988/959,988 and 1,999,988/1,999,988 while decoding non-empty geometry for all 13 periods before/after activation at 2.245/4.534 s first useful, 6.212/12.338 s complete, and 21.026/21.825 ms main gaps. Exact-head review then found one mission-sized accepted-write outing scan and rejected the first incomplete indexed fix. Final behavior head `40a713c` fetches only the indexed latest-start predecessor and applies its end boundary in memory; it passed 262 files / 2,096 tests, lint, TypeScript, CommonJS syntax, exact Dots 10/10, build/budgets, unsigned macOS arm64 plus Ubuntu x64 packaging, and a final replacement packaged soak with 8,664/8,664 exact positions, restart/integrity clean, zero redundant slope, 20.482 ms main / 83.6 ms renderer maxima, 1.092 GB peak RSS, and a 32-chunk/zero-invalidation ledger. Parent `add5639` passed Ubuntu Chromium 158/158; the unchanged UI retains coverage visual 7/7 and critical review 9/9 from `4b740f2`. The 16.557 ms integrity-`ok` 3.704 GB v9→v10 migration retaining 2.04M positions remains standing because schema/open code did not change. Full bindings and proof limits are in `docs/breadcrumb-pr3-t1-evidence.md`.
