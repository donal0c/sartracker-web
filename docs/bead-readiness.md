...
Last updated: 2026-08-26 by Codex (Breadcrumb PR-3 bounded trust and I5 gate)
...
- Implementation readiness: implementation, G2/G3, migration, Candidate-B evidence and the earlier qualification remain complete. Fresh review invalidated evidence head `d044e71` because request-derived result cardinality and manifest outing metadata were not independently attested. Commit `68768f7` closes that trust boundary red-first. Its workflow-green Linux run was correctly rejected at 224.974 ms against the binding 200 ms Electron-main I5 limit; `e1b51d8` removes the per-fix SQLite lookup and `cfd733c` gives I5 its own gate. Exact application-head Linux run `32934141302` passes at 22.2073 ms. Deterministic/Linux gates and five fresh independent reviews on this evidence record's final head remain the live review-readiness gate. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) is open to `master`.
...
- Evidence: G2's prescribed 18 serial packaged runs completed on the Ubuntu reference host at measured SHA `8eff87b724ae6`; B alone passed 960k and 2M. The corrected production driver delivered 959,988/959,988 and 1,999,988/1,999,988 while decoding non-empty geometry for all 13 periods. The prior replacement soak and 3.704 GB migration remain standing for unchanged geometry/schema surfaces. The accumulated external/renewed findings include false freshness/classification, worker/query and tile trust failures, catalog/renderer recovery defects, false progress/completeness paths, invalidation under-dirtying, incomplete results, and unbounded/unattested result metadata. `68768f7` makes result limits request-derived, preflights them in the worker, and attests inventory/outing/claim metadata against SQLite. After run `32932901604` exposed a 224.974 ms I5 breach, `e1b51d8` replaced per-fix SQLite outing reads with one bounded snapshot and binary search; `cfd733c` enforces the 200 ms main limit separately. Full unit is 266 / 2,149; lint, TypeScript, build/budgets, and focused hot-path/gate 56/56 pass. Exact Linux run [`32934141302`](https://github.com/donal0c/sartracker-web/actions/runs/32934141302) passed package build, AppImage launch, and the 8,664/8,664 soak with source digest `413b61e1…`, integrity `ok`, one restart, 22.2073 ms main maximum, zero renderer crashes, and four healthy operator interactions. Full bindings and proof limits are in `docs/breadcrumb-pr3-t1-evidence.md`.
