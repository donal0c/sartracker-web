...
- Implementation readiness: the first seven P1/six P2, second-pass eight P1/five P2, third-pass five P1/two P2, and fourth-pass one P1/one P2 findings are remediated; latest code commit `6b2fe59e7025` is review ready after this evidence-doc commit is pushed.
...
- Evidence: the fourth pass adds red-first coverage for the exact pending membership observation being retried under a finish fence, the fence remaining closed through the persisted transition, core/mission lifecycle wiring, and restart from a later participant's scoped durable cursor. Full unit passes serially at `229 files / 1,862`; lint/build, backend `51 / 1 ignored`, exact Dots `10/10`, and Chromium `154/154` pass. Standing participant visual Playwright `4/4` and fresh critical finish-refusal review `1/1` remain valid because the dialog rendering is unchanged. The preceding Ubuntu flag-off/on restart soaks each persisted `8,664/8,664`, integrity `ok`, zero redundant slope, with main/renderer maxima `7.2/50.2 ms` and `7.7/100.3 ms`; the immutable 3.704 GB field-v2 cache reused without changing its checksum or mtime. Electron main/schema/generator, fixture shape, outing worker, current-position hot path, 500k renderer shape, and live GET-only network shape were untouched, so those standing heavy proofs remain valid only for their unaffected surfaces under surface-based invalidation. Proof limits: the default highly parallel unit command made the unrelated 500k accumulator timing assertion miss its `<100 ms` local gate at `100.139 ms` and `108.031 ms`; that exact gate passes alone and the complete suite passes with one worker. Ubuntu Xwayland/llvmpipe 500k remains unqualified (`3.685 s` final; `2.941 s` pre-remediation), while the feature remains release-off. No result treats a render budget as a persistence/evidence budget.
