...
- Implementation readiness: implementation, G2/G3, migration, and Candidate-B scale evidence remain complete. Review of `848bfb2` found that fatal main-process exit bypassed the renderer-evidence coordinator. The red-first remediation fences evidence before fatal relaunch and before the first operator window after any unclean session. Exact macOS/Linux package gates and all five fresh PR-3 reviews still gate review readiness. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) is open to `master`.
- Recommended action: commit and push the fatal/unclean-session remediation, run replacement exact-head macOS/Linux package and soak proof, bind it in a documentation-only descendant, then complete five fresh independent reviews from zero. Do not merge or release from this implementation task.
