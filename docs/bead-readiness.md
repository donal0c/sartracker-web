...
- Evidence: G2's prescribed 18 serial packaged runs completed on the Ubuntu reference host at measured SHA `8eff87b724ae6`; B alone passed 960k and 2M. The mission/source/worker lifetime remediation reran B's six rows plus two kill probes at `53e38bf`; both scales passed unchanged budgets and remain standing because the latest fixes do not change the measured pipeline. The corrected production driver delivered 959,988/959,988 and 1,999,988/1,999,988 while decoding non-empty geometry for all 13 periods before/after activation at 2.245/4.534 s first useful, 6.212/12.338 s complete, and 21.026/21.825 ms main gaps. The single replacement packaged CI soak passed 8,664/8,664 fixes, restart, zero redundant slope, 13.889 ms main / 167.2 ms renderer maxima, 1.123 GB peak RSS, and a 32-chunk/zero-invalidation ledger. The 16.557 ms integrity-`ok` 3.704 GB v9→v10 migration retaining 2.04M positions remains standing because schema/open code did not change. Final production/test head `4b740f2` passed 262 files / 2,095 tests, lint, TypeScript, CommonJS syntax, exact Dots 10/10, build/budgets, Ubuntu Chromium 158/158, coverage visual 7/7, and critical review 9/9. Operator-visible/package head `fea89db` passed its manual contract and unsigned macOS arm64 plus Ubuntu x64 packaging. Full bindings and proof limits are in `docs/breadcrumb-pr3-t1-evidence.md`.
