...
Last updated: 2026-08-26 by Codex (Breadcrumb PR-3 complete-query trust gate)
...
- Implementation readiness: implementation, G2/G3, migration, Candidate-B evidence and the earlier qualification remain complete. Fresh review invalidated evidence head `862a300` because structurally plausible manifest omission could hide a real stale chunk and produce an empty ready claim. Application head `20ca76e` closes that P1 red-first with kind-specific result envelopes, exact bounded main-side inventory/claim attestation, and a non-vacuous final controller revision check. Its exact Linux package/soak run is green; deterministic gates and five fresh independent reviews on this evidence record's final head remain the live review-readiness gate. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) is open to `master`.
...
- Evidence: G2's prescribed 18 serial packaged runs completed on the Ubuntu reference host at measured SHA `8eff87b724ae6`; B alone passed 960k and 2M. The corrected production driver delivered 959,988/959,988 and 1,999,988/1,999,988 while decoding non-empty geometry for all 13 periods. The prior replacement soak and 3.704 GB migration remain standing for unchanged geometry/schema surfaces. External and renewed review reproduced false freshness/classification, worker/query trust-boundary failures, tile-cache escape, catalog/renderer recovery defects, retained-source failure misclassification, early recovery-epoch clearing, a progressive old-mission handoff deadlock, premature predecessor retirement, unbounded claim/catalog arrays, lost worker-generation release, malformed staged-result poisoning, a false full progress bar after rejected final completeness claim, worker-authorized invalidation under-dirtying, and finally incomplete non-invalidation query results. Application head `20ca76e` closes the latest P1 red-first: every query kind is request-bound; canonical inventory omission rejects before apply/return; the final claim is checked against a bounded direct ledger snapshot; and Complete requires exact unique selected revision coverage. Full unit is 264 / 2,143; lint, TypeScript, build/budgets, CommonJS syntax, and focused coverage/participant 251/251 pass. Exact Linux run [`32930525474`](https://github.com/donal0c/sartracker-web/actions/runs/32930525474) passed package build, AppImage launch, and the 8,664/8,664 soak with digest `c074b33b…`, integrity `ok`, one restart, 46.882 ms main maximum, zero redundant slope, zero renderer crashes, and four healthy operator interactions. Full bindings and proof limits are in `docs/breadcrumb-pr3-t1-evidence.md`.
