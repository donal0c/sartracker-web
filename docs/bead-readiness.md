...
- Ready-to-start checklist: safety invariants, failure modes, persistence/coordinate impact, TDD regressions, browser/visual proof, packaged multi-day proof, and Fable review are complete. Three tag runs stopped safely before promotion: the first exposed Linux WebGL blocklisting; the second rendered and completed 4/4 healthy interactions but exposed headless background/occlusion timing throttle; the third preserved exact truth and healthy interactions but identified a failed SwiftShader/Vulkan fallback on the Azure runner. All three harness boundaries are now covered under TDD. The exact ANGLE/OpenGL wrapper passes Ubuntu/Xvfb at 8,664 exact positions, 4/4 healthy interactions, 166.6 ms maximum renderer gap, 16.7 ms median renderer cadence, and 689.1 ms maximum action without weakening the 1,000 ms gate. Full committed-state beta verification passed 8/8 with no skips. Replacement CI artifact provenance and the full Ubuntu artifact matrix remain the release gates specified in `DON-260` and `docs/two-track-execution-workplan.md`.
