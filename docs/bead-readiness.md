...
Last updated: 2026-08-26 by Codex (Breadcrumb PR-3 lost-evidence remediation)
...
- Implementation readiness: implementation, G2/G3, migration, and Candidate-B scale evidence remain complete. The review of `fa863da` found two lost-evidence P1s: RAM-only success after a health-marker write failure, and Finish leaving renderer evidence outside the active-mission fallback. Both are remediated red-first by strict marker durability and a Finish drain/acceptance seal. Exact macOS/Linux package gates and all five fresh PR-3 reviews still gate review readiness. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) is open to `master`.
...
- Evidence: G2's prescribed 18 serial packaged runs completed on the Ubuntu reference host at measured SHA `8eff87b724ae6`; B alone passed 960k and 2M. Production qualification delivered 959,988/959,988 and 1,999,988/1,999,988 with all 13 periods non-empty. The 3.704 GB migration remains standing because schema/geometry construction did not change. The new red set reproduced swallowed marker failure, unfenced Finish, and the missing Finish fence; focused green is 3 files / 51 tests. Full unit is 272 / 2,194; lint, TypeScript, build/budgets, backend 51/51 plus one intentional ignore, Chromium 13/13, and selected critical visual plus fresh uncached review 2/2 pass. The manual now states that an unflushable Finish is refused. Earlier exact Linux run [`32966604838`](https://github.com/donal0c/sartracker-web/actions/runs/32966604838) remains standing only for the unchanged package harness and active-mission close path; the new application head still needs its own macOS/Linux package evidence. Full bindings and proof limits are in `docs/breadcrumb-pr3-t1-evidence.md`.
- Ready-to-start checklist: [x] accepted plan and requirement trace; [x] exact base; [x] G2 ratification and affected-row rerun; [x] BCP-08; [x] flag-on BCP-09 implementation/manual; [x] T1 migration and replacement soak; [x] G3; [x] separate default flip; [x] external F1/F2/F3 remediation; [x] progressive recovery and mission-switch remediation; [x] lost-evidence P1 remediation; [ ] new application-head macOS/Linux package gates; [ ] five fresh exact-head reviews; [x] one PR to `master` opened without merge/release.
