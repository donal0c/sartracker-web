...
- Implementation readiness: the first seven P1/six P2 review findings and the second-pass eight P1/five P2 findings are remediated; latest code commit `8f637a192164` is review ready after this evidence-doc commit is pushed.
...
- Evidence: strict red-first regressions now also cover removed-marker retention across an omitted poll, provisional selected-group visibility, serialized/coalesced roster races and retry, incomplete-roster group refusal, mission-keyed reconciliation, finish/finalize fences, monotonic checkpoints, group progress, worker-constructor failure, and fixture reachability. Software gates are `228 files / 1,846` unit, lint/build, backend `51 / 1 ignored`, exact Dots `10/10`, Chromium `153/153`, and changed visual Playwright/review `3/3`. Generator-v4 schema-v9 fixtures are 960k (`476,151,808` bytes, SHA `6b92005b…ccdc0`) and 2M (`991,227,904` bytes, SHA `f70f1ac4…133ca`), byte-identical on macOS/Ubuntu with 100 active selected devices and participation-at-fix-time validation. The exact Ubuntu worker returned 1,999,988 total / 1,242,988 Unassigned across 12 outings in `1.072 s`, parent maximum `14.3 ms`, one covering mission-position stream, and no global timestamp index. The unchanged 3.704 GB v8→v9 proof migrated in `24.7 ms`, retained 2,040,000 positions and 32 grandfathered participants, had zero outings, and integrity `ok`. Exact Ubuntu flag-on/off packages passed 32-device / 8,664-position restart soaks with integrity `ok`, zero redundant slope, and main/renderer maxima `6.1/83.6 ms` and `10.6/66.9 ms`; the final macOS 100 × 5,000 flag-on profile passed at `84.3 ms`; GET-only live confirmation remained 1 group / 34 devices / 9 current fixes with only the selected device persisted. Proof limit: the same 500k flag-on profile misses on Ubuntu Xwayland/llvmpipe (`3.685 s` final; `2.941 s` at pre-remediation `896700e`), so it is not a remediation regression but Linux/field qualification is unproven while the feature remains release-off. No result treats a render budget as a persistence/evidence budget.
- Ready-to-start checklist for reviews: [x] red-first remediation; [x] full software/browser gates; [x] exact code package/fixture gates; [x] exact evidence-doc head prepared for push and pull-request reporting.
