...
- Implementation readiness: implementation, G2/G3, migration, and Candidate-B scale evidence remain complete. Donal paused the repeated PR-3 review loop on 2026-08-26. Fresh Fable whole-seam pass `a5493464-eb53-4231-a686-1aed764e3cb7` at `397adc8` plus central source retrace produced one coherent red-first correction for power-loss durability, renderer-loss lifecycle ordering, and permanent audited closure of an unrecoverable gap without weakening Complete/100%.
- Recommended action: bind the completed replacement macOS/Linux package and soak proof, then run one fresh broad exact-head review plus targeted persistence/completeness, concurrency/finalization, and renderer/input-containment rechecks. Do not restart five full review waves after each fix. Do not merge or release from this implementation task.
...
- Evidence: G2's prescribed 18 serial packaged runs completed on the Ubuntu reference host at measured SHA `8eff87b724ae6`; B alone passed 960k and 2M. Production qualification delivered 959,988/959,988 and 1,999,988/1,999,988 with all 13 periods non-empty. The 3.704 GB migration remains standing because schema/geometry construction did not change. The whole-seam correction has final local code-tree proof: 273 files / 2,227 tests, lint, TypeScript/build/budgets, exact Dots 10/10, Chromium 14/14, critical visual 1/1 plus fresh uncached PASS, backend 51 passed / 1 intentional ignore, and changed CJS syntax. Exact `a4de1f0` unsigned macOS and Ubuntu packaging passed; the Ubuntu packaged CI-scale soak passed 6/6 batches, 8,664/8,664 exact positions, one restart, zero renderer crashes/redundant slope, integrity/WAL, four healthy interactions, 8.199 ms main maximum and 1,135,435,776-byte peak RSS. Full bindings and proof limits are in `docs/breadcrumb-pr3-t1-evidence.md`.
- Ready-to-start checklist: [x] accepted plan and requirement trace; [x] exact base; [x] G2 ratification and affected-row rerun; [x] BCP-08; [x] flag-on BCP-09 implementation/manual; [x] T1 migration and replacement soak; [x] G3; [x] separate default flip; [x] external F1/F2/F3 remediation; [x] progressive recovery and mission-switch remediation; [x] strict marker/Finish remediation; [x] capacity and in-flight observation remediation; [x] finished-mission main fallback remediation; [x] pre-cutoff accepted-fix Finish remediation; [x] fatal/unclean-session evidence remediation; [x] fresh whole-seam Fable architecture/attack pass; [x] coherent seam REDs and fix; [x] replacement application-head macOS/Linux package gates; [ ] one fresh broad plus three targeted exact-head rechecks; [x] one PR to `master` opened without merge/release.
