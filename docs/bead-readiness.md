...
Last updated: 2026-08-26 by Codex (Breadcrumb PR-3 final attestation remediation)
...
- Implementation readiness: implementation, G2/G3, migration, and Candidate-B scale evidence remain complete. The first final-head review wave was invalidated by five confirmed boundary defects: an unbound worker period digest, a repeated-effective-from participant checkpoint collision, finalized-mission health contaminating the next mission, normal claim/write drift being treated as corruption, and default-on coverage starting against the unsupported historical Tauri store. Application head `f842190` closes all five red-first. Exact Linux run `32940999122` passes at 23.3174 ms on Electron main. This evidence record's documentation-only descendant still requires deterministic exact-head/Linux gates and five fresh independent reviews from zero. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) is open to `master`.
- Recommended action: commit this evidence binding, repeat deterministic gates on that exact head, and complete five fresh independent reviews. Do not merge or release from this implementation task.
...
- Evidence: G2's prescribed 18 serial packaged runs completed on the Ubuntu reference host at measured SHA `8eff87b724ae6`; B alone passed 960k and 2M. The production qualification delivered 959,988/959,988 and 1,999,988/1,999,988 while decoding non-empty geometry for all 13 periods. The 3.704 GB migration remains standing because schema/geometry construction did not change. `f842190` derives period digests from request-owned contributors, preserves immutable checkpoint edges while adding only uncovered adjacent intervals, retires finalized health from the live aggregate with unlock restoration, returns the newer direct claim on sequence drift while rejecting same-sequence divergence, and gates coverage to Electron. Full unit is 267 / 2,147; lint, TypeScript, build/budgets, participant/coverage Chromium 10/10, selected critical visual 2/2 plus uncached review 2/2, and backend 51/51 with one intentional OS-keychain ignore pass. Exact Linux run [`32940999122`](https://github.com/donal0c/sartracker-web/actions/runs/32940999122) passed package build, AppImage launch, and the 8,664/8,664 soak with source digest `05d0957d…`, integrity `ok`, one restart, 23.3174 ms main maximum, zero renderer crashes, and four healthy operator interactions. Full bindings and proof limits are in `docs/breadcrumb-pr3-t1-evidence.md`.
- Ready-to-start checklist: [x] accepted plan and requirement trace; [x] exact base; [x] G2 ratification and affected-row rerun; [x] BCP-08; [x] flag-on BCP-09 implementation/manual; [x] T1 migration and replacement soak; [x] G3; [x] separate default flip; [x] external F1/F2/F3 remediation; [x] progressive recovery and mission-switch remediation; [x] application-head Linux CI; [ ] final evidence-head deterministic gates; [ ] five fresh exact-head reviews; [x] one PR to `master` opened without merge/release.
