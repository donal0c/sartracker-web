...
- Implementation readiness: implementation, G2/G3, migration, rebound Candidate-B rows, exact-production qualification, the rebound packaged soak, and the documentation-head gate are complete; five reviews remain after one exact-head repeat on the closeout commit.
...
- Evidence: G2's prescribed 18 serial packaged runs completed on the Ubuntu reference host at measured SHA `8eff87b724ae6`; B alone passed 960k and 2M. Review-driven source-strategy alignment invalidated and reran only B's six rows plus two kill probes at `5653133`; both scales passed unchanged budgets. The same code delivered 959,988/959,988 and 1,999,988/1,999,988 through the production staged-read/activation/active-read path within the five-second first-useful gate, then passed the packaged CI soak with 8,664/8,664 fixes, restart, zero redundant slope, 7.924 ms main / 100.4 ms renderer maxima, 1.205 GB peak RSS, and a 32-chunk/zero-invalidation ledger. The 16.557 ms integrity-`ok` 3.704 GB v9→v10 migration retaining 2.04M positions remains standing because schema/open code did not change. Full bindings and proof limits are in `docs/breadcrumb-pr3-t1-evidence.md`.
- Ready-to-start checklist: [x] accepted plan and requirement trace; [x] exact base; [x] G2 ratification and affected-row rerun; [x] BCP-08; [x] flag-on BCP-09 implementation/manual; [x] T1 migration and rebound soak; [x] G3; [x] separate default flip; [x] documentation-head gate; [ ] closeout-head repeat; [ ] five exact-head reviews.
