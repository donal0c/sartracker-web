...
### `DON-273` / `DON-276` / `DON-275` — Breadcrumb PR-3 complete coverage — P1 remediation

- Research required: `Low`; the renderer decision and domain requirements are locked.
- Implementation readiness: implementation, G2/G3, migration, and Candidate-B scale evidence remain complete. Review of `1f34299` found that Finish could commit `finished` before an already-started accepted current fix persisted. The red-first remediation closes mission-evidence admission, waits and drains every pre-cutoff observation while the mission remains writable, then commits Finish. Exact macOS/Linux package gates and all five fresh PR-3 reviews still gate review readiness. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) is open to `master`.
- Recommended action: bind the exact `9253d89` macOS/Linux package evidence in a documentation-only descendant, attest unchanged executable/test trees, then complete five fresh independent reviews from zero. Do not merge or release from this implementation task.
...
- Evidence: G2's prescribed 18 serial packaged runs completed on the Ubuntu reference host at measured SHA `8eff87b724ae6`; B alone passed 960k and 2M. Production qualification delivered 959,988/959,988 and 1,999,988/1,999,988 with all 13 periods non-empty. The 3.704 GB migration remains standing because schema/geometry construction did not change. The latest REDs proved the early durable Finish call, post-cutoff mission persistence and obsolete manual wording. A real SQLite regression proves a pre-cutoff accepted fix and its one-fix exact coverage claim survive before Finish resolves. Focused green is 6 files / 170 tests; full unit is 272 / 2,204; lint, TypeScript, build/budgets, backend 51/51 plus one intentional ignore, Chromium 13/13, and selected critical visual plus fresh uncached review 2/2 pass. Exact code head `9253d89` passed unsigned macOS arm64 packaging and Ubuntu run [`32981424686`](https://github.com/donal0c/sartracker-web/actions/runs/32981424686): Linux x64 package/native inspection, llvmpipe attestation, 8,664/8,664 exact rows, integrity `ok`, one restart, zero renderer crashes, two graceful exits, packaged soak and real AppImage launch/close. Full bindings and proof limits are in `docs/breadcrumb-pr3-t1-evidence.md`.
- Ready-to-start checklist: [x] accepted plan and requirement trace; [x] exact base; [x] G2 ratification and affected-row rerun; [x] BCP-08; [x] flag-on BCP-09 implementation/manual; [x] T1 migration and replacement soak; [x] G3; [x] separate default flip; [x] external F1/F2/F3 remediation; [x] progressive recovery and mission-switch remediation; [x] strict marker/Finish remediation; [x] capacity and in-flight observation remediation; [x] finished-mission main fallback remediation; [x] pre-cutoff accepted-fix Finish remediation; [x] replacement application-head macOS/Linux package gates; [ ] five fresh exact-head reviews; [x] one PR to `master` opened without merge/release.
