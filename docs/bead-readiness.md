...
Last updated: 2026-08-26 by Codex (Breadcrumb PR-3 false-100 gate)
...
- Implementation readiness: implementation, G2/G3, migration, Candidate-B evidence and the earlier qualification remain complete. Review invalidated the previous evidence descendant because a rejected final database claim left equal delivered/total counts and the panel rendered a false full progress bar. Application head `6946446` closes that path red-first: unverified equal counts have explicit operator wording and no progress element, while partial progress remains visible. Its exact Linux package/soak run is green; deterministic gates and five fresh independent reviews on this evidence record's final head remain the live review-readiness gate. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) is open to `master`.
...
- Evidence: G2's prescribed 18 serial packaged runs completed on the Ubuntu reference host at measured SHA `8eff87b724ae6`; B alone passed 960k and 2M. The corrected production driver delivered 959,988/959,988 and 1,999,988/1,999,988 while decoding non-empty geometry for all 13 periods. The prior replacement soak and 3.704 GB migration remain standing for unchanged geometry/schema surfaces. External and renewed review reproduced false freshness/classification, worker/query trust-boundary failures, tile-cache escape, catalog/renderer recovery defects, retained-source failure misclassification, early recovery-epoch clearing, a progressive old-mission handoff deadlock, premature predecessor retirement, unbounded claim/catalog arrays, lost worker-generation release, malformed staged-result poisoning, and a false full progress bar after rejected final completeness claim. Application head `6946446` closes the last path red-first and updates the operator manual, screenshot, controller/panel coverage, and critical visual proof. Full unit is 264 / 2,137; lint, TypeScript, build/budgets, exact Dots 10/10, selected visual Playwright plus uncached independent visual review, CommonJS syntax, and focused Chromium 10/10 pass. Exact Linux run [`32926442726`](https://github.com/donal0c/sartracker-web/actions/runs/32926442726) passed package build, AppImage launch, and the 8,664/8,664 soak with digest `ab671355…`, integrity `ok`, one restart, 32.024 ms main maximum, zero redundant slope, zero renderer crashes, and four healthy operator interactions. Full bindings and proof limits are in `docs/breadcrumb-pr3-t1-evidence.md`.
