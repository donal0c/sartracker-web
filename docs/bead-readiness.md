...
- Evidence: the third pass adds red-first coverage for mission-switch loading scope, generation-fenced mutation completion, immutable failed-membership retry observations, incomplete-roster provisional visibility, history-write participation scope/cursors, active write-error presentation, finish/backfill/archive equality, and real legacy-v2 cache reuse. Software gates are `229 files / 1,857` unit, lint/build, backend `51 / 1 ignored`, exact Dots `10/10`, Chromium `154/154`, participant visual Playwright `4/4`, and fresh critical finish-refusal review `1/1`. Exact-code Ubuntu flag-off/on restart soaks each persisted `8,664/8,664`, integrity `ok`, zero redundant slope, with main/renderer maxima `7.2/50.2 ms` and `7.7/100.3 ms`. Exact code reused the immutable 3.704 GB field-v2 cache (`3,704,676,352` bytes, SHA `8bcfa9a3…d8cc`) without changing its mtime. Migration/DDL, BCP generator mode, outing worker, 500k renderer shape, and live GET-only network shape were untouched: standing proofs remain the byte-identical generator-v4 960k/2M fixtures; 1,999,988 total / 1,242,988 Unassigned worker result in `1.072 s` with `14.3 ms` parent maximum; v8→v9 retention of 2.04M positions/32 participants; macOS 500k `84.3 ms`; and 1 group/34 devices/9 live fixes with selected-only evidence. Proof limit: Ubuntu Xwayland/llvmpipe 500k remains unqualified (`3.685 s` final; `2.941 s` pre-remediation), while the feature remains release-off. No result treats a render budget as a persistence/evidence budget.
