...
### `DON-273` / `DON-276` / `DON-275` — Breadcrumb PR-3 complete coverage — review ready

- Research required: `Low`; the renderer decision and domain requirements are locked.
- Implementation readiness: implementation, G2/G3, migration, all exact-head review remediations, replacement Candidate-B rows, decoded all-period production qualification, the single final packaged soak, final software/browser/visual/package repeat, and five independent reviews are complete. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) is open to `master`.
- Recommended action: perform normal external PR review. Do not merge or release from this implementation task.
...
- Evidence: G2's prescribed 18 serial packaged runs completed on the Ubuntu reference host at measured SHA `8eff87b724ae6`; B alone passed 960k and 2M. The mission/source/worker lifetime remediation reran B's six rows plus two kill probes at `53e38bf`; both scales passed unchanged budgets and remain standing because the latest fixes do not change the measured pipeline. The corrected production driver delivered 959,988/959,988 and 1,999,988/1,999,988 while decoding non-empty geometry for all 13 periods before/after activation at 2.245/4.534 s first useful, 6.212/12.338 s complete, and 21.026/21.825 ms main gaps. Review then bounded the remaining accepted-write outing lookup at `40a713c`. Review #4 subsequently proved that initial direct devices lacked fixed-window backfill checkpoints and that the open-outing cooldown did not suppress early rebuilds. Red-first fixes through `691775a` preserve the completeness/finish fence, enforce the automatic 30-second throttle, and let only explicit operator Retry bypass it. Tool head `69a1096` explicitly counts required backfill-completion audits after the first soak rejected them as unexplained. Final reviews then found the browser proof mirror still omitted the direct-device checkpoint and the packaged G2 harness could race destroyed renderer state. Both are red-first remediated at `928158c`. The first restarted review then found the manual still claimed G3-approved controls were disabled; red-first manual corrections through `133cc14` now state the exact default-on and controlled-rollback posture. Final deterministic evidence is 263 files / 2,104 tests, lint, TypeScript, CommonJS syntax, exact Dots 10/10, build/budgets, full Chromium 158/158, participant/coverage visual 11/11, fresh critical review 13/13, focused manual/default tests 3/8 plus final manual contract 1/1, and unsigned exact-head macOS arm64 plus Ubuntu x64 packages. The final replacement Ubuntu soak remains standing with 8,664/8,664 exact positions, restart/integrity/WAL clean, zero redundant slope, 9.558 ms main / 83.6 ms renderer maxima, 1.130 GB peak RSS, four healthy interactions, zero unexplained events, and a 32-chunk/zero-invalidation ledger because the final proof/manual changes alter no operational packaged hot path. The 16.557 ms integrity-`ok` 3.704 GB v9→v10 migration retaining 2.04M positions also remains standing because schema/open code did not change. Full bindings and proof limits are in `docs/breadcrumb-pr3-t1-evidence.md`.
- Ready-to-start checklist: [x] accepted plan and requirement trace; [x] exact base; [x] G2 ratification and affected-row rerun; [x] BCP-08; [x] flag-on BCP-09 implementation/manual; [x] T1 migration and replacement soak; [x] G3; [x] separate default flip; [x] all exact-head review remediations/evidence rebound; [x] evidence-head repeat; [x] five exact-head reviews; [x] one PR to `master` opened without merge/release.
