...
- Ready-to-start checklist: safety invariants, failure modes, persistence/coordinate impact, TDD regressions, browser/visual proof, packaged multi-day proof, and Fable review are complete. Four tag runs stopped safely before promotion: the first exposed Linux WebGL blocklisting; the second exposed headless background/occlusion timing throttle; the third and fourth proved that ANGLE/OpenGL arguments did not fail closed while unsafe SwiftShader fallback remained permitted. All four retained exact position truth and the latter three completed 4/4 healthy interactions. The final TDD correction removes fallback permission, provisions/scopes Mesa llvmpipe, measures click-to-state without recorder CDP overhead, and separately fails renderer gaps at the unchanged 1,000 ms limit. Three consecutive Ubuntu/Xvfb rehearsals pass at 8,664 exact positions, 4/4 healthy interactions, 133.4-166.7 ms renderer maxima, 16.7 ms median cadence, and 600.9-776.6 ms maximum action with no Vulkan/SwiftShader errors. Full committed-state beta verification passed 8/8 with no skips from `sha.a852dc198832`. Replacement CI artifact provenance and the full Ubuntu artifact matrix remain the release gates specified in `DON-260` and `docs/two-track-execution-workplan.md`.
