...
### `DON-273` / `DON-276` / `DON-275` — Breadcrumb PR-3 complete coverage — final Opus P1 qualification

- Research required: `Low`; the renderer decision and domain requirements are locked.
- Implementation readiness: implementation, G2/G3, migration, and Candidate-B scale evidence remain complete. Donal's Opus exact-`ae13424` review confirmed the core no-false-Complete/100% invariant and found one additional P1: a five-second main timeout could permanently mark a later clean renderer drain as evidence loss. Central retrace confirmed the defect and produced one red-first soft-deadline/provisional-marker correction without changing coverage geometry or schema.
- Recommended action: bind and push the package/soak evidence plus the test-only DON-269 gate alignment, obtain green expanded CI, then run one fresh broad exact-head review plus targeted persistence/completeness, concurrency/finalization, and renderer/input-containment rechecks once. Do not restart five full waves. Do not merge or release until clean.
...
- Evidence: G2's 18 serial runs, B's 960k/2M results, and the 3.704 GB migration remain standing on unchanged surfaces. Application head `42f1694` passes focused 6 files / 156 tests, full deterministic 273 / 2,242, lint, TypeScript/build/budgets, diff, and CJS syntax. It directly proves soft-deadline waiting, late-ack retraction without token change, exact once-only restart promotion, named mission scopes, and production `not_enumerated` / `chunk_missing` / `ingest_outbox_pending` blockers. Exact macOS/Linux packages pass; the Ubuntu X11 packaged soak passes 6/6, 8,664/8,664, one restart, integrity/WAL `ok`, zero renderer crashes, main max 8.871 ms, and four healthy interactions. Browser/visual evidence remains UI/workflow proof, not the production database claim oracle. Expanded CI exposed the old DON-269 200 ms test asserting 100 ms; its 140.628 ms measurement meets the contract and the assertion is aligned test-only. Green CI and exact-head reviews remain open.
- Ready-to-start checklist: [x] accepted plan and requirement trace; [x] exact base; [x] G2/G3; [x] BCP-08/09; [x] migration and scale proof; [x] whole-seam Fable pass; [x] Opus P1 central retrace; [x] red-first provisional-loss correction; [x] direct production blocker tests; [x] full local deterministic gates; [x] replacement application-head macOS/Linux package and CI soak; [ ] green expanded CI; [ ] one fresh broad plus three targeted exact-head rechecks; [x] one PR open without merge/release.
