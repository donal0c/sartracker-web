...
- Ready-to-start checklist: safety invariants, failure modes, persistence/coordinate impact, TDD regressions, browser/visual proof, packaged multi-day proof, and Fable review are complete. Two tag runs stopped safely before promotion: the first exposed Linux WebGL blocklisting; the second rendered and completed 4/4 healthy interactions but exposed headless background/occlusion timing throttle. Both harness boundaries are covered under TDD. Ubuntu/Xvfb passes the foreground-equivalent harness at 8,664 exact positions, 4/4 healthy interactions, 149.9 ms maximum renderer gap, and 646.2 ms maximum action without weakening the 1,000 ms gate. Replacement CI artifact provenance and the full Ubuntu artifact matrix remain the release gates specified in `DON-260` and `docs/two-track-execution-workplan.md`.
