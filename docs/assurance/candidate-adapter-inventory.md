...
| C03/C11/C17 | Scoped outing/known-at, 50k search-pass and raw recursive diagnostics probes, with retained ASAR/raw-byte custody | C03/C11 development passed. DON-237 repairs recursive/encoded/compound credential and legacy private/system-path redaction; C17 exact-head packaged Linux development proof is pending hosted run `35527863604` at `c34fddc2`, and intentional coverage gaps remain. No candidate qualification is implied. |
