...
| `AUD-04` equal map-style writes cause continuous idle redraw | P2 | **Train C repaired; final source/review/CI pending** | `DON-264` / `DON-254`; branch `codex/repair-train-c` from merged PR28 | [Causal record](findings/repair-train-c.md): quiet/idle 0 writes and 0 renders vs old tracking writes 24/43 over 350 ms; real overlay visibility/style recovery passes. Strict frame-latency qualification remains separate |
| `AUD-06` repeated Go To can be ignored/lose its target during style loading | P2 | **Train C repaired; final source/review/CI pending** | `DON-6` / `DON-254`; same map target/style lifecycle | [Causal record](findings/repair-train-c.md): request identity/lifetime and stale camera restoration red/green, delayed-style and repeated rendered-target proof pass |
