...
| `AUD-06` repeated Go To can be ignored/lose its target during style loading | P2 | **Claude-review corrections locally verified; new-head CI required** | `DON-6` / `DON-254`; same map target/style lifecycle | [Review disposition](findings/pr30-claude-review.md): both camera orderings, bounded request lifetime, teardown and failure/retry verified. Stable source 4,997 tests; six browser cases pass. [PR checks](https://github.com/donal0c/sartracker-web/pull/30/checks) |
