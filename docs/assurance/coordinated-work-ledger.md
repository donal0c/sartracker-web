...
| `WAR-06-AUD-01`, `WAR-06-AUD-02`, `WAR-06-CACHE-SIBLING` lifecycle/cache boundary | P1 candidate | **Investigation evidence repaired in PR #20; production hazard remains open** | `DON-267` / `DON-254` (**In Progress**); WAR-06 branch `codex/war-06-tracking-lifecycle`; evidence commit `2390b57b` rebased on `3db57a79` | Three passing real-runtime characterizations: polling-manager delayed flush, mission finish → idle → start with current-fix hydration, and unkeyed cache hydration. Prior reviews `5175815340` and `5176300059` are pre-rebase provenance; new exact-head review/CI is required. No production fix or closure. Require Astra retrace before any production identity guard, then red/green current-position/stationary/persistence/browser/package proof. Retain unresolved strict `<200 ms` failures; PRs #19/#21 remain draft. [WAR-06 report](findings/war-06/WAR-06.md) |
