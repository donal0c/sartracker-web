...
| `WAR-06-AUD-01`, `WAR-06-AUD-02`, `WAR-06-CACHE-SIBLING` lifecycle/cache boundary | P1 candidate | **Investigation evidence repaired in PR #20; production hazard remains open** | `DON-267` / `DON-254`; WAR-06 branch `codex/war-06-tracking-lifecycle`; evidence commit `5ddeb6fd` rebased on `49b2e1d4` | Three passing real-runtime characterizations: polling-manager delayed flush, mission finish → idle → start with current-fix hydration, and unkeyed cache hydration. Exact review `5175815340` reconciled; no production fix or closure. Require Luna/Astra exact-head review, then red/green identity guard, current-position/stationary/persistence/browser/package proof. Retain unresolved strict `<200 ms` failures. [WAR-06 report](findings/war-06/WAR-06.md) |
