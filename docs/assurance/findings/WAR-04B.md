...
The latest published beta, labelled for internal use, remains a historical,
...
proof. For the archive lifecycle successor, only the recorded ref value and
commit named below are inspectable snapshot evidence; the remote ref and active
implementation are mutable and are used only to define a future refresh
boundary.
...
| --- | --- | --- | --- |
| Active renderer/application JavaScript | React, React DOM, MapLibre, Leaflet, Turf, Terra Draw, `geojson-vt`, `saxes`, `vt-pbf`, `proj4`, Zustand and their production runtime closure | active renderer/build graph; Leaflet is the URL-selected fallback; the locked builder also selects production modules for ASAR copy | No npm advisory record at the registry snapshot; no broader clearance. The zero-record production audit does not prove support, compatibility, code safety, exact reachability or exact-candidate contents. |
| Retained Tauri JavaScript | `@tauri-apps/api@2.10.1`, `@tauri-apps/plugin-dialog@2.7.0`, `@tauri-apps/plugin-sql@2.4.0` | historical Tauri lane: API/core is statically imported by ten source adapters and dialog by the GPX adapter; normal Electron execution selects Electron adapters. No JS/TS import of plugin-sql was found. All three are production-declared and selected by the locked builder's production-module collector. | API/dialog: `retained/conditionally reachable`, not current Electron execution. Plugin-sql: `source-unreferenced/package-selected`. Zero audit records do not prove absence from the Electron package. |
| `@types/leaflet@1.9.21` | direct production declaration; its tarball contains `index.d.ts` plus metadata/docs and depends on `@types/geojson` | compile-time only; the locked builder excludes `*.d.ts` from node-module copy | `non-executable/type-only`: it belongs in the audit partition but not in bundled runtime JavaScript. Exact-candidate inspection is still needed to say whether metadata remains in ASAR. |
| `better-sqlite3@12.10.0` | direct production dependency | shipped native SQLite module; rebuilt for Electron target | No npm advisory record at the snapshot; native compatibility qualification remains open. Upstream Electron 42 PR #1475 is no longer blocked: it merged and shipped in `12.10.1`. |
...
| `electron@40.10.0 -> extract-zip@2.0.1` | direct Electron install dependency; `@electron/get@2.0.3` is the sibling downloader, not an intermediate | build/install only | `confirmed exposure`, not shipped app code: Electron postinstall downloads the archive with `@electron/get`, then directly invokes affected `extract-zip` handling. Embedded checksums are used by default, and no repository override was found. A maintained Electron line removes this dependency path. |
...
| `WAR04B-C10` | **Release/support wording overstates enforced state.** The active release guide and seven historical source notes used “immutable” for a procedural write-once rule that GitHub did not enforce. This evidence branch corrects the guide and appends a dated amendment to every affected note; it does not rewrite the historical statements or mutate the live release. Two active release-code comments—`scripts/electron-release-publish.mjs:141` and `build/electron-release-lib.js:314`—also call twice-observed GitHub asset metadata “immutable”; they describe point-in-time identity consistency, not server enforcement, and remain unchanged because release-code edits are outside this branch. The desktop support policy still describes Windows CI/release and platform support not present in the Linux-only workflow, and treats dev dependencies as low release priority despite Electron/builder being release-bearing. | Operators or maintainers can mistake procedural intent, identity observations, or historical lanes for current technical enforcement and qualification. | The amendments and this residual-source record make the historical proof limit explicit, and release notes warn that artifacts are unsigned and not approved for live incidents; neither warning restricts public download. | Reconcile the support policy with the live workflow and selected controls. Rename the two release-code comments to “identity-bearing asset metadata” only in an authorized release-code change. Retain the existing `PKG-002` owner rather than creating a duplicate platform finding. |
...
| `WAR04B-H04` | The eventual merged breadcrumb programme PR-6 leaves dependency/release posture unchanged. | No exact merged archive-implementation SHA exists to test this claim. At the recorded audit snapshot, `origin/codex/breadcrumb-pr6-archive-lifecycle` pointed to `eec92812b783a795c093f37268b295dd2179a3af`, the merged PR5 commit, so that observed ref value contained no successor evidence. The remote ref and active implementation are mutable; local uncommitted state is deliberately excluded from this evidence ledger. This programme ref is not GitHub PR #6, the merged WAR-01 charter. | Run the exact post-programme-PR-6 checklist below against the merged SHA. Until then dependency/release impact is **unproved**, not unchanged. |
...
| --- | --- | --- |
| `WAR04B-K01` | The npm production-classified graph returned a known advisory record. | Cleared only for that narrow claim at the 2026-08-30 registry snapshot: `npm audit --omit=dev --json` returned zero. It is not a vulnerability-free, support, reachability or packaging clearance, and it excludes the Electron/builder classification errors noted above. |
...
| Artifact origin | explicit unsigned/not-for-live-incidents status | public artifacts have no code signing/notarization | keep status explicit; decide and qualify signing separately before stable field use |
...
| `REL-003` | Unsigned/intended-internal/no-auto-update posture remains explicit, but the repository and prerelease are public and world-downloadable. No attestation or SBOM exists; checksums are co-located with mutable unsigned assets. Accepted residual is not stable-field approval. |
...
Breadcrumb programme PR-6 is not GitHub PR #6 and is not release evidence. At
the recorded audit snapshot, `origin/codex/breadcrumb-pr6-archive-lifecycle`
pointed to its PR5-base commit
`eec92812b783a795c093f37268b295dd2179a3af`; that observed ref value contained
no archive-successor implementation to inspect. The remote ref and active
implementation remain unmerged and mutable. No claim from a local uncommitted
worktree is admitted as evidence; the exact merged head must be assessed
afresh.

After breadcrumb programme PR-6 merges, and before WAR-04B is used in any
release decision:

1. Record the exact merged programme PR-6 SHA and diff it from this audited base
   across `package*.json`, `electron-builder.json`, both Electron workflows,
...
- [Electron 40.10.0 install script](https://github.com/electron/electron/blob/v40.10.0/npm/install.js)
- [electron-builder 26.0.12 file rules](https://github.com/electron-userland/electron-builder/blob/v26.0.12/packages/app-builder-lib/src/options/PlatformSpecificBuildOptions.ts)
- [electron-builder 26.0.12 npm production-module collector](https://github.com/electron-userland/electron-builder/blob/v26.0.12/packages/app-builder-lib/src/node-module-collector/npmNodeModulesCollector.ts)
- [electron-builder 26.0.12 module copier](https://github.com/electron-userland/electron-builder/blob/v26.0.12/packages/app-builder-lib/src/util/appFileCopier.ts)
