...
| Tracking | Operator-configurable Traccar settings | Plugin exposes settings dialog/config workflow | Complete | `src/components/settings-workspace.tsx`, desktop settings-store adapters, `tests/e2e/settings.spec.ts` | Settings supports Traccar URL, basic/bearer auth, Test Connection, and Save & Connect; hosted HTTPS deployments connect directly to the team-managed HTTPS Traccar server |
| Tracking | Authentication variants for Traccar | Plugin supports real deployment auth/session shape | Complete | `src/features/tracking/traccar-client.ts`, desktop settings-store adapters, `tests/e2e/settings.spec.ts`, `tests/unit/browser-settings-store.test.ts` | Basic email/password and bearer are implemented; basic auth validated against the live KMRT Traccar server over HTTPS on 2026-06-21 |
