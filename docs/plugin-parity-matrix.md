...
| Tracking | Operator-configurable Traccar settings | Plugin exposes settings dialog/config workflow | Complete | `src/components/settings-workspace.tsx`, `src/infrastructure/settings-store/tauri-settings-store.ts`, `tests/e2e/settings.spec.ts` | Settings supports Traccar URL, basic/bearer auth, Test Connection, and Save & Connect |
| Tracking | Authentication variants for Traccar | Plugin supports real deployment auth/session shape | Complete | `src/features/tracking/traccar-client.ts`, `src/infrastructure/settings-store/tauri-settings-store.ts`, `tests/e2e/settings.spec.ts` | Basic email/password and bearer are implemented; basic auth validated against the live KMRT Traccar server on 2026-05-14 |
