...
> When this file disagrees with `docs/web-parity-verification-matrix.md`, `docs/qgis-replacement-roadmap.md`, or `docs/parity-execution-protocol.md`, this file wins for QGIS replacement capability. Browser-vs-Electron runtime parity is tracked separately in `docs/hosted-browser-testing-plan.md`.
...
| Mission lifecycle | Mission notes at lifecycle level | Plugin stores mission-level notes/summary data | Partial | persistence model includes `notes` on mission in the desktop mission-store contract, currently still in the legacy-named `src/infrastructure/mission-store/tauri-mission-store.ts` file | Not clearly exposed as a live operator workflow in current UI |
| Mission lifecycle | Finalize mission | Plugin has explicit finish then finalize/archive/lock flow | Backend only | `MissionStatus` includes `finalized`; `createMissionArchive` exists in the desktop mission-store contract; legacy Rust archive code exists in `src-tauri/src/persistence.rs` | No complete operator-facing finalize/archive workflow found in frontend |
...
| Mission lifecycle | Backup mirror sync | Plugin persists crash-safe backup copy | Complete | `syncBackup` surface in the desktop mission-store contract, Electron persistence, and retained Rust persistence implementation | Present at persistence layer |
| Mission lifecycle | Mission audit events | Plugin records mission lifecycle/audit events | Backend only | `MissionEvent`, `listMissionEvents`, Electron persistence, and retained Rust audit/event support | Audit trail exists, but there is no operator-facing mission log UI yet |
...
| Tracking | Operator-configurable Traccar settings | Plugin exposes settings dialog/config workflow | Complete | `src/components/settings-workspace.tsx`, desktop settings-store adapters, `api-lib/traccar-proxy.js`, `api/session.js`, `api/devices.js`, `api/positions.js`, `tests/e2e/settings.spec.ts` | Settings supports Traccar URL, basic/bearer auth, Test Connection, and Save & Connect; hosted HTTPS deployments can use the same-origin proxy for the team-managed HTTP Traccar server |
| Tracking | Authentication variants for Traccar | Plugin supports real deployment auth/session shape | Complete | `src/features/tracking/traccar-client.ts`, desktop settings-store adapters, `api-lib/traccar-proxy.js`, `tests/e2e/settings.spec.ts`, `tests/unit/traccar-proxy.test.ts` | Basic email/password and bearer are implemented; basic auth validated against the live KMRT Traccar server on 2026-05-14 |
...
| Map | Packaged/offline topo map bundles | QGIS can operate with local data/offline sources more deeply than viewed-tile cache | Partial | `docs/offline-map-resilience.md`, offline map readiness and current-view coverage preflight models | Field-ready viewed-tile workflow is explicit and tested with current-view cache checks; packaged regional bundles and saved coverage manifests remain gaps if full offline parity is required |
...
| Persistence | SQLite mission store | Plugin evolved to structured mission persistence; desktop app targets SQLite behind runtime IPC adapters | Complete | Electron persistence implementation, retained Rust persistence implementation, and shared mission-store contract | Present |
