# SAR Tracker Electron 0.1.0-beta.13.1 — preparation draft
...
Tag `electron-v0.1.0-beta.13` remains pinned to
`2d4f436add40ed9c279488c9ac4e3cb267c5cee2`. Release run
[36223196332, attempt 1](https://github.com/donal0c/sartracker-web/actions/runs/36223196332)
failed before packaging or draft creation: the AUD-03 500,000-row stationary
projection count-oracle test took 21,443 ms and exceeded its 20,000 ms harness
deadline. Same-source serial post-merge CI passed that case in 5,586 ms; focused
unchanged local checks passed. Parallel suite contention is the supported
explanation, not a demonstrated product regression.

This replacement aligns the release correctness command with the established
serial source gate. Application code, fixture size, count assertions, 20-second
test deadline and separate strict 200 ms qualification remain unchanged. No
failed evidence is discarded; the replacement release workflow must pass before
its exact artifacts are admitted for Ubuntu testing.
