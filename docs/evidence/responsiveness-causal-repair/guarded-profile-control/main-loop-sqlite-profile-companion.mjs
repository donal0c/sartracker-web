...
async function connect(url) {
  ...
  socket.addEventListener('message', ({ data }) => {
    ...
    if (message.error || message.result?.exceptionDetails) {
      ...
      const error = new Error(message.error ? 'Inspector protocol error' : 'Inspector evaluation failed')
      error.inspectorDetail = {
        code: message.error?.code ?? null,
        text: String(message.error?.message ?? details?.text ?? '').slice(0, 200),
        exceptionClass: String(details?.exception?.className ?? '').slice(0, 80),
        description: String(details?.exception?.description ?? '').split('\n')[0].slice(0, 240),
      }
...
async function save() {
  ...
  const report = {
    ...
    preinstallationErrors, readinessObservations, launches,
  }
...
try {
  while (!stopping && Date.now() - startedAtUnixMs < durationMs) {
    ...
    try {
      inspector = await connect(target.webSocketDebuggerUrl)
      stage = 'electron_readiness'
      // Do not resolve Electron's lazy performance/fetch globals while its entry
      // imports or bootstrap may still be running. This predicate reads neither.
      const readiness = await inspector.evaluate(`(() => {
        if (!process.versions.electron) return { ready: true, basis: 'native Node control' };
        const main = process.mainModule;
        if (!main || main.loaded !== true || typeof main.require !== 'function') {
          return { ready: false, basis: 'Electron entry module is not loaded' };
        }
        const app = main.require('electron').app;
        return { ready: app?.isReady() === true, basis: 'Electron entry module loaded and app ready' };
      })()`)
      const readinessCheckedAtUnixMs = Date.now()
      readinessObservations.push({ targetId: target.id, atUnixMs: readinessCheckedAtUnixMs, ...readiness })
      if (readinessObservations.length > 32) readinessObservations.shift()
      if (readiness?.ready !== true) {
        inspector.close()
        await delay(100)
        continue
      }
      stage = 'timer_installation'
      ...
      if (!launch) {
        ...
        launch = { number: launches.length + 1, targetId: target.id, timeOriginMs: snapshot.timeOriginMs,
          readinessAtInstallation: { ...readiness, checkedAtUnixMs: readinessCheckedAtUnixMs },
      ...
      if (launch) {
        ...
        preinstallationErrors.push({ targetId: target.id, stage, atUnixMs: Date.now(),
          message: error.message, inspectorDetail: error.inspectorDetail ?? null })
        if (preinstallationErrors.length > 32) preinstallationErrors.shift()
      }
    } finally {
