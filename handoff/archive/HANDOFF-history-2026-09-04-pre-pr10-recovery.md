# Archived HANDOFF Snapshot — Before PR #10 Recovery

> Historical snapshot only. Do not use this as the current baton; resume from
> `handoff/HANDOFF.md`. This file preserves the pre-recovery state for context.
...
- **Beta.8 release evidence:** tag `electron-v0.1.0-beta.8` (HEAD `34e3fc1`); CI release run `28012741523` is **green** and the deep Ubuntu packaged smoke PASSED on that CI-built artifact. Release note: `docs/releases/sartracker-electron-0.1.0-beta.8.md`.
  ...
  - Ubuntu reference-host note: the machine was upgraded off Wayland to **X11** (kernel 6.17); drive packaged smoke with `--ozone-platform=x11` (Wayland flag segfaults). Beta.8 smoke scripts were retained in a host-local validation workspace; evidence was mirrored to repo `output/beta8-ubuntu-smoke/evidence-fixed/`.
...
- **Ubuntu reference-host evidence:** kernel 6.17, 338 GB free. Host-local cached
  fixtures: five-day 1.254 GB, fourteen-day 3.514 GB, field 3.704 GB.
...
- **Completed checkpoint:** `DON-243` now reproduces beta.11 deterministically on packaged Ubuntu.
  ...
  memory high-water, per-cycle phase timings, and fail-closed JSON verdicts were
  retained in the host-local MSR evidence workspace. Durable summary:
...
- **Completed checkpoint:** `DON-244` bounded storage diagnostics are implemented and packaged-
  ...
  mission survived, and privacy scan passed. Host-local evidence was retained;
  durable summary:
...
- **Completed local checkpoint:** `DON-240` removes all size-dependent work from the Electron main
  ...
  2.5 ms. Worker kill/restart/support export passed. Candidate AppImage SHA: `147ec92…`;
  host-local evidence was retained. Durable summary:
...
- **Completed local checkpoint:** `DON-245` preserves device/position rows and every `last_seen`,
  ...
  paused-mission restart recovery. Host-local evidence was retained; durable
  summary: `docs/releases/beta12-redundant-write-removal.md`.
- **Completed local checkpoint:** `DON-246` now has a deterministic packaged mock-Traccar soak with
  ...
  zero renderer crashes, main maxima 147.9/146.9 ms, and RSS maxima 1.49/1.64 GB.
  Host-local evidence was retained; durable summary:
...
- GitHub Linux validation build `28959613296` for `78fa836` passed, including Linux artifact
  inspection and AppImage launch smoke. CI artifacts were copied into a host-local
  validation workspace; AppImage and deb hashes were verified from `SHA256SUMS`
  (path-normalized).
- Ubuntu reference-host map-only A/B on X11 with the private
  `reeks-standard-60km-z16.mbtiles` package
...
- Historical note: a protected Vercel preview was used for Beta.8 validation. The preview and its expired share link are no longer retained here.
...
## Historical provider-validation note

- Provider endpoints and inline validation credentials were deliberately removed from this archived baton. Retrieve current credentials only at action time through the approved channel.
