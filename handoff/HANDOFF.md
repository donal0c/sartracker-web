...
- **Breadcrumb PR-3 Opus P1 correction in final qualification (2026-08-26):** Donal's exact-`ae13424` Opus review confirmed the core no-false-Complete/100% claim but found one new lifecycle P1: main's five-second renderer timeout could turn a later clean drain into permanent evidence loss. Central retrace confirmed the deadline inversion (but disproved the report's 3.704 GB migration trigger because migration finishes before renderer creation). The red-first correction now treats five seconds as a durable soft deadline: provisional mission-scoped uncertainty keeps the renderer alive, late clean acknowledgement retracts it without advancing the loss generation, and only negative acknowledgement, actual renderer/process loss, or unresolved restart promotes permanent loss exactly once. Active/paused/finished scope reasons are explicit; production tests now pin `not_enumerated`, `chunk_missing`, and `ingest_outbox_pending`; PR Linux CI now runs lint, the full unit suite, and production build before packaging. Local green is 273 files / 2,242 tests, focused 6 / 156, lint, TypeScript/build/budgets, diff, and CJS syntax. Commit/push this application correction, rerun exact-head macOS/Ubuntu package plus the one CI soak, bind evidence, then run exactly one fresh broad plus persistence, concurrency, and renderer-containment rechecks. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) is **not merge-ready** until those gates are clean; do not merge or release.
