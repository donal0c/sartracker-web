...
- **Breadcrumb PR-3 final remediation gate (2026-08-26):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and G2/G3 approvals remain binding. The review of documentation head `fa863da` found two lost-evidence P1s: health-marker write failure was swallowed as RAM-only success, and Finish could strand renderer evidence outside the active-mission teardown fallback. Both are now remediated red-first: evidence-loss marking rejects until atomic marker rename plus directory sync succeeds; Finish drains every bounded batch, seals late acceptance across the durable status transition, and refuses to finish on drain failure while later finalization advances the sealed state. Focused lifecycle/outbox green is 3 files / 51 tests; full unit is 272 / 2,194; lint, TypeScript, build/budgets, backend 51/51 + one intentional ignore, Chromium 13/13, and selected critical visual plus uncached review 2/2 pass. The manual documents Finish refusal/repair. Commit and push this code-and-documentation head, then repeat unsigned macOS and exact-head Linux package/soak gates and restart all five independent PR-3 reviews from zero. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and unmerged. The 2026-08-26 review-topology amendment is prospective for PR-4 onward and does not relax PR-3. Do not merge or release from this implementation task.
