...
- **Breadcrumb PR-3 final remediation gate (2026-08-26):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and both G2/G3 approvals remain binding. Review then found that a rejected final database completeness claim retained equal delivered/total counts and the panel rendered a full progress bar despite withholding `Complete`. Application head `6946446` closes that false-100 path red-first: equal counts are explicitly labelled as not yet verified, the progress element is withheld until the final claim succeeds, and partial progress remains visible. The operator manual, screenshot, critical visual regression, controller failure-path test, and panel tests are updated. Full unit is 264 files / 2,137 tests, plus lint, TypeScript, build/budgets, exact Dots 10/10, selected visual Playwright and uncached independent visual review, and participant/coverage Chromium 10/10. Exact Linux run [`32926442726`](https://github.com/donal0c/sartracker-web/actions/runs/32926442726) passed x64 AppImage/`.deb` packaging, AppImage launch, and the 8,664/8,664 packaged soak with digest `ab671355…`, integrity `ok`, one restart, 32.024 ms main maximum, zero redundant slope, zero renderer crashes, and four healthy operator interactions. The next documentation-only descendant requires deterministic exact-head gates and five fresh independent exact-head reviews from zero before review readiness. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and unmerged. Do not merge or release from this implementation task.
