...
- **Fable deep-analysis remediation queue:** `DON-230` is the parent Linear issue for `output/fable-deep-analysis.md` (2026-07-06). `DON-231`, `DON-232`, `DON-233`, `DON-234`, `DON-236`, and `DON-237` are complete locally; `DON-235` remains open/skipped for a separate pass. DON-236/DON-237 harden Electron runtime boundaries: fatal main-process errors now flush crash/runtime diagnostics before relaunch/exit, packaged `file:` IPC senders must be the built renderer entrypoint, windows deny external navigation/popups, Traccar proxy requests are constrained to the configured provider origin with a response-size cap, file/path IPC is limited to app-owned or operator-selected paths, clean-exit marking is awaited, diagnostic fields cannot override app-owned metadata, diagnostic redaction is recursive for Authorization headers/tokens/URL credentials/home paths, and settings no longer advertise Traccar auth if credential persistence fails. Small UI polish from visual validation: paused live-refresh status reads `PAUSED`, drawing action rows stay visible, and required badges are high-contrast. Verification passed: red-to-green Electron/settings/filesystem/diagnostics regressions, `node --check` Electron modules, `npm run lint`, `npm run build`, `npm run test` (152 files / 1041 tests), `npm run test:backend` (47 passed / 1 ignored), targeted diagnostics/settings Chromium E2E, full `npm run test:e2e` (162/162), full visual project (34/34), `npm run visual:review -- --fail-on critical` (39/39), `npm run electron:pack`, packaged bad-secret smoke, and in-app Browser validation against the running dev server for startup, settings, diagnostics, recovery resume, tracking panel, and drawing dialogs. Official offline smoke was not applicable because no private MBTiles package is present. Next recommended queue item is either `DON-235` or the remaining deep-analysis children `DON-238`/`DON-239`.
