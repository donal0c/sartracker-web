...
- **Breadcrumb PR-3 final exact-head gates (2026-08-25):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and both G2/G3 approvals remain binding. The final review-wave fixes now also cancel destroyed-renderer reads, finalize tile files atomically, attest the applied history filter before Complete, stop settled sync loops, and clear old-mission geometry. Production/test head `4b740f2` passed 262 files / 2,095 tests, lint, TypeScript, CommonJS syntax, exact Dots 10/10, build/budgets, and Ubuntu Chromium 158/158. Exact coverage visual passed 7/7 and fresh critical review 9/9. Operator-visible/package head `fea89db` passed the manual contract plus unsigned macOS arm64 and Ubuntu x64 packaging. Corrected 960k/2M production qualification, the ratified G2 rows, the single packaged soak, and the 3.704 GB migration remain standing under their explicit invalidation rules. Next: commit this evidence binding, run five independent exact-head reviews, then open one PR to `master` if all five are clean.
