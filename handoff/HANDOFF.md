...
- **Breadcrumb PR-2 exact-head review remediation (2026-08-23):** code commit `6b2fe59e7025` closes the fourth-pass one P1/one P2 after the earlier review passes. Mission finish now drains the immutable pending group-membership observation and holds the membership fence through the persisted transition; continued write failure leaves the mission active with the actionable dialog error. A restart accepts a later participant's scoped checkpoint and resumes from its durable cursor rather than mission start. Full serial unit `229 files / 1,862`, lint/build, backend `51 / 1 ignored`, exact Dots `10/10`, and Chromium `154/154` pass; standing participant visual `4/4` plus critical review `1/1` remain valid. The default parallel unit runner made the unrelated 500k timing assertion miss at `100.139/108.031 ms`, while that gate passes alone and the complete suite passes serially. Electron main/schema/generator, package soak shape, fixture/worker, current-position, 500k renderer, and live GET-only surfaces were untouched, so their standing proofs remain valid: flag-off/on `8,664/8,664` with integrity `ok`; 2M worker `1.072 s`; macOS 500k `84.3 ms`; Ubuntu 500k still unqualified at `3.685 s`; v8→v9 retains 2.04M positions/32 participants; immutable 3.704 GB v2 cache unchanged. Await an independent exact-current-head approvable comment; do not merge or start PR-3.
