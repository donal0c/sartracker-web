...
- **PR5 mission evidence and replay is at final qualification/review entry:** the four exact-head reviews on `044a73887eef791b18c30df0838b7f1bd021fc56` were not clean. Their accepted persistence/completeness, concurrency/finalization, renderer/input and broad findings are source-retraced in `docs/breadcrumb-pr5-evidence.md` and covered by retained regressions: durable GPX receipt/byte recovery and persisted issues, ID/path collision rejection, all-path per-file settlement, lifecycle fences and worker ownership, explicit future legacy gaps, opaque replay paging/filters/whole totals, same-mission evidence links, watched-directory mission containment, and bounded renderer IPC. New-store two-clock daily counts remove the cold whole-position replay scan while legacy stores remain explicit. Local proof is green: unit `286 files / 2,361 tests`, backend `51 / 1 ignored`, Chromium `162/162`, visual Playwright `58/58`, fresh critical/high visual rechecks PASS, unsigned macOS arm64 package, packaged soak `8,664/8,664` with `4.0 ms` main maximum, and actual pending/retained GPX `SIGKILL` restart recovery. Fresh 960k/2m scale seeks are `59.46/86.78 ms`, exact late pages `55.91/69.16 ms`, restart `53.26/50.42 ms`, concurrent current-write maxima `41.92/47.84 ms`, and event-loop maxima `47.21/54.95 ms`. The final pushed-head Linux workflow and the fresh broad plus all three focused exact-head rechecks remain completion gates. Their immutable-head attestations belong in the PR/final coordinator handoff so the reviewed tree does not move. Do not merge or release.
