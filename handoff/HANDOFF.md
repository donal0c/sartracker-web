...
Most recent verification in this turn (`DON-70` coordinate footer and converter marker workflow):

- Red reproduced: `npx vitest run tests/unit/coordinate-bar.test.ts tests/unit/coordinate-converter-dialog.test.ts` failed because the footer had no DMS readout and the converter had no `coordinate-create-marker-btn`.
- Browser validation exposed a DMS rollover bug (`9°41'60.000"W`) at the default map coordinate; red reproduced with `npx vitest run tests/unit/coordinates.test.ts`.
- Passed focused coverage: `npx vitest run tests/unit/coordinate-bar.test.ts tests/unit/coordinate-converter-dialog.test.ts tests/unit/coordinate-tool.test.ts tests/unit/coordinates.test.ts` — 4 files / 21 tests.
- Passed inbuilt Browser validation at `http://127.0.0.1:1420/?missionHarness=1`: footer rendered DD `51.970000°N, 9.700000°W`, Irish Grid `V 83219 81075`, and DMS `51°58'12.000"N, 9°42'00.000"W`; Convert opened the dialog; converting `Q 99842 04015` and choosing `Create Marker Here` opened the normal Marker Details form with `52.179336°N, 9.464944°W` and `Q 99842 04015`.
- Passed: `npx tsc --noEmit`, `npm run lint`.
- Passed: `npm run test` — 126 files / 625 tests.
