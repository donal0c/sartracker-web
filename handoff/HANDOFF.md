...
**Resolve DON-35 Linux native-module baseline before sending the full Electron build wider.** The old Ubuntu 18.04 smoke fails before meaningful app validation because `better_sqlite3.node` requires GLIBC `2.29` and the machine has `2.27`. Next: decide whether to rebuild Linux artifacts against an older glibc baseline or set a newer minimum Linux requirement, then run the full Linux smoke for map render, settings secret persistence, mission create/restart persistence, GPX import, attachment save/open, diagnostics export, and direct `https://kmrtsar.eu` tracking. Before any tester-facing release, confirm the diagnostics button produces enough sanitized evidence to debug launch/map/tracker/SQLite failures without repeated manual questions. DON-27 now proves the read-only Leaflet fallback route; Electron remains the preferred runtime lane unless DON-29 decides degraded-mode editing is required.
...
- `DON-35` — S8c.5: Electron Linux artifact build and field-validation gate. Local artifact build done; Ubuntu 18.04 smoke blocked by `better-sqlite3` requiring GLIBC `2.29`.
