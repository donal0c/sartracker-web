...
- **PR5 current remediation (2026-08-29):** persistence/completeness and concurrency/finalization remain CLEAN on exact `c1ad54b8`; Cicero's renderer recheck was CLEAN on `7998755d`, but Grace's fresh broad review rejected it after the real sandboxed preload forwarded a 64 MiB Replay request ID into both read and cancel IPC. Strict retrace retained P2 because the production path and pre-main structured-clone mechanism were deterministic, PR5 explicitly requires closed bounded pre-IPC operations, current-position IPC shares the operational main boundary, and Replay owns the bridge. Linux run `33239678392` was canceled. The required repeated-seam pause traced the shared cause to projected query bodies plus a main-only request-ID assumption and a generic cancel wrapper; architecture/scope/domain/proof remain unchanged. Executable fix `1b84639d5804` excludes cancel from the generic map and projects every Replay request identity through the exact non-empty/100-character/allowed-ASCII contract before any IPC. Red-first attacks cover empty, wrong-typed, malformed and 64 MiB IDs across four reads and cancel with zero invokes; valid operations still use exact channels. Local proof: focused renderer/replay 18 files / 121 tests, full unit 296 / 2,530, backend 58/one intentional ignore, affected Chromium 18/18 (complete 168/168 and fresh visual 4/4 remain from the renderer-identical parent), lint/build/diff, unsigned arm64 package `sha.bb71bc0d0eca`, and a 13.773 s two-launch soak with 6/6 batches, exact 8,664/8,664 positions/digest, integrity `ok`, zero redundant slope/crashes, four healthy interactions and 5.929 ms maximum main round trip. Exact-head Linux plus fresh broad and affected renderer rechecks remain; persistence/concurrency do not restart. PR opened/review-ready is intermediate; do not merge or release.
