...
- **PR5 replacement exact head `299c0b72` is rejected (2026-08-28):** broad and renderer review were clean. Concurrency/finalization found that Tauri's raw immediate transaction could survive task cancellation on the pooled connection, expose an uncommitted GPX projection/audit after caller failure and then disappear on restart. Persistence/completeness found byte-unbounded legacy GPX selection/copying on the Electron main thread (427.61 ms open at 64 MiB; 161.49 ms deferred event-loop gap) plus direct archive creation bypassing the unsettled-GPX fence. The candidate Linux run was cancelled. Red-green replacements use SQLx's owned immediate transaction, launch with only bounded SQLite length/preview probes, migrate one bounded legacy row per background turn, quarantine over-envelope originals outside map/Replay while blocking mutation/lifecycle/archive, and apply the complete fence to direct archives. Current proof: serial unit 289/2,422; backend 55/1 intentional ignore; lint/build/budgets; Chromium 165/165; visual 58/58; uncached review 69/69; indexed and authentic-v11 960k gates green. Commit/push, exact-head Linux/package proof and all four clean independent reviews on the final head remain mandatory. Do not merge or release.
