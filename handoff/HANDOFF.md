...
- **Breadcrumb PR-2 exact-head review remediation (2026-08-23):** code commit `c6f417c3b7f1` closes the fifth-pass two P1/one P2. The finish cutoff waits for pre-fence roster calls even through participant-scope recovery; observations received while the store finish is pending are replayed at their exact time when finish is refused and discarded only after durable success. Late-participant anti-entropy retains the validated scoped history origin instead of resetting to mission start. Full serial unit `229 files / 1,865`, focused `5 / 77`, lint/build, backend `51 / 1 ignored`, exact Dots `10/10`, participant Chromium `6/6`, and full Chromium `154/154` pass; standing participant visual `4/4` plus critical review `1/1` remain valid. Electron main/schema/generator, package soak shape, fixture/worker, current-position, 500k renderer, and live GET-only surfaces were untouched, so their standing proofs remain valid only for those surfaces; Ubuntu 500k remains unqualified at `3.685 s`, and the immutable 3.704 GB v2 cache remains unchanged. Await an independent exact-current-head approvable comment; do not merge or start PR-3.
