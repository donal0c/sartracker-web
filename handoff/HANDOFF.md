...
- **Breadcrumb PR-3 final remediation gate (2026-08-26):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and both G2/G3 approvals remain binding. External and renewed review invalidated the earlier clean verdicts with false freshness/classification, worker/query trust-boundary, tile-cache, catalog recovery, and retained-source attestation defects. Application head `080abe8` closes them red-first. Renderer failures now carry the source's actual finalized activation, including retained unchanged periods; an accepted failure revokes Complete and one bounded recovery replaces every superseded source before Complete can return. Delayed failures from the superseded source set coalesce, while a genuinely new-activation failure still starts a new fail-closed recovery. Focused green is 5 files / 82 tests; full unit is 264 files / 2,126 tests, plus lint, TypeScript, build/budgets, and participant/coverage Chromium 10/10. Exact Linux run [`32917584753`](https://github.com/donal0c/sartracker-web/actions/runs/32917584753) passed x64 AppImage/`.deb` packaging, AppImage launch, and the 8,664/8,664 packaged soak with integrity `ok`, one restart, 25.3 ms main maximum, zero redundant slope, and four healthy operator interactions. This record's documentation-only descendant still requires its own deterministic gates and five fresh independent exact-head reviews before review readiness. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and unmerged. Do not merge or release from this implementation task.
