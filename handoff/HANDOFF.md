...
- **Breadcrumb PR-3 external-review remediation (2026-08-26):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and both G2/G3 approvals remain binding. External and renewed review invalidated the earlier clean verdicts with false freshness/classification, worker/query trust-boundary, tile-cache, and catalog recovery defects. Commits through `9dd736c` close them red-first. Coverage IPC preserves failed terminal ownership, recovers renderer-lost and alive-renderer stages before Retry, and uses a progress-based abandoned-stage sweep: it tries all candidates, settles the live token where possible, then retries stale conflicts after the worker is empty, failing only after a full no-progress retry. Lost-generation cleanup remains idempotent only with no different live stage. Focused green is 5 files / 57 tests; full unit is 264 files / 2,118 tests, plus lint, TypeScript, CommonJS syntax, build/budgets, and participant/coverage Chromium 10/10. The `2d316da` Linux CI pass is superseded by this operational-path change; a new exact-head Linux package/soak check and five fresh independent reviews must be green before review readiness. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and unmerged. Do not merge or release from this implementation task.
