...
- **Active release blocker — `DON-260` / `0.1.0-beta.12.1`:** breadcrumb identity, correction audit, deterministic whole-route selection, worker restart reconstruction, mission-wide late-fix reconciliation, strict parsing, runtime isolation, explicit persistence/cache failures, and fix-time/accuracy UI are complete under TDD. The Linux gate also found and fixed a real Devices-workspace transition delay; workspaces now open/close directly, preserve modal focus, restore the exact opener, and keep docked mode non-modal. Final local proof: lint/build; `169 files / 1241 tests`; backend `51 passed / 1 ignored`; Chromium `135/135`; visual `36/36`; fresh independent visual review `41/41`; candidate Ubuntu/Xvfb exact truth and 4/4 healthy interactions. Exact-head branch run `30441228109` passed at `c7ffcb43755c`: 8,664/8,664 exact positions and digest, two direct Mesa llvmpipe launches, 4/4 focused trusted interactions, AppImage launch, main max `32.7 ms`, renderer max `416.6 ms`, internal/external action maxima `128.1/263.3 ms`, zero crashes/errors/throttling/redundant growth, healthy SQLite/WAL. Fable and independent adversarial reviews found no P1/P2. No failed run created a release. Still required: final no-skip verifier, final tag workflow, full exact-CI-artifact Ubuntu matrix, controlled publication, fresh-download smoke, and Linear closeout. Full Discovery loading and beta.13 storage/archive work remain out of scope; beta.12 cannot reopen schema 5.
