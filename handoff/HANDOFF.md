...
- **Breadcrumb PR-3 final remediation gate (2026-08-26):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and both G2/G3 approvals remain binding. Final review invalidated evidence head `4b84fb0` with a P1: an empty invalidation-worker result could mark an outing edit drained without dirtying moved chunks, leaving stale counts fresh and restoring a false database claim. Application head `08ce78a` closes it red-first. The worker result now has a strict identity/key envelope, and the ledger independently computes a position-free conservative canonical device×affected-period floor before any drain; an empty result over-dirties safely instead of under-dirtying. Full unit is 264 files / 2,140 tests, plus lint, TypeScript, build/budgets, exact Dots 10/10, CommonJS syntax, and participant/coverage Chromium 10/10. Exact Linux run [`32928637039`](https://github.com/donal0c/sartracker-web/actions/runs/32928637039) passed x64 AppImage/`.deb` packaging, AppImage launch, and the 8,664/8,664 packaged soak with digest `e02b19b6…`, integrity `ok`, one restart, 116.250 ms main maximum, zero redundant slope, zero renderer crashes, and four healthy operator interactions. This backend-only remediation does not change the operator manual or screenshots. The next documentation-only descendant requires deterministic exact-head gates and five fresh independent exact-head reviews from zero before review readiness. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and unmerged. Do not merge or release from this implementation task.
