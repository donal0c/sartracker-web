...
- **Breadcrumb PR-2 review remediation qualified (2026-08-23):** exact code commit `e1e7f46ffa06` closes the three P1s and regraded outing-runtime P2 reproduced at `11c3b8d`: immediate selected-participant current visibility is separate from historical evidence windows; direct/group selection has one active path with UI/store/browser checks plus partial unique indexes; scope loading/failure is visible, preserves the raw fallback cache, and retains deferred live evidence; operational recovery scope outranks unrelated review state; and stale outing mutations cannot republish the wrong mission. `idx_positions_mission_timestamp` now supports the summary join. Gates: `227 files / 1,812` unit, lint/build, backend `51 / 1 ignored`, Chromium `153/153`; exact flag-off and flag-on macOS arm64 packages each passed a 32-device, 8,664-position restart soak with integrity `ok`, zero redundant slope, and main/renderer maxima `2.1/133.3 ms` and `5.0/166.2 ms`; regenerated schema-v9 fixtures are 960k `545,128,448` bytes / `ad6ff8e4…129a0` and 2M `1,135,009,792` bytes / `53d7edc5…136fb`. Ubuntu x64 independently reproduced the 2M SHA, used the new range index across 12 outings, returned exact 1,999,988 total / 1,242,988 Unassigned counts in `1.087 s`, and kept the parent event loop at `15.8 ms` maximum; report SHA `363272c4…ddc1d`. The earlier 3.704 GB zero-outing run remains valid only for migration/integrity/grandfathering/Unassigned and responsiveness, not the join. Schema v9 still migrates in flag-off builds; that known non-downgradable behavior remains an explicit programme decision. PR-2 may enter Donal's next four exact-head reviews; do not merge or start PR-3.
...
- **Breadcrumb coordination workflow:** one implementation task owns a complete PR; BCP units are internal TDD checkpoints. PR-2 remediation is qualified; Donal may start the next four exact-head reviews.
