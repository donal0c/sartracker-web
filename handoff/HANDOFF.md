...
- **Breadcrumb programme status (2026-08-22):** BCP-00 decisions are locked in `docs/breadcrumb-programme-execution-policy.md`. BCP-10 (`DON-266`) remains green: its named contract proves 20,006 selected fixes through three SQLite/controller/GeoJSON pages and detects eight injected faults. BCP-01 (`DON-267`) is implemented on `codex/breadcrumb-pr1-ingest-safety` at `d1daa3b3556b4abb8c6106cf5824c5fe4e749011`: mixed current-position responses preserve valid fixes with structured per-device rejection health; roster failure is advisory when positions succeed; history reset retains the last current snapshot; and fix/device/server timestamp provenance prevents server receipt time masquerading as a fresh device fix. Focused unit/integration tests, full unit `197 files / 1,587 tests`, lint, build, targeted Chromium `2/2`, the high-severity visual entry/review, and BCP-10 `10/10` are green. BCP-02 is paused at its durability/design gate; no BCP-02/05 implementation has landed. This is local/browser proof only, not packaged, provider, field, or release proof.
...
- **Breadcrumb coordination workflow (2026-08-22):** `docs/breadcrumb-programme-coordination-workflow.md` is binding. Donal controls PR review and explicitly authorizes every implementation start. The current PR-1 executor remains paused while BCP-02 is reconciled against the canonical team Q&A; keep `DON-267` open until PR-1 merges.
