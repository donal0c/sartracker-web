...
- **Breadcrumb PR-2 exact-head review remediation (2026-08-23):** code commit `896700ef3064` closes all seven P1 and six P2 findings from review `5002724165`. Current positions no longer wait on history windows or participant SQLite; removal retains the non-stale last accepted marker; incomplete rosters cannot infer leaves and reconcile them when completeness returns; hydration retries; selection and checkpoint/audit writes fail closed; membership ties use durable sequence; outing workers release only after physical exit; and stale mission mutations cannot republish another mission. Schema v9 no longer creates the prohibited mission-wide positions index and repairs candidate-v9 stores by dropping it; outing summaries stream positions once in a read-only worker. Fresh fixtures are 960k `476,151,808` bytes / `1e0537c6…595d1` and 2M `991,227,904` bytes / `80790414…5d00`. Exact Ubuntu x64 proof returned 1,999,988 total / 1,242,988 Unassigned across all 12 outing windows in a cold `2.719 s` with `1.6 ms` parent maximum. A 3.704 GB v8→v9 copy opened/migrated in `24.7 ms`, retained 2,040,000 positions and 32/32 grandfathered participants, had zero outings, integrity `ok`, and no global positions index. Exact flag-on/off Ubuntu packages each passed 32-device / 8,664-position restart soaks with integrity `ok`, zero redundant slope, and main/renderer maxima `12.7/83.6 ms` and `10.6/66.9 ms`; the flag-off package also passed live Traccar confirmation with 34 devices / 9 fixes and no reconciliation warning. Software gates: `227 files / 1,829` unit, lint/build, backend `51 / 1 ignored`, Chromium `153/153`. Schema v9 still migrates in flag-off builds; that approved non-downgrade boundary is unchanged. Await an independent exact-current-head approvable comment; do not merge or start PR-3.
