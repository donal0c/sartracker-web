...
- **Breadcrumb PR-3 final five-review gate (2026-08-26):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and G2/G3 approvals remain binding. The fresh review of `dcc950c` found that main could lose a finished mission's renderer-held evidence because its teardown fallback queried only `active/paused`. Central source retrace confirmed the full chain. Code head `0c674e8` gives main one bounded mission-table query for every unfinalized evidence scope (`active`, `paused`, `finished`) and durably marks all of them before unacknowledged close/reload/crash may proceed; finalized missions remain excluded. Composed lifecycle green is 9 files / 163 tests; full unit is 272 / 2,202; lint, TypeScript, build/budgets, backend 51/51 + one intentional ignore, Chromium 13/13, selected critical visual 2/2 and fresh uncached review 2/2 pass. Exact-head unsigned macOS arm64 packaging passed. Exact-head Ubuntu run [`32977328663`](https://github.com/donal0c/sartracker-web/actions/runs/32977328663) passed Linux x64 package/native inspection, llvmpipe, 8,664/8,664 exact persistence with integrity `ok`, one restart, zero renderer crashes, two graceful exits, packaged soak and real AppImage launch/close. This binding change is documentation-only; restart all five independent PR-3 reviews from zero on its final head. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and unmerged. The 2026-08-26 review-topology amendment is prospective for PR-4 onward and does not relax PR-3. Do not merge or release from this implementation task.
