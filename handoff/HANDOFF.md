...
- **`DON-261` / `DON-262` / `DON-263` beta.12.5 in qualification:** beta.12.3 field evidence proves a warning-state flash rather than a crash; the first-history state, Open Devices target, packaged input audit, and basemap-independent overlays are corrected. Beta.12.4 tag `a312568957b1` and CI run `30491599605` are green but remain unpublished/private: final review found that marker/helicopter async icon failures escaped retry, disposed work could write stale state, and one changed-surface 2,000-position browser test passed only on CI retry. Beta.12.5 remediation is strict red-to-green: async work is awaited/retried/coalesced, cleanup aborts stale icon continuations, and browser harness bulk injection writes once. Current local proof: focused `21/21`; unit `1,296/1,296`; backend `51 / 1 ignored`; Chromium `142/142` with no retry/flake and the former failure passing first attempt in 1.9 s; visual `37/37`; uncached review `43/43`; lint/build/diff. Independent code review returned `RELEASE` with no P1/P2/P3; Fable 5 returned `RELEASE` with no P1/P2. Next: commit the candidate locally, run clean no-skip beta verification, then push/tag beta.12.5, require green tag CI, and qualify exact CI AppImage plus real installed `.deb` on Ubuntu before publication. Persistent sync diagnostics remain the non-blocking `DON-264`.
