...
- **Breadcrumb PR-3 verification complete; five exact-head reviews are the only remaining gate (2026-08-25):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B, G2/G3, the v9→v10 field migration, and the packaged tracking soak stand. Two fresh review waves found fourteen production/renderer-attestation blockers; every blocker was reproduced red-first and remediated, most recently at `7ec7a81`. Exact production delivery passes at 959,988/959,988 and 1,999,988/1,999,988 fixes, with 2.228 s/4.490 s first-useful, 5.726 s/11.931 s complete, 21.849 ms/21.796 ms main gaps, and false Complete blocked by the intentional backfill marker. The documentation-complete tree passes 257 files / 2,007 unit tests, lint, TypeScript, changed CJS, exact Dots 10/10, build, Chromium 157/157, coverage visual 4/4, fresh critical review 6/6, and macOS arm64 plus Ubuntu X11 x64 packaging. Binding: `docs/breadcrumb-pr3-t1-evidence.md` and `output/pr3-production-qualification/`. Next: restart all five independent reviews on the same pushed head; if clean, update Linear and open one PR without further repository mutation.
