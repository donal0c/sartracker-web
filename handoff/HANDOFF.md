...
- **PR5 current remediation (2026-08-29):** broad review rejected exact head `d314db88` because its refresh-only Search Operations barrier left the retained-snapshot `load()` path open; a held generation-one page could still replace generation-two evidence. Concurrency and renderer were clean on that invalid head, persistence was not restarted, and none of those verdicts counts. Central retrace accepted the finding. Executable fix `a0a4cdf47719` treats `loading || refreshing` as one backend-authoritative Review-read barrier, retains exact refresh-token/result fences, and disables search/page/write controls throughout either state. The retained-load regression failed red and both load modes now pass. Local proof is green through the exact renderer/replay set 181/181, full unit 296 files / 2,516 tests, backend 57/one intentional ignore, Chromium 167/167, visual 2/2 plus fresh uncached critical review 4/4, lint/build/diff, unsigned macOS arm64 packaging, and a two-launch packaged soak with 6/6 batches, exact 8,664/8,664 positions/digest, integrity `ok`, zero redundant slope, zero crashes, four healthy interactions and 2.608 ms maximum main round trip. Exact-head Linux and all four independent exact-head reviews must restart on one final code-and-documentation head because this fix crosses renderer/runtime concurrency. PR opened/review-ready is intermediate; do not merge or release.
