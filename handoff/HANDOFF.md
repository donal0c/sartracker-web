...
- **Next candidate (DRAFT, NOT yet promoted):** `0.1.0-beta.7` / `electron-v0.1.0-beta.7` — DON-177 app-owned local credential storage + DON-176 docked Review, and the first published-track build to carry the DON-175 keyring startup guard (beta.6 was built/smoked but never shared). Built by GitHub Actions (final run `27596…` superseded; current **run `27601812958`, success**). Full deep Ubuntu on-device smoke against the CI-built AppImage PASSED all gates incl. the DON-177 credential matrix, real-keyring migration, docked Review 6/6, and the release-blocking bad-secret gate. **Verdict: PROMOTE — awaiting Donal's explicit promotion approval before publishing.** AppImage SHA256 `848eb06321536c76f831a4e566450be82695f4d3b6c8336fce3fbcb4926ca4a7`. (beta.6 draft is obsolete and was deleted; beta.6 was never published.)
...
**ACTION FOR DONAL: promote beta.7 or not.** `electron-v0.1.0-beta.7` (DON-177 local credential storage + DON-176 docked Review + the DON-175 keyring guard) is built green (run `27601812958`) and its draft GitHub prerelease holds the Linux AppImage + `.deb` + `SHA256SUMS`. The full deep Ubuntu on-device smoke against the CI-built artifact passed every gate (core lifecycle, coordinate safety, sanitized diagnostics with `credential storage: local-file`, the DON-177 credential matrix 3/3, real same-machine keyring migration via live Traccar online 33 devices/8 fixes, the release-blocking bad-secret gate, and docked Review 6/6). It is **NOT yet published** — promotion needs Donal's explicit approval. To publish: `gh release edit electron-v0.1.0-beta.7 --repo donal0c/sartracker-web --draft=false` (then set `docs/releases/sartracker-electron-0.1.0-beta.7.md` Publication State + this handoff). Note: beta.7 supersedes the unpublished beta.6 (its draft was deleted). Residual risk: Traccar credentials are now local app-owned plaintext with best-effort `0600` (intentional trade for reliability on trusted team machines, DON-177); not OS-keyring encryption.
