...
- **Breadcrumb PR-3 external-review remediation (2026-08-25):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and both G2/G3 approvals remain binding. External review invalidated the earlier clean-review claim with false freshness, worker-envelope override, and tile-cache escape; `259fd74` fixed those red-first. The first renewed review then proved a follow-on false Complete path: building a newly discovered closed outing left its previously fresh Unassigned sibling unchanged, duplicating the same two source fixes across both chunks. It also found that failed duplicate activation could orphan the committed stage and renderer input could replace the main-owned chunk worker operation. Commit `31ba509` revises every existing sibling when inventory grows, proves the post-build partition is exactly outing 2 / Unassigned 0 before Complete, retains and idempotently retries activation, and copies validated chunk fields with main-owned `kind`. Focused green is 5 files / 51 tests; full unit is 264 files / 2,112 tests, plus lint, TypeScript, build/budgets, and focused participant/coverage Chromium 10/10. The standing G2, decoded qualification, v9→v10 migration, and prior soak remain historical proof for unchanged geometry/schema surfaces; the live PR exact-head Linux package/soak check and five fresh independent reviews must be green before review readiness. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and unmerged. Do not merge or release from this implementation task.
...
- **Breadcrumb coordination workflow:** one implementation task owns complete PR-3 through BCP-07 → BCP-08 → BCP-09. G2/G3 remain approved; the external-review remediation above supersedes the earlier five-review result. Require the live exact-head Linux check and five fresh clean reviews before treating PR #3 as review ready.
