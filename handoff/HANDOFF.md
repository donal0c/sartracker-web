...
- **PR5 current remediation candidate (2026-08-28):** exact pushed head `356a9e03` passed Linux run `33198210110` but all four independent reviews rejected it. Central retrace accepted five defects: filtered Replay object pages dropped their filter context; all-invalid GPX lost structured point/segment rejections; 200k-point outing assignment copied the full evidence set on Electron main; idempotent Finalize could return stale success after Unlock; and Replay worker IPC cloned unbounded renderer fields. Executable fix `9a1a54f8` passed the full local deterministic/browser/visual/backend/960k/package gates, but head `52465dfc` was invalidated by Linux run `33203567089`: a local Replay preload import could not load inside Electron's sandbox. Exact package reproduction preceded sandbox-safe fix `09845b8b`; its rebuilt package passed two launches plus 6/6 batches and 8,664/8,664 positions with a 5.7 ms main maximum. Head `b59c54e1` was then rejected by Linux run `33205517532` when a 50-object legacy baseline turn produced a 239.90 ms event-loop gap. Fix `fcf55f88` reduces that durable turn to 25; five focused 50k runs, the complete 60-test file and full 2,480-test gate are green. Linux run `33206596556` then showed the old 15-second test loop ending at 42,500/50,000 while the cursor still advanced; test correction `84eb2245` retains exact 50k completion and the unchanged `<200 ms` assertion while allowing the smaller turns 30 seconds. Documentation binding, push, fresh exact-head Linux and all four restarted reviews remain mandatory. PR opened/review-ready is intermediate; do not merge or release.
