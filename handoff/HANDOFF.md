...
- **PR5 pushed head `1e9b6acc` is rejected (2026-08-28):** the prior signed-rowid/batched-cursor remediation remained correct, but Linux run `33160578384` measured 300.63 ms startup at 500,000 settled rows because migration initialization still used repeated full-table extrema aggregates. Renderer review also found unbounded Replay time/timezone parsing and incomplete GPX IPC/filesystem/issue-page containment. The replacement uses indexed rowid extrema; strict calendar-valid Replay instants and the fixed Europe/Dublin contract; atomic 100-file/4,096-character filesystem admission; bounded GPX IDs, outing actors and byte-packed issue pages with visible truncation warnings. Fresh tree proof is unit 289/2,438, backend 55/1 intentional ignore, syntax/lint/build/budgets, Chromium 165/165, visual 58/58, uncached visual 69/69 (`visual-review-2026-08-28T15-24-05Z.json`), indexed 960k (60.77 ms seek, 46.33 ms late page, 45.58 ms max current write, 48.15 ms event-loop, exact restart) and authentic-v11 960k (5.59 ms open, 610.87 ms seek, 3.86 ms current write, explicit fallback). Commit/bind/push, package/Linux proof and a restarted four-review exact-head wave remain mandatory. Do not merge or release.
