...
- **Breadcrumb PR-3 external-review remediation (2026-08-26):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and both G2/G3 approvals remain binding. External and renewed review invalidated the earlier clean verdicts with false freshness/classification, worker-envelope and chunk-operation override, tile-cache escape, and activation recovery defects. Commits through `55163cc` close them red-first. Coverage IPC now preserves failed terminal ownership, retains retryable abandoned stages after renderer loss, settles them before replacement sync, accepts cleanup for a lost worker generation only when no different stage exists, and treats a new catalog sync as the same renderer explicitly superseding its prior doubly failed stage. Focused green is 5 files / 55 tests; full unit is 264 files / 2,116 tests, plus lint, TypeScript, CommonJS syntax, build/budgets, and participant/coverage Chromium 10/10. The `745ab3e` Linux CI pass is superseded by this operational-path change; a new exact-head Linux package/soak check and five fresh independent reviews must be green before review readiness. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and unmerged. Do not merge or release from this implementation task.
