...
- **Breadcrumb PR-1 review remediation (2026-08-23):** [PR #1](https://github.com/donal0c/sartracker-web/pull/1) remains unmerged on `codex/breadcrumb-pr1-ingest-safety`. The latest red-first review pass adds a renderer flush plus main-process completeness fence, finalized-mission rejection guard, exact-delivery recovery, mission-scoped multi-reason health markers, 4,096-file/64 MiB durable outbox backpressure, eight-record/background replay, safe worker-construction cleanup, source-device ownership enforcement, explicit fix-time warning preservation, pre-mission warning correction, disposal draining, duplicate stationary-publication suppression, and cached per-device stationary projection. New corrupt records remain quarantined and mission-scoped; automatic deletion was rejected because it would erase unique evidence. Tauri remains historical/reference. Current proof: full unit `210 / 1,706`, backend `51 / 1 ignored`, full Chromium `146 / 146` plus final stationary Chromium `1 / 1`, touched visual `3 / 3` and final stationary visual `1 / 1`, fresh critical reviews `4 / 4` then `2 / 2`, BCP-10 `10 / 10`, lint/build/Node checks/diff check, and macOS arm64 Electron packaging green. The earlier exact `f24906c425e0` 3.704 GB migration/Review and 8,664-position packaged soak remain valid because this pass does not change migration, Review SQL, tracking persistence, or soak instrumentation. DON-251 still owns the cold `~7.5 s` query/index/retention optimization; the 200 ms gate is not waived.
