...
- **PR5 exact head `96c2047b` is rejected (2026-08-28):** fresh broad, concurrency/finalization and renderer/input-containment reviews accepted five further findings: incomplete legacy-coordinate/rejection retention and hidden zero-point artifacts; chunked same-hash comparison bypass; unbounded all-import startup migration; Tauri GPX identity/finalization checks outside the write transaction; and browser `Earlier exact page` repeating the current page with drifted bounds. The persistence slot was not spent on an invalid head. Replacement work is red-green locally: one validated legacy pass, three-import launch cap plus atomic resumable background slices and lifecycle fence, full same-hash row comparison, transaction-current Tauri checks with deterministic two-connection races, and Electron-compatible bidirectional browser paging with a 502-fix E2E. Prior Linux/package/review proof is historical only; rerun full gates and all four independent reviews on one final replacement head before completion. Do not merge or release.
