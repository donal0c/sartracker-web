...
- **Next candidate (DRAFT, NOT yet promoted):** `0.1.0-beta.6` / `electron-v0.1.0-beta.6` — Linux keyring/startup hotfix for DON-175. Built by GitHub Actions (run `27596864861`, success). Full Ubuntu on-device smoke against the CI-built AppImage PASSED, including the new release-blocking bad-secret/keyring gate beta.5 missed. **Verdict: PROMOTE as a keyring hotfix — awaiting Donal's explicit promotion approval before publishing the GitHub release.** SHA256 `555e1a6c5d78da4b88c432c477d6c4382d0bf5ed29dea5c43cf41c48f22b2656`.

## Last Work Done

DON-175 (S2 Electron, Urgent Bug) — Linux keyring / undecryptable Traccar secret startup fault. **DONE & packaged-smoke verified on CI-built beta.6.** Closed in Linear.
...
**ACTION FOR DONAL: promote beta.6 or not.** `electron-v0.1.0-beta.6` (DON-175 keyring hotfix) is built green (run `27596864861`) and its draft GitHub prerelease holds the Linux AppImage + `.deb` + `SHA256SUMS`. The full Ubuntu on-device smoke against the CI-built artifact passed all release-blocking gates (including the new bad-secret/keyring gate). It is **NOT yet published** — promotion needs Donal's explicit approval. To publish: `gh release edit electron-v0.1.0-beta.6 --repo donal0c/sartracker-web --draft=false` (then update `docs/releases/sartracker-electron-0.1.0-beta.6.md` Publication State + this handoff). Residual risk to weigh: DON-176 (Review overlay blocks active-mission controls) is a known, separately-tracked issue carried into beta.6, documented under that release's Known Limitations.
