...
- **Breadcrumb PR-2 exact-head review remediation (2026-08-23):** code commit `73f5f4e013b8` closes the third-pass five P1/two P2 findings after the earlier `b7591572d780` / `8f637a192164` passes. Mission changes now publish an empty loading participant scope; stale mutations cannot release another mission's write guard; failed membership deltas retain their original observation and provisional current-map visibility; initial and anti-entropy history writes apply participation-at-fix-time and clamp evidence cursors; active membership-write failures are visible; finish remains active until every fixed participant backfill window completes; and legacy preset-v2 fixture caches remain reusable while BCP fixtures stay generator-v4. Software gates: `229 files / 1,857` unit, lint/build, backend `51 / 1 ignored`, exact Dots `10/10`, Chromium `154/154`, participant visual Playwright `4/4`, and fresh critical finish-refusal review `1/1`. Exact-code Ubuntu flag-off/on 32-device restart soaks each persisted `8,664/8,664`, integrity `ok`, zero redundant slope, with main/renderer maxima `7.2/50.2 ms` and `7.7/100.3 ms`. The immutable 3.704 GB field-v2 cache reused with checksum `8bcfa9a3…d8cc` and unchanged mtime. Migration/DDL, BCP generator mode, outing worker, 500k renderer shape, and live GET-only network shape were untouched, so their standing proofs remain valid under the cost-aware invalidation rule: 2M worker `1.072 s` / parent `14.3 ms`; macOS 500k `84.3 ms`; Ubuntu 500k still unqualified at `3.685 s`; v8→v9 retains 2.04M positions/32 participants; live proof remains 1 group/34 devices/9 fixes with selected-only evidence. Await an independent exact-current-head approvable comment; do not merge or start PR-3.
