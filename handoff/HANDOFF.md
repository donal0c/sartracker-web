...
- **PR5 current remediation (2026-08-29):** broad, persistence and concurrency were clean on exact head `210826cc`; renderer/input containment rejected it with three P2s. Central retrace confirmed that PR5 preload calls could carry unknown multi-megabyte fields, Search Operations synchronously returned unbounded records and used per-pass link queries, and Replay silently hid the 201st eligible GPX outing behind a fixed 200-row filter list. Executable fix `f3a14d53847c0830a1404ac6de95e123fcd65154` uses closed preload projections with byte/count checks, an off-main read-only worker with searchable opaque-keyset pages and exact totals for Search Operations, and a replay-generation-bound searchable outing-filter page. The 201-outing and 32/64 MiB regressions were red then green; a 50,000-pass worker fixture returns 25 projected rows, exact total and a continuation cursor under the 256 KiB envelope while the main-process timer remains below 200 ms. Local proof is green through 296 files / 2,508 unit tests, backend 57/one intentional ignore, Chromium 167/167, visual 2/2 and fresh uncached critical review 4/4, lint/build/CommonJS/diff checks, unsigned macOS arm64 packaging, and a packaged soak with two launches, 6/6 batches, exact 8,664/8,664 positions, integrity `ok`, zero redundant slope and 1.8 ms maximum main round trip. Because the remediation crosses renderer, IPC and persistence boundaries, exact-head Linux and all four independent exact-head reviews restart on one final code-and-documentation head. PR opened/review-ready is intermediate; do not merge or release.
