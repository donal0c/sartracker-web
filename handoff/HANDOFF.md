...
- **WAR-01 whole-application assurance baseline is open on [PR #6](https://github.com/donal0c/sartracker-web/pull/6) from `codex/war-01-assurance-charter` (2026-08-29):** the docs-only charter and 39-row stable hazard register are pinned to base `80309c995a18eeb190cce4310c9a46b0f46d5263` and cover all 12 resilience domains plus the five absolute blockers. The `DON-274`/`DON-277`/`DON-278`/`DON-279` issue set on GitHub PR #5 is unmerged, so its GPX/versioned-evidence/replay/search-area/attachment-custody seams remain `rewrite-pending` and require a short exact-merge reconciliation; the `DON-248`/`DON-252`/`DON-253` archive successor remains unimplemented and required before streamed archive/restore resilience. Local lint, the serial full unit suite (`276` files / `2,293` tests), structural/citation audit, and diff checks pass. The first exact-head reviews required remediation; author handoff is valid only when both the broad life-safety and focused traceability reviews are clean on the eventual final SHA. No production code, tests, workflow, dependency, release configuration, operator behavior, or manual surface changed.
