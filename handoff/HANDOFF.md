...
- **Next beta candidate:** `0.1.0-beta.9` is a **ready-to-publish draft prerelease** from the Fable remediation queue (`DON-230`; fixes `DON-228`, `DON-231`-`DON-238`; `DON-239` parked). Local macOS/web gates passed on 2026-07-07, GitHub Actions run `28875685324` is green, and deep Ubuntu packaged smoke passed on 2026-07-08 against the CI-built Linux artifacts: AppImage sha `99d3aeb3…`, deb sha `7d3490b4…`. Release note: `docs/releases/sartracker-electron-0.1.0-beta.9.md`. **Keep draft until Donal explicitly approves publication/tester sharing.**
