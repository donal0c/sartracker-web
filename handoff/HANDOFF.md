...
- **PR5 current remediation candidate (2026-08-28):** exact head `a584b2aa` passed Linux run `33207406645` and was then rejected by all four independent reviews. Central retrace accepted five P2s: unbounded marker/drawing mutations blocked Electron main; schema-v12 rewrote 500k legacy events synchronously; restart GPX receipt settlement was not atomic with batch accounting; the Tauri fallback could delete GPX after a concurrent committed Finish; and default Replay object paging changed omitted filters to empty arrays. Executable fix `12532025` bounds mutable evidence before persistence, moves legacy event provenance to durable restartable 1,000-row background turns with Replay/Finish fail-closed, makes receipt settlement atomic, transaction-fences the fallback GPX delete, and preserves unfiltered paging context. Local proof is green: 294 files / 2,484 tests, lint/build/CommonJS/Rust formatting, backend 57/1, Chromium 167/167, visual 58/58 plus fresh uncached 69/69 review, indexed 960k with 44.16 ms maximum event-loop gap and exact restart equality, unsigned macOS package, and two-launch 8,664/8,664 packaged soak with 6.3 ms main maximum. Documentation binding, push, fresh exact-head Linux and all four restarted exact-head reviews remain mandatory. PR opened/review-ready is intermediate; do not merge or release.
