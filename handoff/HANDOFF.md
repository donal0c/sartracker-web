...
- **Breadcrumb PR-3 final remediation (2026-08-25):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and both G2/G3 approvals remain binding. The latest exact-head review blockers are now fixed red-first: MapLibre style changes reattach the active catalog; renderer and worker activation use an owned commit/finalize/rollback handshake; retained tiles remain readable during a build; worker stage IDs cannot repeat after restart; startup and cross-mission evidence health cannot erase a newer/stronger warning; and the operator sees an explicit evidence-wait state without false 100% or an irrelevant Retry. The production qualification driver is now committed and self-attests its checkout. Focused unit 86/86, full serial unit 2,035/2,035, lint, TypeScript, CommonJS syntax, exact Dots 10/10, build/budgets, focused Chromium coverage 4/4, and the new critical visual + independent review pass. The parallel full-unit invocation still trips the pre-existing 500k breadcrumb timing assertion under runner contention (149–199 ms versus its 100 ms assertion) while that test passes alone and the complete single-worker suite is green. Next: commit this bounded remediation, rebind the invalidated Ubuntu production/G2/packaged evidence, run final exact-head gates, then restart five independent reviews from scratch.
