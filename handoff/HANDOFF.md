...
- **PR5 mission evidence and replay is replacing rejected exact head `4761405b340eece7bebea9eb8ae37147962bc188`:** its broad and concurrency rechecks confirmed canonical `datetime-local` fractions and the GPX/pass race fixes, with concurrency clean, but broad and renderer found the bounded Search Operations envelope incomplete. Huge fractional timestamps, legacy drawing IDs and UI-owned search-area geometry could still consume the main isolate; malformed optional text could silently become absence; browser advisory coverage diverged. The repeated seam is now one complete Electron/browser preflight for area, assignment, pass, retirement and list scope before lookup/sort/state copy/persistence, with strict types, raw-length-first rejection, bounded/valid JSON, retained advisory coverage and matching UI limits. New red-green probes cover 64 MiB geometry, 32 MiB timestamp/legacy ID, malformed optional text and advisory parity under the 200 ms current-position gate. Focused affected units are 116/116; full unit 288 files/2,403 tests, backend 51/1 intentional ignore, lint/build/budgets, full Chromium 164/164, visual 58/58 and fresh uncached review 69/69 (`visual-review-2026-08-28T01-38-13Z.json`) are green. The first Chromium pass exposed only a historical `{}` search-area test fixture; it was changed to a valid Polygon, its layer suite passed 8/8 and full Chromium reran clean. The fresh broad review plus persistence, concurrency and renderer focused rechecks remain required on one pushed exact head. PR opened/review-ready remains intermediate; do not merge or release.
