...
- **PR5 `60bf8907` review wave is rejected; cross-boundary replacement is locally green (2026-08-28):** exact-head Linux run `33187744020` passed, but all four independent reviews rejected that head. Accepted findings were unbounded mutable-object and GPX migration work, a direct-archive write race, transition verbs shown as lifecycle state, terminal pass outcomes without an end, cross-renderer/abandoned Replay workers, unbounded renderer GPX accumulation, and lost large-object disclosure on later Replay pages. Executable replacement `1f69c7907e79739914b49955b1d391bb02c315e0` fixes them red-first with bounded durable baselines, current-position priority, fail-closed Finish/Finalize/archive fences, sender-owned Replay workers, reconstructed lifecycle state, required completed-pass ends, 25-entry replace-in-place GPX pages and page-local summary limitations. Local proof: unit 294/2,460; backend 55/1 intentional ignore; lint/build/budgets; Chromium 166/166; visual 58/58 and effective uncached review 69/69 after one blurred-raster recapture; indexed/authentic-v11 960k; unsigned macOS arm64 package; packaged soak 6/6 and 8,664/8,664 with 5.1 ms main maximum. Bind docs/version, push, exact-head Linux and all four restarted exact-head reviews remain mandatory. Do not merge or release.
