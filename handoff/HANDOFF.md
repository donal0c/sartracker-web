...
- **Breadcrumb PR-3 review remediation active (2026-08-25):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and both G2/G3 approvals remain binding, and the v9→v10 migration proof is unchanged. The latest exact-head review invalidated Candidate B's measured source-strategy rows and found that worker catalog commit could precede actual replacement-source activation, renderer-held rejection evidence could leave a false Complete window, an obsolete stage could cross-acknowledge an identical newer catalog, a failed stage could poison retry, and coverage could start while the required mission model was off. Every finding is now reproduced red-first and remediated: predecessors remain serviceable through source load and opaque worker activation, pending evidence revokes completeness synchronously without downgrading stronger health, stage failure is retryable, and the two flags are combined. Current remediation checks: 259 files / 2,019 unit tests, lint, TypeScript, changed JS syntax, Candidate-B renderer build, and focused Chromium 6/6. Prior G2 Candidate-B rows, exact-production qualification, packaged soak, final exact-head gates, and reviews are deliberately not claimed for this tree. Next: commit/push the remediation, rerun only the invalidated B rows plus exact-production/soak evidence on Ubuntu X11, bind the results, rerun final gates, then restart all five reviews from scratch.
...
- **Breadcrumb coordination workflow:** one implementation task owns complete PR-3 through BCP-07 → BCP-08 → BCP-09. G2/G3 are ratified. Review-driven source-activation changes require the affected Candidate-B rows and downstream hot-path qualification to be rebound before final exact-head gates and five newly restarted independent reviews.
