...
- **Breadcrumb PR-3 final remediation (2026-08-25):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and both G2/G3 approvals remain binding. Exact code head `17e75f2` fixes the latest review blockers red-first: MapLibre style changes reattach the active catalog; renderer and worker activation use an owned commit/finalize/rollback handshake; retained tiles remain readable during a build; worker stage IDs cannot repeat after restart; startup and cross-mission evidence health cannot erase a newer/stronger warning; and the operator sees an explicit evidence-wait state without false 100% or an irrelevant Retry. The committed self-attesting production driver passes exact 960k/2M delivery at 2.317/4.527 s first useful and 15.884/23.219 ms main gaps. Candidate B's six affected G2 rows plus two kill probes pass unchanged budgets. The single replacement packaged Ubuntu soak passes 8,664/8,664 fixes, restart, integrity, four healthy interactions, 9.099 ms main maximum, 100.3 ms renderer maximum, 1.116 GB peak RSS, and zero redundant slope. Focused unit 86/86, full serial unit 2,035/2,035, lint, TypeScript, CommonJS syntax, exact Dots 10/10, build/budgets, focused Chromium coverage 4/4, and the new critical visual + independent review pass. The parallel full-unit invocation still trips the pre-existing 500k breadcrumb timing assertion under runner contention (149–199 ms versus its 100 ms assertion) while that test passes alone and the complete single-worker suite is green. The 3.704 GB v9→v10 migration remains standing because schema/open code is unchanged. Next: commit this evidence binding, run final exact-head deterministic/Chromium/visual/macOS+Ubuntu package gates, then restart five independent reviews from scratch.
...
- **Breadcrumb coordination workflow:** one implementation task owns complete PR-3 through BCP-07 → BCP-08 → BCP-09. G2/G3 and rebound heavy evidence are complete. Final exact-head gates and five newly restarted independent reviews remain before the PR opens.
