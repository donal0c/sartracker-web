...
- 2026-06-02 by Codex — `DON-69` completed as a conservative clutter-reduction change. Mission Control now offers a header `Minimize` control only for an active configured mission; the collapsed state keeps a restore affordance and mission summary visible while hiding lifecycle controls, and paused/recovery/non-active states stay expanded so `DON-64` safety cues remain reachable. The coordinate footer now keeps operator-facing coordinates plus `Convert`, with low-use fixed CRS cells removed from the live surface. Operator manual updated. Verification passed: red→green tests for active Mission Control collapse, paused non-collapse, and coordinate footer CRS removal; focused tests 2 files / 3 tests; `npx tsc --noEmit`; `npm run lint`; `npm run test` 124 files / 621 tests; `npm run build` (budgets clean); `npm run test:backend` 45 passed / 1 ignored; and `git diff --check`. UI/browser caveat: no Playwright/Chrome DevTools run per Donal's standing restriction, and the inbuilt Browser tool was unavailable in this session.
...
- Passed focused coverage: `npx vitest run tests/unit/mission-control-panel-collapse.test.ts tests/unit/coordinate-bar.test.ts` — 2 files / 3 tests.
- Passed: `npx tsc --noEmit`, `npm run lint`.
- Passed: `npm run test` — 124 files / 621 tests.
