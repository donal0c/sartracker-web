...
- **Next beta candidate:** `0.1.0-beta.9` is being cut from the Fable remediation queue (`DON-230`; fixes `DON-228`, `DON-231`-`DON-238`; `DON-239` parked). Local macOS/web gates passed on 2026-07-07, including no-skip `npm run beta:verify`, visual E2E/review, browser smoke, packaged mission lifecycle/archive smoke, coordinate/diagnostics smoke, and bad-secret packaged smoke. Release note: `docs/releases/sartracker-electron-0.1.0-beta.9.md`. **Do not publish or share as current tester artifact until `electron-release.yml` is green and Donal completes the Ubuntu smoke on the CI-built artifact.**
