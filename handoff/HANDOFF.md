...
- **Branch:** `master` remains canonical for unrelated work. Breadcrumb programme changes use the five authorized PR branches; PR-1 merged as `9481cf462e0e`, and PR-2 merged as exact PR-3 base `7021fc1ef33e6da5c91c96cd86e836fc3754f48f`.
- **Breadcrumb PR-3 Opus P1 correction in final qualification (2026-08-26):** Donal's exact-`ae13424` Opus review confirmed the core no-false-Complete/100% claim but found one new lifecycle P1: main's five-second renderer timeout could turn a later clean drain into permanent evidence loss. Central retrace confirmed the deadline inversion (but disproved the report's 3.704 GB migration trigger because migration finishes before renderer creation). Application head `42f169426170cbc450dd02e32b617c7238f7705e` now treats five seconds as a durable soft deadline: provisional mission-scoped uncertainty keeps the renderer alive, late clean acknowledgement retracts it without advancing the loss generation, and only negative acknowledgement, actual renderer/process loss, or unresolved restart promotes permanent loss exactly once. Active/paused/finished scope reasons are explicit; production tests pin `not_enumerated`, `chunk_missing`, and `ingest_outbox_pending`; PR Linux CI now runs lint, the full unit suite, and production build before packaging. Local green is 273 files / 2,242 tests, focused 6 / 156, lint, TypeScript/build/budgets, diff, and CJS syntax. Exact macOS/Linux packages pass; the Ubuntu X11 packaged soak passes 6/6, 8,664/8,664, one restart, zero renderer crashes, integrity/WAL `ok`, main max 8.871 ms, and four healthy operator interactions. CI's expanded full suite exposed only the old DON-269 200 ms test asserting 100 ms; its 140.628 ms result meets the named gate, and the assertion is aligned red-to-green at 26/26. Bind this test/evidence descendant, rerun CI, then run exactly one fresh broad plus persistence, concurrency, and renderer-containment rechecks. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) is **not merge-ready** until those gates are clean; do not merge or release.
