...
- **Breadcrumb PR-2 review remediation (2026-08-23):** four exact-head reviews of `11c3b8d` reproduced three P1s: current positions were incorrectly tested against historical participation time, duplicate active selection paths could survive one Remove, and an unhydrated/failed participant scope could blank and overwrite the fallback cache. The regraded outing mutation reentrancy P2 was also reproduced. Red-to-green fixes now separate immediate current visibility from historical evidence persistence, reject/disable duplicate direct/group selection with partial unique-index backstops, retain deferred live evidence through scope hydration, eagerly hydrate and visibly gate participant scope while preserving raw cache data, prefer operational recovery scope over an unrelated review mission, fence stale outing mutations, and add `idx_positions_mission_timestamp` for the outing-summary join. Software gates are green (`227 files / 1,812` unit, lint/build, backend `51 / 1 ignored`, Chromium `153/153`). A three-outing query over the deterministic 1,999,988-fix mission used the new index and returned exact counts in `966.9 ms` locally. The earlier Ubuntu 1.530 s Review proof had zero outings and therefore proves only the Unassigned/count path, not the outing join. Commit-bound package/fixture invalidation gates remain before a new review-ready head. Schema v9 still migrates in flag-off builds; that known non-downgradable behavior remains an explicit programme decision, not silently changed in remediation.
...
- **Breadcrumb coordination workflow:** one implementation task owns a complete PR; BCP units are internal TDD checkpoints. PR-2 is in review remediation; finish the exact-head package/fixture gates before Donal starts the next four-review round.
