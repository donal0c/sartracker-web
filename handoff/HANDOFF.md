...
- **`DON-267` field-feedback bridge is open for review in [PR #4](https://github.com/donal0c/sartracker-web/pull/4) (2026-08-27):** `SAR-QA-021` locks Traccar position `fixTime` as the only breadcrumb evidence clock. Current locations using device/server time—or legacy cache rows whose provenance cannot be established—stay visible only as explicitly time-unverified and are filtered before exact history or mission persistence. Exact fix times and Last Seen/Last Success surfaces render as local operator time with timezone/offset while stored UTC remains unchanged. Live-current polling is separated from single-flight/coalesced history; current and history now also emit independent timings, outage recovery, success, and rejection-count diagnostics. Exact-history source-row rejections retain counts/reasons as durable anomaly evidence without overwriting the latest current-position health. Regressions prove a second current request/publication after 5 seconds with no history overlap, prevent older history completion from overwriting a newer offline status, and guard late old-mission cursor settlement. The 37,479 exact-fix contract traverses 10,000 + 10,000 + 10,000 + 7,479; progressive PR-3 coverage remains the default all-history view. Reviewer-remediation verification: lint; TypeScript/build/bundle budgets; unit 276 files / 2,282 tests; legacy backend 51 passed / 1 ignored; Chromium 159/159; focused warning visual Playwright 1/1 with manual screenshot inspection; and the earlier headed-browser proof of `2026-08-22T15:10:17.000Z` as `22/08/2026, 16:10:17 GMT+01:00 (Europe/Dublin)` with zero console warnings/errors. Exact implementation-head CI run `33052939355` attempt 1 rejected one 261.38 ms main-process sample while retaining 8,664/8,664 positions, zero redundant slope, 8.30 ms p95, 42.21 ms p99, and healthy interactions; unchanged-SHA attempt 2 passed the complete Linux build, artifact/native-SQLite inspection, packaged soak, and AppImage launch smoke with an 18.29 ms main maximum. The isolated first-attempt stall was not reproduced but remains retained as unexplained runner/host variance, not silently erased. No programme-final Ubuntu, live-server, or field proof was run. Pre-bridge SQLite rows do not retain source-clock provenance, so this change cannot retrospectively classify whether any used a fallback; no such field data loss or substitution is asserted without evidence. Do not merge or release this PR from the implementation task.
