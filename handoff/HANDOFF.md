...
- **PR5 approval correction (2026-08-29):** the later Opus 5 multi-agent review on exact `2136ad6b` accepted one P2: the append-only casualty treatment log inherited a 2,000-byte Search Operations note limit, eventually blocking new clinical updates and making pre-existing longer casualty markers uneditable even for unrelated fields. The correction gives the cumulative log a fail-closed 524,288 UTF-8-byte envelope across renderer, preload, Electron persistence and browser parity; preserves earlier saved entries; shows a live byte budget and actionable error; and regression-tests later unrelated edits. Two worthwhile P3 hardenings are included: Replay string/filter preload bounds now enforce their documented UTF-8 byte ceilings, and `shared/**` now triggers Linux validation. Donal explicitly authorized focused correction, tests and smoke instead of another four-review restart. Earlier four-review verdicts remain historical evidence and are not represented as exact-head independent review of this successor. Local proof is 296 unit files / 2,535 tests, focused 148/148 after the final UI-state cleanup, Chromium casualty workflow 7/7, targeted visual 1/1 with manual screenshot inspection, Rust 58 passed / one intentional keychain ignore, lint/build/bundle budgets, Electron package, and packaged macOS launch/CDP/render proof. The pushed exact head still requires its Linux check before merge approval. Do not merge or release from an agent task.
