...
- **Latest test counts:** 150 unit files / 946 tests; 117 standard Playwright E2E; 47 backend tests.
...
DON-192 Mission-control, helicopter, and top-panel layout simplification — **DONE locally; ready to commit/push.**
...
- DON-192 closeout: `npm run lint`; `npm run build`; `npm run test` (150 files / 946 tests); `npm run test:backend` (47 passed / 1 ignored); `npm run test:e2e:chromium` (117/117); `npx playwright test tests/e2e/visual/visual-mission-lifecycle.spec.ts --project=visual` (7/7); `npx playwright test tests/e2e/visual/visual-app-shell.spec.ts --project=visual` (5/5); visual review PASS for `mission-minimized-mast-state` and `shell-idle-state`.
