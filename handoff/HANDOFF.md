...
- **Breadcrumb PR-3 final remediation gate (2026-08-26):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and G2/G3 approvals remain binding. Fresh review of `b82298f` found two further lost-evidence P1 classes: a failed one-shot capacity marker could leave the 4,097th rejection unrepresented, and Finish/stop could discard a current-position observation already in flight. The centrally retraced red set failed four exact assertions. Code head `5245504` retains and retries every unresolved loss marker before Finish/finalize/dispose, tracks mission-scoped current-position observations, keeps intake open through the durable Finish transition, closes the mission-evidence cutoff only after success, and makes poller/settings/renderer teardown await the last retrieved safety observation while live positions remain independent after cutoff. Focused lifecycle green is 7 files / 192 tests; full unit is 272 / 2,199; lint, TypeScript, build/budgets, backend 51/51 + one intentional ignore, Chromium 13/13, and selected critical visual plus fresh uncached review 2/2 pass. Exact-head unsigned macOS arm64 packaging passed; exact-head Ubuntu run [`32974681357`](https://github.com/donal0c/sartracker-web/actions/runs/32974681357) passed x64 packaging, native SQLite/llvmpipe inspection, 8,664/8,664 exact persistence with integrity `ok`, one restart, zero renderer crashes, two graceful exits, the packaged soak and real AppImage launch/close. This evidence-binding change is documentation-only; five fresh independent PR-3 reviews from zero still gate review readiness. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and unmerged. The 2026-08-26 review-topology amendment is prospective for PR-4 onward and does not relax PR-3. Do not merge or release from this implementation task.
