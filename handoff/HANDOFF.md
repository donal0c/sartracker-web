...
- **`DON-261` / beta.12.4 field-flash correction in qualification:** the beta.12.3 PCLinuxOS support bundle proves a renderer warning-state flash, not an app crash or packaging split: 109 successful polls toggled the amber history-loading warning on and off 77-99 ms apart while tracking stayed online, storage stayed healthy, and crash history remained empty. Strict TDD now makes the loading warning first-fetch-only per mission history key and keeps durable reconciliation/failure warnings stable between polls. Local proof is unit `1,277/1,277`, Chromium `136/136`, visual `36/36`, independent visual review `41/41`, lint/build, and same-session Fable `PASS_FOR_FULL_QUALIFICATION` with no P1/P2. `electron-v0.1.0-beta.12.4` remains untagged and must not be shared until clean no-skip verification, green tag CI, exact-artifact Ubuntu AppImage/`.deb` smoke, and a >=10-poll packaged field-profile observation pass.
