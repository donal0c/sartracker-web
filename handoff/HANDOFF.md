...
- **Breadcrumb PR-3 evidence-lifecycle remediation in final qualification (2026-08-26):** Donal's Opus review found a real deadline-inversion P1 (the cited 3.704 GB migration trigger was disproved because migration precedes renderer creation). The first coherent fix at `42f1694` made five seconds a soft checkpoint with retractable provisional uncertainty. The bounded exact-`80cc941` recheck then found three related cases, all centrally confirmed. Application head `21a16fec2baadde01be8f79b717a1b8d77592199` now makes `mission_persistence_failed` durable before observation settlement, keeps current fixes visible, makes Finish retry/fail closed, refreshes negative/crash scope, and compensates partial staging. GREEN is focused 9 / 250, full 273 / 2,249, lint/types/CJS/diff/build, Chromium 16/16, and critical visual 2/2 plus uncached Opus 2/2. Exact macOS/Linux packages pass; the Ubuntu soak passes 6/6, 8,664/8,664, one restart, zero crashes, integrity/WAL `ok`, and 12.160 ms main maximum. GitHub run `33002926988` then measured the unchanged 200 ms DON-269 gate at 216.929 ms while test files competed in parallel; the budget remains unchanged and the full workflow is now serial (`--no-file-parallelism`), which passes locally 273 / 2,249 with no skips. Push this evidence/workflow descendant, obtain green CI, then run exactly one fresh broad plus persistence, concurrency, and renderer-containment recheck. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) is **not merge-ready** until those gates are clean; do not merge or release.
