...
- **Active release blocker — `DON-260` / `0.1.0-beta.12.1`:** deep breadcrumb correctness hotfix is locally complete and awaiting exact-artifact qualification. Stable upstream identity/device ownership, audited corrections, deterministic whole-route selection, read-only retryable worker restart reconstruction, mission-wide late-fix reconciliation, strict parsing, five-minute stale semantics, runtime-generation isolation, explicit persistence/cache failure states, and display/fix-time/accuracy UI are implemented under TDD. Current local proof: lint/build; full unit `169 files / 1237 tests`; backend `51 passed / 1 ignored` plus clean formatting/strict Clippy; Chromium `132/132`; visual Playwright `36/36`; fresh uncached independent visual review `41/41`; full no-skip beta gate `8/8` from build `sha.a852dc198832`; packaged CI profile `8,664/8,664`; packaged five-/fourteen-day profiles `691,224/691,224` and `1,935,384/1,935,384`, exact full/prefix digests, one/two restarts, zero redundant-event slope, healthy SQLite/WAL, main maxima `96.3/193.6 ms`, renderer maxima `143.1/198.8 ms`, and individual operator-action maxima `502.8/808.0 ms`; lifecycle/restart/recovery/finalize/archive, bad-secret, and sanitized export smokes passed. Same-session Fable final disposition is `PASS_TO_FULL_RELEASE_GATES` with no P1/P2 finding. Four tag runs stopped safely before release: WebGL blocklisting (`30402564688`), desktop-session throttling (`30426564770`), then two Azure SwiftShader/Vulkan fallback failures (`30427996046`, `30429828590`), all with exact position truth and the latter three with 4/4 healthy trusted interactions. The final red-to-green harness removes SwiftShader permission, provisions/scopes Mesa llvmpipe to Linux validation, keeps ANGLE/OpenGL and foreground-equivalent guards, measures click-to-state without recorder CDP overhead, and independently fails any renderer gap at the unchanged 1,000 ms limit. Three consecutive Ubuntu/Xvfb rehearsals passed at 8,664/8,664 exact, 4/4 healthy, 179.6-188.8 ms main maxima, 133.4-166.7 ms renderer maxima / 16.7 ms median cadence, and 600.9-776.6 ms action maxima, with no Vulkan/SwiftShader errors. Independent adversarial review found no P1/P2. None of the four failed runs created a release or promoted distributable artifacts. Still required: replacement tag-driven CI, exact CI AppImage/`.deb` qualification on Ubuntu, release-note/Linear closeout, and publication. Beta.13 remains `DON-248`-`DON-255`; do not fold map loading or storage/archive redesign into this hotfix. Schema 5 is additive but beta.12 cannot open it, so no in-place downgrade after upgrade.
