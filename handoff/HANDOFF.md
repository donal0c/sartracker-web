...
- **Fable deep-analysis remediation queue:** `DON-230` is the parent Linear issue for `output/fable-deep-analysis.md` (2026-07-06). `DON-231` through `DON-238` are complete locally. `DON-238` locked coordinate semantics: production TM65 uses the EPSG:1641 negative-Y transform, historical S2 spike data is no longer treated as the active oracle, displayed Irish Grid refs truncate to requested precision, coarse Irish Grid refs resolve to the centre of the represented square for converter/Marker At GR actions, IG converter results preserve parsed TM65 at grid-square edges, decimal-comma DD input gets a clear decimal-point error, and impossible live coordinate values render `—` instead of throwing. Verification passed: red-to-green coordinate/converter/marker regressions, `npm run lint`, `npm run build`, `npm run test` (152 files / 1055 tests), `npm run test:backend` (47 passed / 1 ignored), `npm run test:e2e:chromium` (129/129), `npx playwright test --project=visual` (34/34), `npm run visual:review -- --fail-on critical` (39/39; report `test-results/visual-verification/reports/visual-review-2026-07-07T09-15-46Z.json`), `npm run electron:pack`, and in-app Browser validation against `http://127.0.0.1:1420/?missionHarness=1` covering `V 80 84` -> `V 80500 84500`, decimal-comma DD error text, Marker At GR coarse-centre marker form, and invalid live coordinate placeholder `—`. Next recommended queue item is the remaining child `DON-239`.
