...
- **Breadcrumb PR-3 final five-review gate (2026-08-26):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and G2/G3 approvals remain binding. Broad review of `848bfb2` found that fatal main-process exit bypassed the renderer-evidence coordinator. Central retrace confirmed it. The remediation makes fatal relaunch wait for durable mission blockers, refuses relaunch if they cannot be written, and uses an atomic active-session marker so the next startup after fatal exit, SIGKILL, or power loss blocks every unfinalized mission before opening a window. A real SQLite restart regression proves critical `renderer_pending_evidence_lost` health. Focused green is 7 files / 140 tests; final full unit rerun 272 / 2,208; lint, TypeScript, build/budgets, backend 51/51 + one intentional ignore, final Chromium rerun 13/13, selected critical visual 2/2 and fresh uncached review 2/2 pass. Earlier exact package/soak proof is invalidated by the Electron startup change; commit/push and replacement macOS/Linux proof are next, then all five independent PR-3 reviews restart from zero. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and unmerged. The 2026-08-26 review-topology amendment is prospective for PR-4 onward and does not relax PR-3. Do not merge or release from this implementation task.
