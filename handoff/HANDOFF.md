...
- **Breadcrumb PR-3 final remediation gate (2026-08-26):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and both G2/G3 approvals remain binding. The five-review wave on `61c7ed2` was invalidated when the renderer review reproduced an I6 P1: a cooled-down stale-but-rendered open-outing period could be omitted from a later catalog for another pending period, silently removing visible coverage before replacement. It also found that the named completeness unit exercised dead code. Application head `24a5e4d` closes both red-first: positive delivered identities now enter the next two-phase catalog through their current descriptors, and the live controller imports the tested pure progress owner. Full unit is 267 files / 2,140 tests, with full lint, TypeScript, build/budgets, focused plan/progress/controller 84/84, exact Dots 10/10, and participant/coverage Chromium 10/10. Exact application-head Linux run [`32937238855`](https://github.com/donal0c/sartracker-web/actions/runs/32937238855) passed x64 AppImage/`.deb` packaging, AppImage launch, and the 8,664/8,664 source-exact soak with digest `232d4365…`, integrity `ok`, one restart, 28.6847 ms main maximum, zero renderer crashes, and four healthy operator interactions. This preservation/proof remediation does not change the operator manual or screenshots. The next documentation-only descendant requires deterministic exact-head/Linux gates and five fresh independent exact-head reviews from zero before review readiness. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and unmerged. Do not merge or release from this implementation task.
