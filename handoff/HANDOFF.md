...
- **`DON-260` fixed locally — 36-hour breadcrumb regression (2026-08-10):** The root correction remains the continuous bounded history reconciler, current-first wake, active-device pruning, atomic durable frontiers, paced anti-entropy, geometry-aware display selection, bounded memory/query work, and lifecycle-safe worker cancellation. Eamonn's added beta.12.5 evidence showed slow-speed clusters and much larger gaps at 120–145 km/h, so the release proof now includes one deterministic vehicle moving at 12 km/h and then 120–145 km/h plus exact packaged dot-mode comparison. The current selector retains `720/720` fixes in each phase and adds zero gap inflation; the packaged 33-device fault/SIGKILL/restart proof passed with current fix `99 ms`, first breadcrumb `345 ms`, exact `279,936`-row SQLite truth in `37.148 s`, exact `103,614` line coordinates after restart, and exact `103,614` dot coordinates against the independent oracle. The unchanged 14-day soak passed `1,935,384` exact rows, two restarts, integrity/WAL/responsiveness, zero crashes, and peak RSS `1,533,214,720` bytes (`614,268,928` below 2 GiB). Automated beta gates are green (`178` unit files / `1,391` tests, backend `51 / 1 ignored`, Chromium `142/142`, package, CI soak); visual Playwright `37/37` and independent review `43/43` pass. A Vite dynamic-import race found during the first Chromium run was reproduced and replaced with the installed harness boundary (`10/10` repeated; full rerun green). `DON-260` remains **In Progress** pending commit/push and exact CI-built AppImage/`.deb` qualification; do not publish local artifacts.
...
- **Latest published team beta:** [`electron-v0.1.0-beta.12.5`](https://github.com/donal0c/sartracker-web/releases/tag/electron-v0.1.0-beta.12.5), published 2026-07-30 after full qualification and a fresh-public-byte Ubuntu lifecycle smoke. Original-machine confirmation remains `DON-247`.
