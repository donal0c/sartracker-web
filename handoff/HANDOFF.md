...
**B7 follow-through** (`DON-24`). The release workflow now has live dependency preflight plus Linux/Windows launch-smoke jobs. Run `26041435928` passed against existing `v0.1.0-beta.3` assets with the black-map detector in place and showed OpenTopoMap visible in CI. Team feedback isolates the remaining field problem to tester Linux machines: tracking works on `:8082`, but the map is black. Next action: ask testers to try ESRI World Topo and OpenStreetMap from the Maps menu, export Diagnostics, and capture whether only OpenTopoMap is blocked or all tile providers are black. Also collect distro/GPU/session and one launch with `WEBKIT_DISABLE_DMABUF_RENDERER=1`. Real Linux machine smoke is still required before sending artifacts wider. If smoke fails, cut `v0.1.0-beta.4` per the immutability rule.
