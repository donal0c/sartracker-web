...
- **Fable deep-analysis remediation queue:** `DON-230` is the parent Linear issue for `output/fable-deep-analysis.md` (2026-07-06). `DON-231` through `DON-237` are complete locally. `DON-235` reduced tracking overlay/runtime churn: hidden Tracking now reuses a stable empty snapshot, unchanged tracking source data skips lazy FeatureCollection construction, breadcrumb health annotation preserves array identity, current-position health annotation preserves identity when unchanged, polling publishes one settled snapshot per healthy poll, cache writes skip identical snapshot identities, breadcrumb source retention is bounded while observed totals remain cumulative, and hover coordinates are requestAnimationFrame-throttled. Visual validation also tightened tracking label halo and fixed a small-display coordinate-strip overlap found by the final reviewer. Verification passed: red-to-green tracking/map/runtime regressions, `npm run lint`, `npm run build`, `npm run test` (152 files / 1047 tests), `npm run test:backend` (47 passed / 1 ignored), `npm run test:e2e` (161/162 with one external `met.ie` popup navigation flake; targeted retry passed), `npx playwright test --project=visual` (34/34), `npm run visual:review -- --fail-on critical` (39/39; report `test-results/visual-verification/reports/visual-review-2026-07-06T20-57-32Z.json`), `npm run electron:pack`, and in-app Browser validation against `http://127.0.0.1:1420/?missionHarness=1` showing 8 devices / 168 positions, Tracking hidden, one intentional tracking source clear, zero additional `setData` calls across eight forced idle events, zero hidden tracking source features, and strong tracking label halo paint. Official offline smoke was not applicable because no private MBTiles package is present. Next recommended queue item is the remaining deep-analysis children `DON-238`/`DON-239`.
