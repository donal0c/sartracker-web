...
- **PR5 current remediation (2026-08-29):** persistence/completeness and concurrency/finalization are CLEAN on exact head `c1ad54b8`. Renderer rejected later candidate `018c0dd3` because a still-live Pause/Resume rerender replaced an unsubmitted historical Replay time with `now`; strict retrace retained P2 because the production path was deterministic, PR5-owned and could silently submit the wrong evidence time. Broad review on that invalid head was stopped with no verdict or material partial finding, and Linux run `33238597041` was canceled. Executable fix `eb5310772019` gives the initial live Replay draft a stable per-mission owner, promotes the exact submitted time while Replay is loading, preserves accepted scope after remount, and rebinds only on accepted scope/explicit Return to Live/mission change. It also fixes the renderer's non-blocking P3 by preserving unapplied Search text through loading/paging. Red-first component and actual docked Pause/Resume/submit regressions are green. Local proof: focused renderer/replay 18 files / 121 tests, full unit 296 / 2,530, backend 58/one intentional ignore, Chromium 168/168 after one unrelated navigation-context flake passed alone and on complete rerun, visual 2/2 plus fresh uncached critical review 4/4, lint/build/diff, unsigned arm64 package `sha.97c3daf1f7b0`, and a 15.123 s two-launch soak with 6/6 batches, exact 8,664/8,664 positions/digest, integrity `ok`, zero redundant slope/crashes, four healthy interactions and 5.322 ms maximum main round trip. Exact-head Linux plus fresh broad and affected renderer rechecks remain; unaffected persistence/concurrency clean verdicts do not restart. The 48-bit archive-name collision remains speculative P3 and the Tauri mutex note remains qualification-only. PR opened/review-ready is intermediate; do not merge or release.
