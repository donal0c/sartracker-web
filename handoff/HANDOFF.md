...
- **Fable deep-analysis remediation queue:** `DON-230` is the parent Linear issue for `output/fable-deep-analysis.md` (2026-07-06). `DON-231` is complete locally: Electron now accumulates paused time on resume/finish and preserves marker/drawing/GPX create/import timestamps across edits. Verification passed: red-to-green Electron mission-store regressions, focused mission/review units, full `npm run test`, `npm run lint`, `npm run build`, `npm run test:backend`, full Chromium browser E2E `128/128`, and `npm run electron:pack`. Next recommended queue item is `DON-232` (SQLite backup/archive durability), then `DON-233` (Traccar breadcrumb cursor resilience). Speed-unit math still needs a controlled server/unit decision; stale-device threshold is a product decision because code/tests use 60 min while repo/S7 docs say >5 min.
