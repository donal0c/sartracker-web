...
- **`DON-260` beta.12.8 release candidate — 36-hour breadcrumb regression (2026-08-10):** The correction at `98b1b849c0a1` separates current fixes from continuous bounded catch-up, checkpoints durable frontiers, preserves route geometry, bounds renderer/query memory, and cancels stale work. Ubuntu rejected beta.12.6 at the unchanged 60-second proof because every two-hour/device chunk still committed separately (`268,275/279,936` rows at expiry). `4896ecd` now persists each up-to-eight-chunk wave and its checkpoints atomically, with fetch/batch failure isolation and publication-after-durability regression-locked; local exact proof improved to `28.119 s`. Beta.12.7 then failed its first CI gate only because the 198,000-position worst-case GeoJSON test took `5.627 s` against Vitest's generic five-second timeout; assertions were sound, focused/full local reruns passed, and `21ba720` assigns that intentionally large test an explicit 15-second budget. Beta.12.8 must pass a fresh no-skip gate, new immutable tag CI, and the exact CI AppImage/real `.deb` Ubuntu 36-hour, long-soak, lifecycle, diagnostics, safety, and live-provider matrix before publication. Beta.12.6/.12.7 remain unpublished abandoned candidates.
