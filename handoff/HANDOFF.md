...
- **`DON-267` field-feedback bridge is open for review in [PR #4](https://github.com/donal0c/sartracker-web/pull/4) (2026-08-27):** `SAR-QA-021` locks Traccar position `fixTime` as the only breadcrumb evidence clock. Current locations using device/server time stay visible only as explicitly time-unverified and are filtered before exact history or mission persistence. Exact fix times render as local operator time with timezone/offset while stored UTC remains unchanged. Live-current polling is separated from single-flight/coalesced history; regressions prove a second request/publication after 5 seconds with no history overlap and prevent older history completion from overwriting a newer offline status. The 37,479 exact-fix contract traverses 10,000 + 10,000 + 10,000 + 7,479; progressive PR-3 coverage remains the default all-history view. Green: lint, TypeScript/build/bundle budgets, unit 275 files / 2,275 tests, legacy backend 51 passed / 1 ignored, Chromium 159/159, and a headed browser inspection of `2026-08-22T15:10:17.000Z` as `22/08/2026, 16:10:17 GMT+01:00 (Europe/Dublin)` with zero console warnings/errors. No package/Ubuntu/live-server/field proof was run. Pre-bridge SQLite rows do not retain source-clock provenance, so this change cannot retrospectively classify whether any used a fallback; no such field data loss or substitution is asserted without evidence. Do not merge or release this PR from the implementation task.
