...
- **Breadcrumb PR-3 final remediation gate (2026-08-26):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and G2/G3 approvals remain binding. Application head `b383ef1` retains the worker-content, filter-growth, real-production-worker, participant, claim-drift, and Electron-startup fences, and closes the last confirmed finalization race red-first: delayed startup rejection and delayed capacity-loss settlement for a finalized mission stay outside live health until explicit unlock. Full unit is 269 files / 2,160 tests; lint, TypeScript, build/budgets, participant/coverage Chromium 10/10, selected critical visual plus fresh uncached review 2/2, and backend 51/51 with one intentional OS-keychain ignore pass. Exact application-head Linux run [`32951751651`](https://github.com/donal0c/sartracker-web/actions/runs/32951751651) passed x64 AppImage/`.deb` packaging, AppImage launch, and the 8,664/8,664 source-exact soak: digest `5018747b…`, integrity/WAL `ok`, one restart, zero renderer crashes, four healthy interactions, zero redundant slope, and 40.935 ms Electron-main maximum. No operator-visible surface changed; the manual/screenshots remain accurate. Bind this documentation descendant, repeat its exact-head deterministic/Linux gates, and restart five independent reviews from zero. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and unmerged. The 2026-08-26 five-review topology amendment is recorded prospectively for PR-4 onward and does not relax PR-3. Do not merge or release from this implementation task.
