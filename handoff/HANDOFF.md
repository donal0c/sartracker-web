...
- **Branch:** breadcrumb PR-1 through PR-4 are merged on `master`. PR5 is open as [GitHub PR #5](https://github.com/donal0c/sartracker-web/pull/5) on `codex/breadcrumb-pr5-evidence-replay` from exact merged PR4 base `80309c995a18eeb190cce4310c9a46b0f46d5263` and prepared coordination head `c7a5f4fa1668088ed8038fd53bf7061d92db663b`. It remains unmerged and unreleased. PR opened/review-ready is an intermediate state, not task completion.
...
- **PR5 pasted deep review invalidated the earlier completion candidate (2026-08-28):** source retrace confirmed destructive legacy-GPX baseline compaction, same-hash projection/revision divergence, post-T device-filter leakage, unbounded shutdown join, migrated MultiPolygon retirement failure, superseded-page failure publication and browser/Tauri conformance gaps. The replacement retains original legacy geometry, bounds startup reconstruction with explicit rejection evidence, makes exact-hash retries strict, time-fences filter identities, provides an explicit Dublin overlap occurrence, latest-wins page cancellation, production-compatible harness cursors, cross-mission GPX identity fences and Electron-only operational Replay. Local proof is green: 289 files/2,416 unit; backend 52/1 intentional keychain ignore; lint/build/budgets; Chromium 164/164; visual 58/58; uncached independent visual 69/69; indexed and authentic-v11 960k qualifications. The unsigned macOS arm64 package and packaged soak passed. Exact-head Linux run `33149715590` passed the serial 2,416-test gate, build/budgets, x64 AppImage/`.deb`, 960k replay (234.30 ms seek, 80.27 ms current-write maximum, exact restart), packaged soak (6/6, 8,664/8,664, 21.80 ms main maximum), llvmpipe and graceful visible AppImage launch/close. A fresh broad plus persistence/completeness, concurrency/finalization and renderer/input-containment review remains mandatory on the final code-and-documentation head. PR #5 remains open, unmerged and unreleased; PR opened/review-ready is intermediate, not completion.
- **PR5 pushed head `74d0335e` is rejected (2026-08-28):** `6642e383` had already been rejected because the settled legacy scan reached 291.82 ms at 500,000 rows and a pre-classification same-ID/path retry destroyed a 262,201-byte legacy original. `74d0335e` replaced the scan with a durable rowid cursor and fenced all revisionless mutation aliases; full local/browser/visual/960k and macOS packaged-soak proof passed. Persistence then measured a 1,052.48 ms event-loop gap from per-row cursor commits; broad proved negative rowids could silently escape custody and values above 2^53-1 could remain permanently pending; concurrency measured a fully revisioned 20,000-row inventory falsely fencing Finish for 1,728.743 ms. Red regressions reproduced 1,049.55 ms and both signed-rowid failures. The replacement scans one cheap 10,000-row safe-key page per turn with one cursor write, while a SQLite-only signed-int64 scan explicitly quarantines rowids outside 1..2^53-1. Exact boundary and 500,000-row liveness/heartbeat regressions are green. Fresh proof is serial unit 289/2,425, backend 55/1 intentional ignore, syntax/lint/build/budgets, Chromium 165/165, visual 58/58, uncached review 69/69 (`visual-review-2026-08-28T09-39-22Z.json`), indexed 960k (84.15 ms seek, 64.68 ms current write, 68.82 ms event-loop, exact restart) and authentic-v11 960k (738.03 ms seek, 3.78 ms current write, explicit fallback). Commit/push, package/Linux proof and all four independent exact-head reviews remain mandatory. Do not merge or release.
...
- **Breadcrumb coordination workflow:** PR5 remains one coherent review unit for BCP-11/12a/12b/13. The implementation task owns the broad exact-head review plus focused persistence/completeness, concurrency/finalization and renderer/input-containment reviews, source retrace, remediation and clean affected rechecks. Donal retains final approval/merge authority and may ask for more review; the allocated baseline is not deferred to him. Do not merge or release from the implementation task.
