...
- **Breadcrumb PR-3 evidence-lifecycle remediation in final qualification (2026-08-26):** Donal's Opus review found a real deadline-inversion P1 (the cited 3.704 GB migration trigger was disproved because migration precedes renderer creation). The first coherent fix at `42f1694` made five seconds a soft checkpoint with retractable provisional uncertainty. The bounded exact-`80cc941` recheck then found three related cases, all centrally confirmed: accepted mission writes could settle after persistence failure, negative/crash promotion reused a stale mission-scope snapshot, and partial provisional staging lacked compensation. The current uncommitted red-first repair makes `mission_persistence_failed` durable before observation settlement, keeps current fixes visible, makes Finish retry/fail closed, refreshes scope at promotion, and retracts every earlier staged scope after a later staging failure. Local GREEN is focused 9 files / 250 tests, full 273 / 2,249, lint, TypeScript, CJS syntax, diff, build/budgets, Chromium 16/16, and selected critical visual 2/2 with two fresh uncached Opus PASS reviews. Commit/push, repeat exact-head macOS/Linux package plus the single Ubuntu CI-scale soak because runtime code changed, obtain green expanded CI, then run exactly one fresh broad plus persistence, concurrency, and renderer-containment recheck. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) is **not merge-ready** until those gates are clean; do not merge or release.
