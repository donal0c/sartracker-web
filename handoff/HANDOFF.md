...
- **PR5 current review candidate (2026-08-29):** exact pushed head `782167e8` passed Linux but all four task-owned reviewers rejected it. Central retrace accepted four unique P2s: authentic-v11 event-index creation still blocked startup; event reconstruction turns were row-bounded but not byte-bounded on SQLite's writer; archive/finalize omitted the legacy-event readiness fence; and marker retirement/browser validation retained unbounded or divergent mutation paths. Executable fix `d98d7907` keeps the event replay index off legacy synchronous startup with an explicit scan limitation, bounds worker turns to 512 KiB and quarantines a retained event above 256 KiB, applies pending/failed/quarantined event fences to every archive/finalize phase, and closes marker/drawing upsert/retirement bounds across preload, main, harness and operator inputs. Red regressions reproduced every defect. Local proof is green through serial unit 295 files / 2,492 tests, Chromium 167/167, visual 58/58 plus uncached review 69/69, backend 57/one intentional ignore, lint/build/CommonJS/Rust, indexed 960k, unsigned macOS arm64 package and exact 8,664-position two-launch soak. Exact branch-head Linux run [`33222361222`](https://github.com/donal0c/sartracker-web/actions/runs/33222361222) is green on `1909eac0`: 2,492/2,492, Linux AppImage/`.deb`, native x86-64 SQLite, 960k qualification/restart equality, packaged 8,664/8,664 soak and graceful AppImage close. The final code-and-documentation head now requires all four restarted exact-head reviews clean. PR opened/review-ready is intermediate; do not merge or release.
