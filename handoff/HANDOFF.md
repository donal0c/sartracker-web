...
- **Fable deep-analysis remediation queue:** `DON-230` is the parent Linear issue for `output/fable-deep-analysis.md` (2026-07-06). `DON-231` is complete and `DON-232` is complete locally. DON-232 hardens Electron SQLite durability and archive safety: WAL connections now use `synchronous=FULL`; newer schema versions are refused instead of downgraded; rolling backup mirrors write via validated temp+rename with WAL/SHM cleanup; archive snapshots are SQLite-integrity checked before zip commit; finalize/archive work is serialized and idempotent; stale pre-unlock archive recovery is ignored; backup sync events are hidden from the default Review audit feed; layer-catalog metadata upserts now emit audit events. Verification passed: red-to-green Electron mission-store regressions, `npm run lint`, `npm run build`, full `npm run test` (152 files / 1016 tests), `npm run test:backend` (47 passed / 1 ignored), full `npm run test:e2e` (162/162), `npm run visual:review -- --fail-on critical` (overall PASS, 0 critical blockers; 1 existing high nonblocking basemap prompt mismatch), `npm run electron:pack`, packaged Electron mission-store smoke, and in-app Browser validation against the running dev server. Next recommended queue item is `DON-233` (Traccar breadcrumb cursor resilience). Speed-unit math still needs a controlled server/unit decision; stale-device threshold is a product decision because code/tests use 60 min while repo/S7 docs say >5 min.
