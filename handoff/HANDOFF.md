...
- **PR5 mission evidence and replay is in accepted-finding remediation:** heads `f87d75873f11d12d249e3afbc482703b25f99ff4`, `1b35786c5de35355724928354789d779a52d186c` and `77be02aec37da6e0e032e3921d4bd333b7e20d3f` were rejected by the fresh risk reviews. The first leaked staged GPX knowledge time, separated publication from receipt settlement, allowed stale unlock authorization to reopen a newer finalization, and gave impossible Search Operations recovery copy. The second could reconcile newly retained source against a matching legacy baseline without exact bytes, then clear the only exact receipt. The third passed broad and persistence review, but renderer review proved that browser validation accepted future replay times and used current rather than known-at-T GPX outing assignments. Every accepted finding was reproduced red. The working remediation now requires an active current complete GPX revision with backend-verified retained bytes for restart settlement, and browser replay now rejects future T, scopes GPX to the selected mission and derives outing filters from the eligible revision. Full/browser/package/scale/Linux verification and clean broad, persistence, concurrency/finalization and renderer/input-containment reviews must pass on the replacement head. The final handoff must name every reviewer/task, SHA, verdict, finding disposition and recheck. Do not merge or release.
