...
- **PR5 pasted deep review invalidated the earlier completion candidate (2026-08-28):** source retrace confirmed destructive legacy-GPX baseline compaction, same-hash projection/revision divergence, post-T device-filter leakage, unbounded shutdown join, migrated MultiPolygon retirement failure, superseded-page failure publication and browser/Tauri conformance gaps. The replacement candidate retains original legacy geometry, bounds startup reconstruction with explicit rejection evidence, makes exact-hash retries strict, time-fences filter identities, provides an explicit Dublin overlap occurrence, latest-wins page cancellation, production-compatible harness cursors, cross-mission GPX identity fences and Electron-only operational Replay. Focused and full local proof is green: 289 files/2,416 unit; backend 52/1 intentional keychain ignore; lint/build/budgets; Chromium 164/164; visual 58/58; uncached independent visual 69/69. Fresh 960k qualification passed (62.28 ms seek, 49.96 ms late page, 39.74 ms maximum current write, 55.91 ms restart equality). The explicit authentic-v11 960k fallback passed (13.43 ms open, 601.72 ms seek, 3.59 ms concurrent current write, 584.00 ms restart, limitation visible, no large startup indexes). Replacement code is committed; the unsigned macOS arm64 package built successfully and its CI packaged soak passed 6/6 batches, 8,664/8,664 exact positions, one restart, four healthy interactions, 9.74 ms main maximum, zero renderer crashes and zero redundant slope. Push, exact-head Linux proof and a fresh broad plus all three focused reviews remain. PR #5 remains open, unmerged and unreleased; PR opened/review-ready is intermediate, not completion.
