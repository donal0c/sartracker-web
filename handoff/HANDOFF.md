...
- **Breadcrumb PR-2 exact-head review remediation (2026-08-24):** code commit `ecdd86f` closes the tenth-pass React-scope P2. The outing view model now exposes data, summaries, errors, saving state, and mutation authority only when the runtime mission exactly matches the UI mission; a mismatch is empty/loading and read-only. Mission-keyed component state immediately suppresses and clears an old mission's edit draft, including after the new mission hydrates. Both hook and real component regressions failed red first; focused outing tests pass `4 files / 16`, full serial unit `230 / 1,882`, lint/build/bundle budgets, diff checks, and outings+participants Chromium `8/8`. No fresh full Chromium, packaged-runtime, heavy-scale, Ubuntu/Windows, or field-hardware proof was run because the change is confined to the renderer outing boundary; standing proofs bind only unchanged surfaces. Await an independent exact-current-head approvable comment; do not merge or start PR-3.
