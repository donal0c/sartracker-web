...
- **`DON-260` exact Breadcrumb Dots correction — beta.12.11 candidate (2026-08-11):** Team beta.12.9 feedback proved Dots still rendered the bounded Line projection: a steady 5-second trail became adjacent pairs/groups separated by roughly 60 seconds. The replacement gives Dots its own mission-start-bounded exact SQLite page query and dedicated MapLibre source; <=10,000 fixes display automatically and larger missions use exact chronological Earlier/Later pages. Line remains bounded and owns simplification/error-bound language. The immutable beta.12.10 CI AppImage passed the Ubuntu 36-hour 279,936-fix/28-page proof, but the Ubuntu fourteen-day run correctly rejected its 81.096 s outward traversal against the unchanged 60 s gate. Root cause was a full 1,935,384-row positions `GROUP BY` on every all-device page solely to resolve the roster. Beta.12.11 reads the indexed mission `devices` roster instead; the same preserved database now traverses all 194 exact pages in 22.335 s versus 46.406 s, with the identical union. A clean exact-commit verifier then caught a genuine diagnostics privacy gap: non-home app profile paths could survive export sanitization. Final-write redaction now removes raw, normalized, and JSON-escaped exact profile paths while preserving the internal save location. Combined verification is `195 files / 1,559 tests`, TypeScript/lint/build/Node/diff green; full Playwright remains `179/179` and visual review `43/43`. The first clean beta.12.11 verify run is retained as a valid privacy red; rerun is required after the fix. Next: rebuild/tag beta.12.11, then repeat Ubuntu AppImage/.deb 36h/14d, package-safety, and private live-provider proof before publication. **Beta.12.10 is rejected; do not hand over beta.12.9 or beta.12.10 as the correction.**
...
- **Latest published team beta:** [`electron-v0.1.0-beta.12.9`](https://github.com/donal0c/sartracker-web/releases/tag/electron-v0.1.0-beta.12.9) remains the latest public prerelease, but its Breadcrumb Dots are not source-exact and it is superseded for this incident. It must not be represented as the corrected Dots build. Beta.12.10 stays private/draft until the full Ubuntu package and live-provider matrix passes.
