...
- **PR5 current remediation (2026-08-29):** all verdicts on invalid head `09475762` are superseded: broad and concurrency were clean, but persistence found same-name Electron attachment overwrite plus Tauri deletion of version-linked bytes, and renderer found that a failed Review refresh hid its retained snapshot. Central retrace accepted both P2s. Executable fix `b2d7cbe9abeb` gives every Electron attachment a unique retained identity, retains Tauri-managed bytes across marker replacement/deletion, keeps the stale snapshot and bounded Search/First/Next visible after a Review failure, and disables evidence entry until Refresh succeeds. The regressions were observed red and are green in the actual workspace/filesystem/persistence boundaries. Local proof: focused evidence/replay/persistence 20 files / 202 tests, full unit 296 / 2,523, backend 57/one intentional ignore, Chromium 167/167, visual 2/2 plus fresh uncached critical review 4/4, lint/build/diff, unsigned macOS arm64 package `sha.b2d7cbe9abeb`, and a 13.134 s two-launch packaged soak with 6/6 batches, exact 8,664/8,664 positions/digest, integrity `ok`, zero redundant slope, zero crashes, four healthy interactions and 4.109 ms maximum main round trip. Exact-head Linux and all four independent reviews now restart on one bound code-and-documentation candidate. PR opened/review-ready remains intermediate; do not merge or release.
