...
- **Breadcrumb PR-3 final remediation gate (2026-08-26):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and both G2/G3 approvals remain binding. External and renewed review invalidated every earlier clean verdict with false freshness/classification, worker/query trust-boundary, tile-cache, catalog recovery, retained-source attestation, multi-period recovery, and progressive mission-switch defects. Application head `ffffe0f` closes them red-first. The controller now waits only for the irreversible catalog finalization before a newer mission/filter/retry context aborts the remaining old progressive load, so an unacknowledged old-mission batch cannot deadlock the switch. Focused green is 5 files / 83 tests; full unit is 264 files / 2,127 tests, plus lint, TypeScript, build/budgets, and participant/coverage Chromium 10/10. Exact Linux run [`32920238110`](https://github.com/donal0c/sartracker-web/actions/runs/32920238110) passed x64 AppImage/`.deb` packaging, AppImage launch, and the 8,664/8,664 packaged soak with integrity `ok`, one restart, 32.603 ms main maximum, zero redundant slope, and four healthy operator interactions. This record's documentation-only descendant still requires its own deterministic gates and five fresh independent exact-head reviews before review readiness. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and unmerged. Do not merge or release from this implementation task.
