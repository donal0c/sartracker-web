...
- **Breadcrumb PR-3 G3 approved; final flip/review next (2026-08-25):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`. G2's 18-run Ubuntu matrix at measured SHA `8eff87b...` selected Candidate B; Donal ratified B and the unchanged 1.5 GiB/2.5 GiB budgets. BCP-08 schema v10, ledger/invalidation worker, progressive claims/controller, Candidate-B vector tiles, bounded diagnostics, and BCP-09's complete-history operator view/manual are implemented through tested code head `d1e884b`. T1 passed 257 files / 1,983 unit tests, lint, TypeScript, CJS syntax, exact Dots 10/10, build/budgets, Chromium 157/157, six critical visual reviews, macOS/Linux packaging, a 16.557 ms integrity-`ok` 3.704 GB v9→v10 migration retaining 2.04M positions, and the packaged Ubuntu CI soak with 8,664/8,664 exact fixes, restart, zero redundant slope, 5.586 ms main / 66.9 ms renderer maxima, 1.004 GB peak RSS, and a 20 KiB coverage-ledger line item. Durable binding: `docs/breadcrumb-pr3-t1-evidence.md`. Donal approved G3 at exact pre-flip head `d05c787`; coverage and the required mission model may now flip on together in one final two-boolean commit. Next: final exact-head gates and five independent reviews; any remediation creates a new head and restarts all five reviews.
...
- **Breadcrumb coordination workflow:** one implementation task owns the complete PR-3 through BCP-07 → BCP-08 → BCP-09. G2 and G3 are ratified and T1 is green; only the final flip, exact-head gates, and five reviews remain before the PR opens.
