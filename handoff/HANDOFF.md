...
- **Breadcrumb PR-3 final remediation gate (2026-08-26):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and both G2/G3 approvals remain binding. Review invalidated evidence head `48ee118` with two recovery P2s. Application head `635a7b3` closes both red-first: a replacement tile worker safely settles predecessor finalization from a lost generation only when it owns no live stage, while malformed staged results terminate their poisoned worker and raise the existing failure signal so Retry performs a fresh rebuild. Full unit is 264 files / 2,135 tests, plus lint, TypeScript, build/budgets, exact Dots 10/10, CommonJS syntax, and participant/coverage Chromium 10/10. Exact Linux run [`32924343484`](https://github.com/donal0c/sartracker-web/actions/runs/32924343484) passed x64 AppImage/`.deb` packaging, AppImage launch, and the 8,664/8,664 packaged soak with digest `e26da482…`, integrity `ok`, one restart, 21.185 ms main maximum, zero redundant slope, and four healthy operator interactions. The next documentation-only descendant requires deterministic exact-head gates and five fresh independent exact-head reviews from zero before review readiness. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and unmerged. Do not merge or release from this implementation task.
