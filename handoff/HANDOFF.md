...
- **Breadcrumb PR-3 final remediation gate (2026-08-26):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and G2/G3 approvals remain binding. The fresh review of evidence-bound head `dcc950c` found a further P1: if post-Finish renderer persistence and teardown both failed, main queried only `active/paused`, treated the finished evidence scope as absent, and could permit destructive teardown. Central source retrace confirmed the full chain. The composed RED failed with `no_active_mission` and a missing store boundary. The remediation gives main one bounded mission-table query for every unfinalized evidence scope (`active`, `paused`, `finished`) and durably marks all of them before unacknowledged close/reload/crash may proceed; finalized missions remain excluded. Composed lifecycle green is 9 files / 163 tests; full unit is 272 / 2,202; lint, TypeScript, build/budgets, backend 51/51 + one intentional ignore, Chromium 13/13, selected critical visual 2/2 and fresh uncached review 2/2 pass. The manual is current and layout unchanged. The exact `5245504` macOS/Linux package evidence is invalidated as application-code proof by this main-process change, though its unchanged harness evidence remains useful. Commit/push, repeat exact macOS/Linux package gates, bind the evidence, and restart all five independent PR-3 reviews from zero. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and unmerged. The 2026-08-26 review-topology amendment is prospective for PR-4 onward and does not relax PR-3. Do not merge or release from this implementation task.
