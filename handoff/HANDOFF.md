...
- **Active release blocker — `DON-260` / `0.1.0-beta.12.3`:** the deep breadcrumb identity/order/correction/restart/late-fix/lifecycle work and Devices-workspace latency fix are complete. Beta.12.2 passed exact-head local verification and tag run `30445518186`, but its unpublished exact draft failed the mandatory Ubuntu newer-schema gate: the database remained byte-identical, yet startup entered an unhandled-rejection/relaunch loop behind a native dialog. The draft is marked **ABANDONED — DO NOT PUBLISH** and its tag remains immutable. A red-to-green correction now records a sanitized `startupFailure`, shows the explicit schema compatibility boundary, exits after acknowledgement, and never relaunches into the persistent fault. The smoke requires the native dialog, exit 1, no renderer process across continuous Linux process-tree sampling, no unhandled rejection, and full hashes of every mission-store/backup/WAL/SHM/temp file before and after. Focused safety proof, lint, and build are green; full proof is `171 files / 1,274` unit tests, Chromium `135/135`, visual `36/36`, fresh independent visual review `41/41`, and backend `51 passed / 1 ignored`. Fable's same-session initial and post-delta reviews found no P1/P2. A locally rebuilt pre-tag Ubuntu AppImage passed the corrected gate with 50 renderer-process samples at zero and byte-identical database/backup hashes. Beta.12.3 exact-head no-skip verification, tag CI, and the complete exact-artifact Ubuntu matrix remain release-blocking. Full Discovery loading and beta.13 storage/archive work remain out of scope.
