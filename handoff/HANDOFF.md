...
- **`DON-261` / `DON-262` / `DON-263` beta.12.4 in qualification:** commit `b1e21c22c323` is pushed. The beta.12.3 PCLinuxOS bundle proves a warning-state flash, not a crash: 109 successful polls toggled amber history loading 77-99 ms apart while tracking/storage remained healthy. Strict TDD makes loading first-fetch-only, fixes Open Devices in place, fails the packaged soak closed on input-sequence defects, and synchronizes operational overlays from style structure without waiting for basemap tiles or continuously re-entering `styledata`; retry is unlimited with a 2 s cap. Final local proof: unit `1,287/1,287`; Chromium `142/142`; visual `37/37`; uncached independent review `43/43`; backend `51 / 1 ignored`; lint/type/syntax/diff/build; Fable `PASS_FOR_COMMIT` twice, no P1/P2. Exact-commit Ubuntu proof: ten unpacked soaks stored 86,640/86,640 positions with zero identity loss/redundant rows/crashes, 40/40 audited interactions, zero audit/tail errors, fixed Open Devices Y=`615`, main max 20.4 ms, and external action max 180.7 ms; the AppImage and extracted `.deb` payload each passed another 8,664-position restart soak; AppImage corrupt-secret startup passed. A packaged active-tracking OpenStreetMap→OpenTopoMap switch held 40 raster requests while `isStyleLoaded() === false`: two current markers and two trails rendered, trails continued growing, then survived the explicit degraded-map state. A clean Ubuntu beta wrapper passed lint/build/unit but failed closed at legacy Tauri compilation because system DBus/GTK/WebKit development packages are absent; actual `.deb` installation also needs sudo. Fable's final exact-head audit returned `PASS_BLOCKED_ON_RELEASE_GATES`, with no P1/P2 and the current evidence wording judged honest. Persistent overlay-sync failures are still console-only; non-blocking diagnostics/map-health hardening is tracked explicitly in `DON-264`. `electron-v0.1.0-beta.12.4` remains untagged/private until one clean no-skip wrapper run, green tag CI, and exact CI AppImage/installed-`.deb` smoke complete.
...
- **DON-259 breadcrumb field correction complete locally:** Eamonn's 2026-07-20 screenshots led to a red regression proving dot-mode GeoJSON dropped closely spaced retained fixes under the old `DON-126` 40 m default decimation. Dot mode now renders one dot per position in the bounded live snapshot while keeping complete accepted mission truth in SQLite and existing 5,000-position-per-device budgets/diagnostics. Verification: focused red-to-green unit/Chromium; lint; build/bundle budgets; full unit `164 files / 1148 tests`; backend `47 passed / 1 ignored`; serialized Chromium `132/132`; visual `35/35`; independent critical visual review `40/40`; packaged Electron CI soak 8,664 exact positions with 4/4 real operator interactions.
...
- **Latest published team beta:** [`electron-v0.1.0-beta.12.3`](https://github.com/donal0c/sartracker-web/releases/tag/electron-v0.1.0-beta.12.3), published 2026-07-29 for internal field retest. Compare source position IDs, coordinates, and fix times across the original Linux machines after reconciliation completes; original-machine confirmation remains `DON-247`.
