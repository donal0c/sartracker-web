...
- **Breadcrumb PR-3 frozen review-head preparation (2026-08-25):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and both G2/G3 approvals remain binding. Five third-review lifecycle/evidence blockers are fixed red-first at `38ec709`: renderer stages are sender-owned and discarded on renderer loss, obsolete mission activations cannot cross generations, cancellation leaves retained coverage serviceable, renderer-held evidence blocks Complete, and mission ACKs cannot clear another mission's warning. Exact-production Ubuntu qualification passes 959,988/959,988 at 2.244 s first-useful / 5.794 s complete and 1,999,988/1,999,988 at 4.497 s / 12.126 s, with 20.197/21.616 ms main gaps and only `backfill_incomplete`. The single replacement packaged soak passes 8,664/8,664, restart, integrity/WAL, llvmpipe attestation, 15.525 ms main / 83.6 ms renderer maxima, 1.105 GB peak RSS, and zero redundant slope. G2 rows and the 3.704 GB v9→v10 migration remain standing because renderer algorithms and schema/open paths are unchanged. Bindings: `docs/breadcrumb-coverage-renderer-decision.md`, `docs/breadcrumb-pr3-t1-evidence.md`, and matching `output/` directories. Next: commit the evidence binding, run complete exact-head deterministic/Chromium/visual/package gates, then restart all five independent reviews from scratch.
