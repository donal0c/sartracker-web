...
- **Breadcrumb PR-3 final five-review gate (2026-08-26):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and G2/G3 approvals remain binding. Broad review of `1f34299` found that Finish could mark a mission `finished` before an already-started valid current fix persisted. Central retrace confirmed the ordering. Code head `9253d89` closes new mission-evidence admission, waits and drains every pre-cutoff accepted/rejected observation while the mission is writable, then commits Finish; post-cutoff safety positions remain live but do not enter mission evidence, and a failed Finish reopens admission. A real SQLite regression proves the pre-cutoff fix and its one-fix exact coverage claim survive. Focused green is 6 files / 170 tests; full unit 272 / 2,204; lint, TypeScript, build/budgets, backend 51/51 + one intentional ignore, Chromium 13/13, selected critical visual 2/2 and fresh uncached review 2/2 pass. Exact-head unsigned macOS arm64 packaging passed. Ubuntu run [`32981424686`](https://github.com/donal0c/sartracker-web/actions/runs/32981424686) passed Linux package/native inspection, llvmpipe, 8,664/8,664 exact rows, integrity `ok`, one restart, zero renderer crashes, two graceful exits, packaged soak and real AppImage launch/close. Bind this documentation-only descendant, attest unchanged executable/test trees, then restart all five independent PR-3 reviews from zero. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and unmerged. The 2026-08-26 review-topology amendment is prospective for PR-4 onward and does not relax PR-3. Do not merge or release from this implementation task.
...
- **Breadcrumb coordination workflow:** one implementation task owns complete PR-3 through BCP-07 → BCP-08 → BCP-09. G2/G3 remain approved; every earlier review wave is superseded. Require deterministic gates and five fresh clean independent reviews on the final evidence-bound head before treating PR #3 as review ready. Prospectively for PR-4 onward, re-review scope follows affected contracts; all five restart only for unbounded/cross-boundary impact, repeated P1/P2s at one seam force an architecture pause, and a PR-4 process retrospective is required before PR-5.
