...
- **Next candidate:** beta.8. Tag `electron-v0.1.0-beta.8` (HEAD `2b5124c`) pushed 2026-06-23; CI release run `28007848162` is **green** (gates, Linux bundle, AppImage launch smoke, draft prerelease + SHA256SUMS). A **draft prerelease** exists with the Linux AppImage + `.deb` + `SHA256SUMS`. Release note at `docs/releases/sartracker-electron-0.1.0-beta.8.md`.
  - CI-built checksums: AppImage `4407c2df0e1e5c2fb127f941a7dac1f8f660948aa8ad49f0701c3868ec5e193b`, deb `7bb83fc1ef8999acc057430737967777b659d51b8b56f3ccfbc17a397fbb1e8b`.
  - Getting CI green took four prep fixes (all latent prep/test-harness issues, none in shipping app code): removed the retired Tauri `cargo test` from the Electron release gates (`gdk-3.0` not on the runner); widened then CI-scaled the DON-210/212 perf-sweep dataset (GPU-less runner saturation); gave the DON-203 large-hosted-history settings test CI-safe 15s waits. The DON-210 dedup + storage-cap invariants remain covered by deterministic unit tests.
  - **REMAINING GATE: packaged Ubuntu smoke against the CI-built artifact (not a local rebuild) is still required. GitHub release MUST stay a draft until that passes and Donal approves promotion.** Smoke matrix is in the release note's Verification section.
