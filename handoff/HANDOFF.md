...
- **PR5 mission evidence and replay is in accepted-finding remediation:** heads `f87d75873f11d12d249e3afbc482703b25f99ff4` and `1b35786c5de35355724928354789d779a52d186c` were rejected by the fresh risk reviews. The former leaked staged GPX knowledge time, separated publication from receipt settlement, allowed stale unlock authorization to reopen a newer finalization, and gave impossible Search Operations recovery copy. The latter fixed those issues but could still reconcile a newly retained source against a matching legacy baseline without exact bytes, then clear the only exact receipt. Every finding was reproduced red. The working remediation now requires an active current complete GPX revision with retained bytes whose SHA-256 is reverified before restart reconciliation can settle; legacy, retired and superseded evidence becomes an explicit retained-byte failure. Full/browser/package/scale/Linux verification and clean broad, persistence, concurrency/finalization and renderer/input-containment reviews must be repeated on one pushed replacement head. The final handoff must name every reviewer/task, SHA, verdict, finding disposition and recheck. Do not merge or release.
