...
- **`DON-260` beta.12.9 published — 36-hour breadcrumb regression (2026-08-10):** Commit `32a4326442b9`, immutable tag `electron-v0.1.0-beta.12.9`, and workflow `31414363788` are green; AppImage SHA is `d7503d2e…` and `.deb` SHA is `b1baae23…`. Exact Ubuntu AppImage proof: current `144 ms`, first breadcrumb `452 ms`, exact `279,936`-row persistence `46.287 s`, 503 retry, SIGKILL checkpoint resume, integrity `ok`, max history concurrency 8, exact canonical render, and three exact restarts at `5.667/5.557/5.649 s`. Five-/fourteen-day soaks stored exact `691,224/1,935,384` positions; extended peak RSS was `1,488,752,640` bytes. Real `.deb` is `install ok installed`, `dpkg -V` clean, independently persisted all `279,936` rows in `46.447 s`, restarted exactly at `5.703/5.786/5.787 s`, and passed lifecycle/archive, coordinates, duplicate launch, sanitized exports, corrupt secret, and live Traccar (34 devices). Guarded publication succeeded; fresh public hashes matched and the public AppImage passed a final Ubuntu lifecycle/recovery/finalize/archive smoke. Apt status 100 came only from three pre-existing broken NVIDIA/kernel packages after SAR Tracker installed; raw evidence is retained. Linear has the exact evidence. The build is handover-ready for controlled team field testing; it is not approved for live incidents. Beta.12.6/.12.7/.12.8 remain abandoned and unpublished.
...
- **Latest published team beta:** [`electron-v0.1.0-beta.12.9`](https://github.com/donal0c/sartracker-web/releases/tag/electron-v0.1.0-beta.12.9), published 2026-08-10 after full AppImage and installed-`.deb` Ubuntu qualification, deterministic 36-hour and fourteen-day proofs, guarded publication, fresh-public checksum verification, and a final public-AppImage Ubuntu lifecycle smoke. This is for controlled field testing, not live incidents; original-machine confirmation remains `DON-247`.
