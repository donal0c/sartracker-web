...
- **PR5 current remediation (2026-08-29):** exact head `c5c7c702` was broad- and renderer-clean but concurrency rejected it; persistence was not started and no invalid-head verdict counts. Central retrace reproduced one P2: if a whole-Review load/refresh failed after invalidating an in-flight Search Operations page, stale success/error was safely fenced but the retained page remained permanently `loading`, disabling Search/First/Next. Executable fix `139d812d034e` synchronously releases all invalidated page-loading flags while retaining evidence and page metadata. Four load/refresh × stale-success/error regressions failed red and now pass; an explicit UI test proves Search/First/Next recover. Local proof is green through exact renderer/replay 185/185, full unit 296 files / 2,521 tests, backend 57/one intentional ignore, Chromium 167/167, visual 2/2 plus fresh uncached critical review 4/4, lint/build/diff, unsigned macOS arm64 packaging, and a two-launch packaged soak with 6/6 batches, exact 8,664/8,664 positions/digest, integrity `ok`, zero redundant slope, zero crashes, four healthy interactions and 4.365 ms maximum main round trip. Exact-head Linux and all four independent exact-head reviews must restart on one final code-and-documentation head. PR opened/review-ready is intermediate; do not merge or release.
