...
- **`DON-260` fixed locally — 36-hour breadcrumb regression (2026-08-10):** Field evidence showed a 36-hour mission taking about nine minutes for one tracker and appearing in blocks. Root cause was beta.12.3 advancing one two-hour history chunk per normal poll, reconciling the full provider roster, replaying in-memory progress after restart, and using a temporal-only display selector. The red-to-green correction now has a continuous bounded history reconciler independent of live polling, current-first mission wake, active-device pruning, atomic durable frontiers, paced anti-entropy, deterministic geometry-aware display selection with truthful error bounds, bounded source/query memory, and lifecycle-safe worker cancellation. The final frozen macOS package passed the 33-device resilience proof: current fix `91 ms`, first breadcrumb `310 ms`, exact `279,936`-row SQLite truth in `36.112 s`, one injected `503` retry, SIGKILL/frontier resume, exact `104,274`-coordinate render oracle after restart, and history concurrency `8`. The unchanged 14-day soak also passed all `1,935,384` rows, two restarts, integrity/WAL/responsiveness gates, and fail-closed Darwin process-tree memory (`1,464,614,912` byte peak, `682,868,736` below 2 GiB). `DON-260` remains **In Progress** pending exact CI-built beta qualification; do not publish a beta from local artifacts.
