...
- **PR5 mission evidence and replay is implementation-complete and CI-qualified:** schema v12 adds atomic current projection + immutable object version + audit identity for markers, drawings, outings, stable search areas, assignments and repeated coordinator-declared passes; retirement/tombstones replace hard deletion and backend finalization fences fail closed. GPX import now retains exact bytes, SHA-256, aliases, ordered points/elevation/timestamps, rejection provenance and immutable revisions in an off-main worker. Replay is a cancellable read-only worker query for data known at T, uses only exact Traccar `fixTime`, pages exact tracks, shows legacy/undated/missing-time limitations, and never mutates the live map. Local proof: 2,309/2,309 unit tests after the deterministic fixture oracle refresh; Chromium 161/161; visual 58/58 with critical gate PASS (one unrelated pre-existing high-severity clipped-text frame remains non-blocking); backend 51 passed/1 ignored; lint/build/budgets; unsigned packaged macOS arm64 build; packaged soak 8,664/8,664 with 3.8 ms main maximum; 0.57 GiB SIGKILL/restart recovery PASS; 960k replay 331 ms and 2m warm replay 634 ms with sub-2 ms worker dispatch and sub-1 ms concurrent live reads. Exact-head Linux run `33081029856` passed AppImage/`.deb` build, native-module and llvmpipe attestations, 8,664/8,664 packaged-soak equality with restart recovery, graceful AppImage close, and the 960k PR5 gate: 686 ms seek, 668 ms restart seek, 3.35 ms import dispatch, 0.28 ms replay dispatch, 12.6 ms event-loop maximum, and exact first-page equality.
...
- **Breadcrumb coordination workflow:** PR5 remains one coherent review unit for BCP-11/12a/12b/13. After its PR is opened, run the prescribed broad exact-head review plus focused persistence/completeness, concurrency/finalization and renderer/input-containment reviews. Do not merge or release from the implementation task.
