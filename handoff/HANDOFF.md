...
- **Breadcrumb PR-2 candidate (2026-08-23):** `codex/breadcrumb-pr2-mission-model` now contains BCP-03/04/06 (`DON-270/271/272`) at product candidate `37d9ed1f5218`: one additive schema v9; explicit non-overlapping outings with Unassigned fixes; explicit device/group participants; observation-time group changes; resumable late-add backfill; grandfathered legacy/finalized provenance; >100 warn-and-proceed; selected-only evidence/map scope; and deterministic outing-shaped fixtures. `missionModel` is internal and remains off in release packaging. Local proof is green for full software gates, five changed visual entries plus fresh AI review, exact legacy fixture digest, 960k/2m v9 fixtures, GET-only live Traccar selection→local scoped persistence, flag-on macOS packaging, 8,664-position restart soak, and the schema-v9 100×5,000 stationary gate (`90.1 ms` max, `<200 ms`). **Needs attention:** the required cached 3.704 GB Ubuntu v8→v9 migration/Review proof is not rerunnable while `192.168.18.31:22` is offline; do not call PR-2 review-ready or start the four reviews until it passes.
...
- **Breadcrumb coordination workflow:** one implementation task owns a complete PR; BCP units are internal TDD checkpoints. Donal starts the four independent exact-head reviews only after PR-2's missing Ubuntu proof is green.
