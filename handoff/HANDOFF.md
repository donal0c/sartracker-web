...
- **Breadcrumb programme status (2026-08-22):** BCP-00 decisions are locked in `docs/breadcrumb-programme-execution-policy.md`: five ordered PRs, one unpublished post-coverage checkpoint, one final team release, 150 ms main-process target / 200 ms hard gate, and complexity-based planning/review rigor. Linear parent `DON-265` now owns BCP children through `DON-255`. Fable's integrated PR-1 design is `tmp/agent-mail/fable-breadcrumb-pr1-design-20260822.md`. BCP-10 (`DON-266`) is complete locally without production-code changes: `npm run test:contract:dots` proves 20,006 selected fixes across three SQLite/controller/GeoJSON pages and detects eight injected faults; lint, the manual contract, and full unit `196 files / 1,575 tests` pass. Next is BCP-01 current-position hardening under `DON-267`.
- **Breadcrumb coordination workflow (2026-08-22):** `docs/breadcrumb-programme-coordination-workflow.md` is binding. Codex coordinates and guards team requirements; fresh bounded Fable planning is preferred to expensive accumulated context; Donal explicitly approves every implementation start; then Codex creates a fresh GPT-5.6 Sol high task for that slice. Donal controls PR review and remediation normally returns to the same implementation task. The accepted PR-1 artifact already makes BCP-01 ready, so wait for Donal's go-ahead rather than running Fable again or starting implementation here.
