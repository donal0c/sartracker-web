...
- **Breadcrumb PR-3 BCP-08/09 implemented; exact-head T1 next (2026-08-24):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`. G2's 18-run Ubuntu matrix at measured SHA `8eff87b...` selected Candidate B; Donal ratified B and the unchanged 1.5 GiB/2.5 GiB budgets. BCP-08 schema v10, ledger/invalidation worker, progressive claims/controller, Candidate-B vector tiles, and bounded diagnostics are complete through pushed `a9d3930`; its stage gate passed 253 files / 1,972 tests plus lint, TypeScript, CJS syntax, build, and bundle budgets. BCP-09 flag-on coverage status, renderer-only participant/outing/Outside-outings filters, live independence, participant exclusion, exact-fix inspection entry, Chromium flows, critical visuals, and operator manual are implemented in `f96375d` plus the following docs commit. Focused units, TypeScript, lint/build, Chromium 3/3, visual 4/4, and corrected critical visual review pass. Next: push these commits, run the accepted exact-head T1 software/package + one 3.704 GB v9→v10 migration + one packaged CI tracking soak, update `DON-275`, then stop for G3. Do not flip either default before approval. G2 scale results remain standing because no measured query/segmentation/tile/source strategy changed.
