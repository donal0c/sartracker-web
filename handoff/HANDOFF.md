...
- **WAR-01 whole-application assurance baseline is open on [PR #6](https://github.com/donal0c/sartracker-web/pull/6) from `codex/war-01-assurance-charter` (2026-08-29):** the docs-only charter and 40-row stable hazard register are pinned to base `80309c995a18eeb190cce4310c9a46b0f46d5263` and cover all 12 resilience domains plus the five absolute blockers. The `DON-274`/`DON-277`/`DON-278`/`DON-279` issue set on GitHub PR #5 is unmerged, so its GPX/versioned-evidence/replay/search-area/attachment-custody seams remain `rewrite-pending` and require a short exact-merge reconciliation; the `DON-248`/`DON-252`/`DON-253` archive successor remains unimplemented and required before streamed archive/restore resilience. Local lint, the serial full unit suite (`276` files / `2,293` tests), the latest affected-boundary suite (`26` files / `365` tests), targeted Chromium (`29/29`), targeted visual Playwright (`4/4`), structural/citation audit, and diff checks pass. The exact-head review trail is recorded on PR #6; author handoff is valid only when both the broad life-safety and focused traceability verdicts are clean on the live head. No production code, tests, workflow, dependency, release configuration, operator behavior, or manual surface changed.
...
- **Branch:** `master` contains merged breadcrumb PR-1 (`9481cf462e0e`), PR-2 (`7021fc1ef33e`), and PR-3 ([#3](https://github.com/donal0c/sartracker-web/pull/3), merge `e3499d81bad68dcbf35dde4ed6ae6bd4d1434db1`). The active bounded field-feedback bridge is `codex/breadcrumb-field-feedback-bridge`; do not merge or release it from the implementation task.
- **`DON-267` field-feedback bridge is in author remediation on [PR #4](https://github.com/donal0c/sartracker-web/pull/4) (2026-08-27):** `SAR-QA-021` locks Traccar position `fixTime` as the only breadcrumb evidence clock. The five-reviewer rejection at `2770f5a` is addressed centrally: participant checkpoints wait for durable accepted fixes and durable rejection evidence; device-scoped history rejects mismatched rows; admitted history is inside the Finish/settings/shutdown fence; obsolete mission work is cancellable; incremental and reconciliation traffic share an eight-request budget; slow roster/history work cannot delay current-position cadence; and SQLite schema v11 retains `timestamp_source='fix'` so exact dots, ordering, coverage, and restart consumers fail closed on unproved legacy rows. All operator timestamp surfaces now show complete local time with timezone/offset, and the Devices roster uses a stable two-line timestamp at 1440×900. The 37,479 regression still traverses 10,000 + 10,000 + 10,000 + 7,479 while PR-3 progressive coverage remains the default all-history view. Exact-head CI run `33061973935` first exposed unresolved history transport at shutdown; the runtime now aborts only unresolved external transport, waits admitted settlement/evidence, and fabricates no checkpoint. Run `33064617669` exposed fixed-delay Electron startup test races, replaced by condition-based waits. Run `33065162100` then reached all six current batches but timed out at 8,486/8,664 exact rows: device 1's late 178-fix gap was waiting on the real five-minute anti-entropy clock while the isolated soak compressed polling to 25 ms. The validation profile now explicitly compresses anti-entropy too; production retains the five-minute default. Red/green: `/tmp/don267-ci-timeout-{red,green}.txt` and `/tmp/don267-soak-anti-entropy-{red,green}.txt`; local green: lint; build/bundle budgets; unit 276 files / 2,293 tests; backend 51 passed / 1 ignored; Chromium 159/159 from `da17fc5`; focused visual Playwright 3/3 with manual screenshot inspection. Require a green exact-head packaged CI run before handing PR #4 back for review. No programme-final Ubuntu, field soak, live Traccar, release, or merge was run. Pre-v11 rows retain null clock provenance and are excluded from exact evidence until re-observed from authoritative `fixTime`; this is not evidence that field data was lost.
- **Breadcrumb requirements authority corrected (2026-08-22):** The previously missing verbatim team Q&A is now retained in `team-feedback/breadcrumb-question-answers-20260822.md` and indexed/cross-referenced in `docs/breadcrumb-team-question-and-answer-ledger.md`. `SAR-QA-006`/`SAR-QA-013` establish that one immutable Traccar database row is one source record; repeated SAR Tracker polls are idempotent deliveries, not new breadcrumbs/evidence. The withdrawn duplicate question is recorded as `SAR-DUP-001`. `docs/breadcrumb-programme-coordination-workflow.md` now requires a ledger duplicate-check before any team question or model-labelled product ambiguity is forwarded. Reconcile BCP-02 against these IDs before resuming its executor.
- **Breadcrumb coordination workflow:** one implementation task owns complete PR-3 through BCP-07 → BCP-08 → BCP-09. G2/G3 remain approved; every earlier review wave is superseded. PR-3 is now in the same bounded remediation topology approved for later programme PRs: architecture pause, central retrace, one coherent red-first fix, all mandatory gates, then one fresh broad plus the three targeted exact-head rechecks. Repeated P1/P2s at one seam force another architecture pause; they do not trigger an automatic five-review restart. A PR-4 process retrospective remains required before PR-5.
