...
- **Breadcrumb PR-3 Stage 1 / G2 awaiting ratification (2026-08-24):** branch `codex/breadcrumb-pr3-complete-coverage` started from exact base `7021fc1e`. The prescribed 18-run packaged A/B/C matrix completed serially on `donal-Precision-5570` at measured SHA `8eff87b724ae6b4ffa9123479a8982d1d08f47ef`, with Electron on X11/Xwayland `:0` and Mesa llvmpipe. Candidate B (SQLite-backed local vector tiles) is the only candidate that passed both 960k and 2M: worst-warm first useful `3.465/3.334 s`, complete `5.976/7.554 s`, filter `133/104 ms`, main max `58/33 ms`, and settled/peak renderer RSS `0.26/0.27 GiB`. A and C both breached the 5 s first-useful gate. All 18 runs, six kill proofs, nine-pane exact attestations, Candidate-B invalidation falsifiers, and exact Dots `10/10` passed; the 29-file evidence manifest verifies at SHA-256 `470449f7...c980`. Decision memo: `docs/breadcrumb-coverage-renderer-decision.md`. Recommendation: ratify B with unchanged budgets. **No BCP-08 production code exists; do not start it until Donal ratifies G2.**
