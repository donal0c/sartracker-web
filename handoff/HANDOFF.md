...
- **`DON-260` beta.12.6 release candidate — 36-hour breadcrumb regression (2026-08-10):** The correction is pushed at `98b1b849c0a1`: continuous bounded history reconciliation, current-first wake, active-device pruning, atomic durable frontiers, paced anti-entropy, geometry-aware display selection, bounded memory/query work, and lifecycle-safe worker cancellation. Eamonn's added beta.12.5 evidence showed slow-speed clusters and larger gaps at 120–145 km/h, so the proof includes a deterministic 12 km/h then 120–145 km/h vehicle and exact packaged dot-mode comparison. Both phases retain `720/720` fixes with zero gap inflation. The frozen 33-device fault/SIGKILL/restart proof passed with current fix `99 ms`, first breadcrumb `345 ms`, exact `279,936`-row SQLite truth in `37.148 s`, and exact `103,614` line/dot coordinates. The unchanged 14-day soak passed `1,935,384` exact rows, two restarts, integrity/WAL/responsiveness, zero crashes, and peak RSS `1,533,214,720` bytes (`614,268,928` below 2 GiB). Automated beta gates are green (`178` unit files / `1,391` tests, backend `51 / 1 ignored`, Chromium `142/142`, package, CI soak); visual Playwright `37/37` and independent review `43/43` pass. Release metadata is being cut as `electron-v0.1.0-beta.12.6`; exact CI AppImage/`.deb` Ubuntu qualification remains mandatory before publication.
