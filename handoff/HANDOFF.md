...
- **Next beta candidate:** `0.1.0-beta.9` is cut as a **draft prerelease** from the Fable remediation queue (`DON-230`; fixes `DON-228`, `DON-231`-`DON-238`; `DON-239` parked). Local macOS/web gates passed on 2026-07-07, including no-skip `npm run beta:verify`, visual E2E/review, browser smoke, packaged mission lifecycle/archive smoke, coordinate/diagnostics smoke, and bad-secret packaged smoke. GitHub Actions run `28875685324` is green and created draft CI-built Linux artifacts: AppImage sha `99d3aeb3…`, deb sha `7d3490b4…`. Release note: `docs/releases/sartracker-electron-0.1.0-beta.9.md`. **Do not publish or share as current tester artifact until Donal completes the Ubuntu smoke on the CI-built artifact.**
...
- **Fable deep-analysis remediation queue:** `DON-230` is the parent Linear issue for `output/fable-deep-analysis.md` (2026-07-06). `DON-231` through `DON-238` are complete locally and included in beta.9. `DON-239` is intentionally parked as low priority for a later tracking-staleness policy pass. Beta.9 local release verification passed: no-skip `npm run beta:verify` report `tmp/beta-artifacts/verify-0.1.0-beta.9-sha.581d43382545-2026-07-07T14-50-20Z.json`, `npx playwright test --project=visual` (34/34), `npm run visual:review -- --fail-on critical` (39/39; report `test-results/visual-verification/reports/visual-review-2026-07-07T14-39-54Z.json`), browser smoke evidence under `output/playwright/beta8-local-smoke/`, packaged mission lifecycle/archive smoke under `tmp/beta9-local-packaged-core-smoke/`, packaged coordinate/diagnostics smoke under `tmp/beta9-local-packaged-coordinate-diagnostics-smoke/`, and packaged bad-secret smoke under `tmp/beta9-local-bad-secret-smoke/`. GitHub Actions run `28875685324` passed and created the draft beta.9 CI artifacts. Remaining release blocker: Ubuntu smoke against CI-built beta.9 artifact.
