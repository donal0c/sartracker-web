...
- **Breadcrumb PR-3 final remediation gate (2026-08-26):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and both G2/G3 approvals remain binding. The first review of the prior evidence head reproduced one further false-Complete P1: tile-worker count/digest/bounds were not semantically bound to exact SQLite evidence. Application head `603aadf` now requires content evidence for every requested chunk revision, compares it with a main-owned oracle produced by the independent off-main manifest worker, and discards/fences a divergent tile worker before any ledger write. It retains the earlier canonical period digest, immutable repeated-effective-from backfill, finalized-health retirement/restore, safe claim drift, and Electron-only startup fixes. Full unit is 268 files / 2,152 tests; lint, TypeScript, build/budgets, participant/coverage Chromium 10/10, selected critical visual and uncached review 2/2, and backend 51/51 with one intentional OS-keychain ignore pass. Exact application-head Linux run [`32944166541`](https://github.com/donal0c/sartracker-web/actions/runs/32944166541) passed x64 AppImage/`.deb` packaging, AppImage launch, and the 8,664/8,664 source-exact soak with digest `89c138d8…`, integrity `ok`, one restart, 27.1239 ms main maximum, zero renderer crashes, and four healthy operator interactions. No control, wording, geometry, schema, flag, or screenshot changed. The documentation-only descendant still requires deterministic exact-head/Linux gates and five fresh independent exact-head reviews from zero before review readiness. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and unmerged. Do not merge or release from this implementation task.
