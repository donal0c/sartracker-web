...
- **Branch:** `master` remains canonical for unrelated work. Breadcrumb programme changes use the five authorized PR branches; PR-1 merged as `9481cf462e0e`.
- **Breadcrumb PR-2 exact-head review remediation (2026-08-23):** code commits `b7591572d780` and `8f637a192164` close the eight P1 and five P2 findings in the second review pass. Selected-group current positions publish before durable membership bookkeeping; roster reconciliation is mission-fenced, serialized, retryable, completeness-aware, and avoids redundant hot-path repolls; removed participants retain the non-stale last accepted marker; cold/failed scopes preserve cached truth; participant writes lock at finish; backfill checkpoints are monotonic and group progress is visible; synchronous worker construction failure releases its slot. Fresh generator-v4 fixtures are 960k `476,151,808` bytes / `6b92005b…ccdc0` and 2M `991,227,904` bytes / `f70f1ac4…133ca`, identical on macOS and Ubuntu, with 100 active selected devices and every position reachable through participation-at-fix-time. Exact Ubuntu x64 worker proof returned 1,999,988 total / 1,242,988 Unassigned across 12 outings in `1.072 s`, parent maximum `14.3 ms`, using one covering mission-position stream and no global timestamp index. Exact flag-on/off Ubuntu packages passed 32-device / 8,664-position restart soaks with integrity `ok`, zero redundant slope, and main/renderer maxima `6.1/83.6 ms` and `10.6/66.9 ms`; the final macOS flag-on 100 × 5,000 gate passed at `84.3 ms`, and bounded GET-only live confirmation remained 1 group / 34 devices / 9 fixes with only the selected device persisted. Software gates: `228 files / 1,846` unit, lint/build, backend `51 / 1 ignored`, exact Dots `10/10`, Chromium `153/153`, changed visual Playwright/review `3/3`. The unchanged 3.704 GB v8→v9 proof still retains 2,040,000 positions and 32/32 grandfathered participants with integrity `ok`; schema v9 still migrates in flag-off builds under the approved non-downgrade boundary. Remaining proof limit: the 500k flag-on profile misses the renderer gate under Ubuntu Xwayland/llvmpipe (`3.685 s` final; `2.941 s` at pre-remediation `896700e`), so this is not a remediation regression but Linux/field qualification remains open while the feature stays release-off. Await an independent exact-current-head approvable comment; do not merge or start PR-3.
...
- **Breadcrumb coordination workflow:** one implementation task owns a complete PR; BCP units are internal TDD checkpoints. PR-2 is back in exact-head review after remediation; do not merge or start PR-3.
