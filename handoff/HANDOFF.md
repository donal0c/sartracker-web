...
- **`DON-261` / `DON-262` / `DON-263` beta.12.4 in qualification:** the beta.12.3 PCLinuxOS bundle proves a renderer warning-state flash, not an app crash: 109 successful polls toggled the amber history-loading warning 77-99 ms apart while tracking and storage stayed healthy. Strict TDD makes that warning first-fetch-only, fixes the Open Devices target in place, and makes the packaged soak fail closed on missing/late/extra/untrusted input. A deeper map pass also proved operational overlays were incorrectly gated on full basemap tile completion; they now synchronize from style structure without continuous `styledata` re-entry and use unlimited retry with a 2 s backoff cap. Final local non-packaged proof: unit `1,287/1,287`; Chromium `142/142`; visual `37/37`; fresh uncached independent visual review `43/43`; backend `51 passed / 1 ignored`; lint/type/syntax/diff/build budgets; held-open raster requests with `isStyleLoaded() === false` while labelled current positions and breadcrumbs visibly render; same-session Fable `PASS_FOR_COMMIT` twice with no P1/P2. The earlier Ubuntu package passed ten 86,640-position soaks and 40/40 audited interactions but predates the final map diff, so it is preliminary only. `electron-v0.1.0-beta.12.4` remains untagged and private until clean no-skip verification, green tag CI, and exact CI AppImage/`.deb` Ubuntu qualification including >=10 stable polls, active-tracking basemap switch, and pending/degraded-basemap overlays.
