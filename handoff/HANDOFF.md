...
- **Breadcrumb PR-2 exact-head review remediation (2026-08-24):** code commit `f8cc21f` closes the ninth-pass two P2s. Mission switches now publish empty/loading outing truth before awaited summary cancellation, discard stale-generation mutation results, and keep both the controller and visible controls read-only until successful current-generation hydration; failures remain fail-closed. The prior mission can no longer supply the active outing or default label for the new mission. Five deterministic race/scope/error assertions failed red first; focused outing tests pass `3 files / 14`, full serial unit `229 / 1,880`, lint/build/bundle budgets, diff checks, and outings+participants Chromium `8/8`. No fresh full Chromium, packaged-runtime, heavy-scale, Ubuntu/Windows, or field-hardware proof was run because the change is confined to renderer outing orchestration; standing proofs bind only unchanged surfaces. Await an independent exact-current-head approvable comment; do not merge or start PR-3.
