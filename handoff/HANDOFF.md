...
- **PR5 current remediation (2026-08-29):** exact head `8bc2b764` was clean in persistence and concurrency review, but broad life-safety review rejected it; renderer review was not started after invalidation. Central retrace reproduced a Search Operations read begun after refresh invalidated old tokens but before refresh completed, then released after `pass-fresh` to overwrite it with `pass-stale`. Executable fix `2dc3c1662e02` blocks Search Operations reads while Review is refreshing, binds every in-flight result to the exact Review refresh token, and disables search/page/write controls with an explicit refreshing state. The exact race failed red and now passes; the prior pinned SQLite generation/count/page and changed-continuation fences remain green. Local proof is green through 296 files / 2,515 unit tests, backend 57/one intentional ignore, Chromium 167/167, visual 2/2 plus fresh uncached critical review 4/4, lint/build/CommonJS/diff checks, unsigned macOS arm64 packaging, and a packaged soak with two launches, 6/6 batches, exact 8,664/8,664 positions, integrity `ok`, zero redundant slope and 2.7 ms maximum main round trip. Exact-head Linux and all four independent exact-head reviews must restart on one final code-and-documentation head because this fix crosses renderer/runtime concurrency. PR opened/review-ready is intermediate; do not merge or release.
