...
- **Fable deep-analysis remediation queue:** `DON-230` is the parent Linear issue for `output/fable-deep-analysis.md` (2026-07-06). `DON-231`, `DON-232`, `DON-233`, and `DON-234` are complete locally. DON-233/DON-234 harden the Traccar tracking data path: breadcrumb fetches now use bounded cursor overlap, future-timestamp clamping, max-timestamp cursor advancement for unsorted batches, canonical timestamps, source-id-aware accumulation, same-second distinct position persistence, and seed-failure protection; session-cookie 401/403 responses clear stale auth and reauthenticate; request timeout handles clear on failure; Traccar API speed is normalized from knots to km/h before display/storage. Verification passed: red-to-green tracking/client/Electron regressions, `npm run lint`, `npm run build`, `npm run test` (152 files / 1028 tests), `npm run test:backend` (47 passed / 1 ignored), `npm run test:e2e` (162/162), `npm run visual:review -- --fail-on critical` (overall PASS, 0 critical blockers; 1 existing high nonblocking basemap prompt mismatch), `npm run electron:pack`, packaged launch smoke, packaged bad-secret smoke, and in-app Browser validation against the running dev server plus mock Traccar (`ONLINE`, 8 devices, live fixes, Devices workspace, km/h speed display). `npm run beta:verify` passed through lint/build/unit/backend/Chromium E2E/package but its interactive smoke prompt could not be completed after the session detached; official offline smoke was not applicable because no private MBTiles package is present. Next recommended queue item is `DON-235`. Stale-device threshold is still a product decision because code/tests use 60 min while repo/S7 docs say >5 min.
