...
- **Breadcrumb PR-1 review remediation (2026-08-23):** [PR #1](https://github.com/donal0c/sartracker-web/pull/1) remains unmerged on `codex/breadcrumb-pr1-ingest-safety`. The requested-change pass now fixes mission-scoped rejection identity at observation time, durable restart health, same-process replay, invalid/capacity evidence-loss markers, Windows-safe outbox writes, all-invalid current polls, bounded/aggregated anomaly reads, stationary restart identity, the 18-minute default tolerance boundary, conservative unknown accuracy, corroborated movement clearing, and the remaining main-isolate Review health scan. Tauri remains historical/reference. Full unit `208 / 1,684`, backend `51 / 1 ignored`, Chromium `146 / 146`, tracking visual `13 / 13` plus fresh review `15 / 15`, BCP-10 `10 / 10`, lint/build, and macOS Electron packaging are green. The exact committed-head 3.704 GB Review gate and CI packaged soak are the remaining proof steps before push/PR/Linear closeout. DON-251 still owns the cold `~7.5 s` query/index/retention optimization; the 200 ms gate is not waived.
