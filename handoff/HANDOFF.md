...
- **Breadcrumb PR-3 final remediation gate (2026-08-26):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and both G2/G3 approvals remain binding. Application head `6db23e0` closes two further review defects red-first: active history filters now carry omission predicates and derive their selected chunks from every fresh manifest, so a later visible outing cannot be omitted from the claim; delayed startup evidence health for a finalized mission stays retired until explicit unlock. A real-store/real-production-worker integration now binds staged/active tile equality, retained plus rebuilt chunks, ready claims, stale-revision rejection, and honest recovery. It retains all earlier worker-content, canonical-digest, participant, claim-drift, and Electron-startup fences. Full unit is 269 files / 2,156 tests; lint, TypeScript, build/budgets, participant/coverage Chromium 10/10, selected critical visual and fresh uncached review 2/2, and backend 51/51 with one intentional OS-keychain ignore pass. Exact application-head Linux run [`32948959241`](https://github.com/donal0c/sartracker-web/actions/runs/32948959241) passed x64 AppImage/`.deb` packaging, AppImage launch, and the 8,664/8,664 source-exact soak: digest `053bda2d…`, integrity/WAL `ok`, one restart, zero renderer crashes, four healthy interactions, zero redundant slope, and 20.541 ms Electron-main maximum. The manual now states that newly discovered visible periods remain inside the selected-history completion check; controls/layout and screenshots are unchanged. Bind the documentation descendant, repeat its exact-head deterministic/Linux gates, and restart five independent reviews from zero. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and unmerged. Do not merge or release from this implementation task.
