...
- **Breadcrumb PR-3 frozen review-head preparation (2026-08-25):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and both G2/G3 approvals remain binding, and the v9→v10 migration proof is unchanged. Review remediation code is frozen at `5653133`: predecessors remain serviceable through source load and opaque worker activation, pending evidence revokes completeness synchronously without downgrading stronger health, failed/obsolete stages are fenced and retryable, and coverage requires the mission model. The six invalidated B rows plus two kill probes pass unchanged budgets; exact production delivers 959,988/959,988 and 1,999,988/1,999,988 within the first-useful gate with only the intentional `backfill_incomplete` blocker; the rebound packaged soak passes 8,664/8,664, restart, integrity, responsiveness, llvmpipe attestation, and zero redundant slope. Evidence-binding head `c41f1e1` passes 259 files / 2,019 tests, lint, TypeScript, JS syntax, Dots 10/10, build/budgets, Chromium 157/157, selected coverage visual 4/4 plus fresh critical review 6/6, and macOS arm64 + Ubuntu x64 packaging. A subsequent fresh reviewer correctly rejected one out-of-frame visual checklist item; the screenshot prompt is now red-first corrected and focused review passes. Bindings: `docs/breadcrumb-coverage-renderer-decision.md`, `docs/breadcrumb-pr3-t1-evidence.md`, and matching `output/` directories. Next: commit the test-evidence correction, repeat all gates on that exact head, then restart all five independent reviews from scratch.
