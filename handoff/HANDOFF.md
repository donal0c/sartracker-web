...
- **Programme branch state (2026-08-29):** breadcrumb PR-1 through PR-5 are merged on `master`; PR-5 landed at `eec92812b783a795c093f37268b295dd2179a3af` from final head `f5ba8647131950dde457e50ce36fe0b8ded7337d`. The `DON-248`/`DON-252`/`DON-253` archive lifecycle is not implemented. WAR-13A is a docs-only protocol branch; it does not release a candidate or provide field proof.
...
- **WAR-13A protocol:** `docs/assurance/shadow-use-protocol.md` defines primary-source authority, permitted/prohibited use, named roles, exact session identity, stop/fallback/revert triggers, proportionate evidence intake, residual-risk records, and the predeclared WAR-13B scorecard. Before the first session, Donal/team must name the existing primary operational source/process and record a successful primary-only drill. This protocol is not operational qualification.
...
- **Next programme implementation:** `DON-248`/`DON-252`/`DON-253` own archive streaming, custody, restore, archive-backed review, and indefinite-retention lifecycle. Until they are implemented and qualified, shadow sessions must not rely on current archive/restore behavior.
