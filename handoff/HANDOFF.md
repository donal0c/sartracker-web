...
- **`DON-260` exact Breadcrumb Dots correction — beta.12.10 candidate (2026-08-11):** Team beta.12.9 feedback proved Dots still rendered the bounded Line projection: a steady 5-second trail became adjacent pairs/groups separated by roughly 60 seconds. The old packaged oracle was circular because it generated expected dots with the same selector. The replacement gives Dots its own mission-start-bounded exact SQLite page query and dedicated MapLibre source; <=10,000 fixes display automatically and larger missions use exact chronological Earlier/Later pages. Line remains bounded and owns all simplification/error-bound language. The frozen macOS artifact passed an independent 36-hour 279,936-fix/28-page rendered proof and a 14-day 1,935,384-fix/194-page soak with zero gaps/duplicates, two restart checkpoints, exact final Line total, 56.329 s outward / 64.832 s return traversal, 1.737 GB peak RSS, and clean operator-input ownership. Full unit is `195 files / 1,548 tests`; full Playwright is `179/179`; independent visual review is `43/43`. Source is being prepared as `0.1.0-beta.12.10`; CI AppImage, installed `.deb`, Ubuntu exact proofs, and live Traccar provider-to-map equality remain release-blocking. **Do not call beta.12.9 breadcrumb Dots qualified or hand it over as the corrected build.**
