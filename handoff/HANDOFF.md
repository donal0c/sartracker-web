...
- **Breadcrumb PR-3 external-review remediation (2026-08-26):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and both G2/G3 approvals remain binding. External and renewed review invalidated the earlier clean verdicts with false freshness/classification, worker-envelope and chunk-operation override, tile-cache escape, and activation recovery defects. Commits through `b2a3a86` close them red-first. The latest fix preserves failed terminal ownership, retains a retryable abandoned-stage record after renderer loss, settles it before replacement sync, and treats a lost worker-generation token as idempotently discarded only when no different stage exists. Focused green is 5 files / 54 tests; full unit is 264 files / 2,115 tests, plus lint, TypeScript, CommonJS syntax, build/budgets, and participant/coverage Chromium 10/10. The `590df46` Linux CI pass is superseded by this operational-path change; a new exact-head Linux package/soak check and five fresh independent reviews must be green before review readiness. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and unmerged. Do not merge or release from this implementation task.
