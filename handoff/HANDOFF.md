...
- **Breadcrumb PR-3 final exact-head gates (2026-08-25):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and both G2/G3 approvals remain binding. Exact-head review found one remaining mission-sized outing scan on accepted writes. The first indexed attempt still scanned closed outings for a late Outside-outings fix and was rejected; `40a713c` now fetches only the indexed latest-start predecessor and applies its half-open end boundary in memory. That exact head passed 262 files / 2,096 tests, lint, TypeScript, CommonJS syntax, exact Dots 10/10, build/budgets, unsigned macOS arm64 and Ubuntu x64 packaging, and the final replacement packaged soak: 8,664/8,664 exact positions, restart/integrity clean, zero redundant slope, 20.482 ms main max, and four healthy interactions. Full Chromium remains exact at parent `add5639` 158/158; the unchanged operator surface retains coverage visual 7/7 and fresh critical review 9/9 from `4b740f2`. Corrected 960k/2M production qualification, ratified G2 rows, and 3.704 GB migration remain standing under their explicit invalidation rules. Next: commit this evidence binding, restart five independent exact-head reviews, then open one PR to `master` if all five are clean.
