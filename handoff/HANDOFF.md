...
- **Active breadcrumb programme branch:** `codex/breadcrumb-pr1-ingest-safety`. Donal explicitly authorized five substantial PRs for this programme; do not commit programme implementation directly to `master`. Canonical decisions: `docs/breadcrumb-mission-history-architecture-decision.md` and `docs/breadcrumb-programme-execution-policy.md`.
- **Breadcrumb PR-1 review remediation (2026-08-23):** [PR #1](https://github.com/donal0c/sartracker-web/pull/1) remains unmerged on `codex/breadcrumb-pr1-ingest-safety`. Four behavior review findings are closed at verified product head `d5e30b3044aad639e405ccfc4a2c8bdd13709630`: renderer rejection acceptance drains and seals across finalization; a successful admin unlock reopens it, while a per-mission epoch prevents an older finalization continuation from resealing it; bounded outbox replay rotates past terminal records; and stationary projection/breadcrumb accumulation/cache/MapLibre updates stay bounded to changed device trails. Current safeguards still include mission-scoped health/identity, finalized-mission projection guards, 4,096-file/64 MiB durable backpressure, bounded background replay, source-device ownership, fix-time warning preservation, disposal draining, corrupt-record quarantine, and no Tauri delivery claim. Exact product-head proof: focused lifecycle/runtime/store `100 / 100`; full unit `210 files / 1,717 tests`; full Chromium `146 / 146`; BCP-10 `10 / 10`; lint/build/bundle/diff green; and exact-head Linux Electron artifact validation [run `32631592681`](https://github.com/donal0c/sartracker-web/actions/runs/32631592681) green. Backend `51 / 1 ignored`, tracking visual `13 / 13` with fresh critical review `15 / 15`, and macOS arm64 packaging remain from the earlier candidate surface because the lifecycle-only remediation does not touch those paths. The packaged stationary-cluster result remains bounded to older head `9929ded5e694`: schema-v8 177,692,672-byte store, 100 devices × 5,000 fixes, `49.2 ms` maximum renderer gap, exact 100 current fixes/100 attention states, and the unchanged `<200 ms` hard gate. This is synthetic macOS arm64 proof, not Windows/Linux field hardware or live-provider proof. The earlier exact `f24906c425e0` 3.704 GB migration/Review and 8,664-position soak remain valid because migration, Review SQL, persistence, and soak instrumentation did not change. DON-251 still owns the cold `~7.5 s` query/index/retention optimization.
...
- **Breadcrumb coordination workflow (2026-08-22):** one authorized Sol-high implementation task owns a complete PR through review-ready proof. BCP units are internal planning/TDD checkpoints, not separate implementation tasks. Donal coordinates PR review; keep `DON-266`–`DON-269` open until PR-1 merges.
