...
- **Branch:** breadcrumb PR-1 through PR-4 are merged on `master`. PR4 [#4](https://github.com/donal0c/sartracker-web/pull/4) merged as `80309c995a18eeb190cce4310c9a46b0f46d5263` from final reviewed head `a6ebfbebb7b624a85ab437508209d8d56c9cf9d8`. The next prepared branch is `codex/breadcrumb-pr5-evidence-replay`, created from that exact merge; implementation has not started and still requires Donal's explicit go.
...
- **PR5 mission evidence and replay is pre-dispatch ready:** one coherent PR owns `DON-274`, `DON-277`, `DON-278` and `DON-279` through a review-ready head. The exact base is `80309c995a18eeb190cce4310c9a46b0f46d5263`. Reconciliation found no new team question or architecture contradiction: the additive migration starts from schema v11; replay preserves `timestamp_source='fix'`, PR4 lifecycle/cancellation/current-cadence contracts and explicit incompleteness for unproved legacy evidence. Executor remains one fresh GPT-5.6 Sol high task. Review uses one broad plus three risk-focused exact-head passes, with only affected rechecks after remediation rather than an automatic fixed-five restart.
...
- **Breadcrumb coordination workflow:** the coordinator has completed the PR4-to-PR5 retrospective and exact-base reconciliation. When Donal says go, create one fresh Sol-high implementation task for complete PR5; do not split BCP-11/12a/12b/13 into separate tasks. Repeated P1/P2s at one seam force an architecture pause, not review-count escalation by default.
