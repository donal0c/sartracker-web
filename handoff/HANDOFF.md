...
- **PR5 current remediation candidate (2026-08-28):** exact head `a584b2aa` passed Linux run `33207406645` but all four independent reviews rejected it with five accepted P2s. Fix `12532025` closed those findings. Documentation-bound `d051e4c9` then produced conflicting exact-head Linux evidence: automatic run `33213448567` passed, while a redundant simultaneous manual run `33213464281` failed on a slower runner with 201-463 ms current writes and incomplete fixed-time settlements. It was rejected before review. Margin fix `7f14d518` reduces legacy-event turns to 250 rows, lengthens GPX writer yields, removes multi-megabyte exact-source duplication from the active projection while retaining the immutable revision, and extends only harness settlement time—not the `<200 ms` gates. Five affected scale/liveness tests passed three consecutive runs; full unit is 294 files / 2,484 tests; lint/build are green; indexed 960k recorded 4,188 concurrent writes at 24.68 ms maximum, 25.27 ms event-loop maximum and exact restart equality; the rebuilt two-launch packaged soak passed 8,664/8,664 at 19.2 ms main maximum. Browser/visual/backend proof remains green from the unaffected parent. Documentation binding, push, isolated exact-head Linux and all four restarted reviews remain mandatory. PR opened/review-ready is intermediate; do not merge or release.
