...
- **Breadcrumb PR-1 review remediation (2026-08-23):** [PR #1](https://github.com/donal0c/sartracker-web/pull/1) is review-ready and remains unmerged on `codex/breadcrumb-pr1-ingest-safety`. The requested-change pass fixes mission-scoped rejection identity at observation time, durable restart health, same-process replay, invalid/capacity evidence-loss markers, Windows-safe outbox writes, all-invalid current polls, bounded/aggregated anomaly reads, stationary restart identity, the 18-minute default tolerance boundary, conservative unknown accuracy, corroborated movement clearing, and the remaining main-isolate Review health scan. A final packaged soak also caught and red-first fixed transient placeholder-device audit churn with a 50 ms roster grace; genuinely slow roster reads still cannot hold current fixes. Tauri remains historical/reference. Full unit `208 / 1,685`, backend `51 / 1 ignored`, Chromium `146 / 146`, tracking visual `13 / 13` plus fresh review `15 / 15`, BCP-10 `10 / 10`, lint/build, and macOS Electron packaging are green. Exact packaged code commit `f24906c425e0` passed the 3.704 GB v7→v8/Review gate: preserved 2,040,000-row digest, 43.4 ms startup and 20.2 ms Review-open main maxima, exact audit/count results, and live current-position publication during the 1.566 s worker query. The CI packaged soak passed 8,664/8,664 positions, two launches, 4.5 ms main maximum, and zero redundant audit slope. DON-251 still owns the cold `~7.5 s` query/index/retention optimization; the 200 ms gate is not waived.
