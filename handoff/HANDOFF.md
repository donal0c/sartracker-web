...
Current directed work: continue `DON-60` team-requirements issues one at a time. Bug protocol is reproduce first, then fix, then confirm; feature/change protocol is implement and verify the expected visible behaviour. Each issue should be staged, committed, and pushed separately. `DON-64` (paused-state visibility), `DON-75` (opt-in helicopter slots), and `DON-73` (layer workspace controls: Collapse All added; Move Up/Down verified working; Favorite removed) are complete and pushed. Next likely item: `DON-65`.
...
- `DON-63` — Device list focus behavior opens marker list / defocuses map. Fixed locally 2026-06-01 by stopping passive device-list cells and nested controls from bubbling into row selection; needs commit/push closeout.
- `DON-64` — Make paused mission state unmistakable. Done locally 2026-06-01: flashing bright-red PAUSED chip on mast + Mission Control, red paused banner with frozen-time text and a dedicated Resume Mission button, panel ring, non-collapsible dock while paused, reduced-motion fallback. Pure selector `mission-phase-presentation.ts`. Needs commit/push closeout.
...
Most recent verification in this turn (`DON-73` layer workspace controls):
