...
- **PR5 current remediation (2026-08-29):** broad, persistence and concurrency rejected exact head `e55292bb`; renderer was not started after invalidation. All three reproduced the same P2: filtering before `LIMIT 1000` let one sparse incomplete event turn rewrite up to 2,000,001 complete rows, blocking current fixes for 383.93–388.24 ms. Concurrency found a second P2: unlock could win during invalid finalized-archive validation, leave a stale recovery archive, and a later finalization epoch could reuse it. Executable fix `9a55b987e63e0ac8729fa3c3e79d4eac74509f1a` pages raw row IDs before filtering, updates only selected incomplete identities, and binds recovery fence/audit/reuse to the exact finalization epoch. Both strict regressions were red then green; focused 167/167 and full unit 295 files / 2,500 tests, backend 57/one intentional ignore, lint/build/CommonJS and diff checks are green. Previous Chromium 167/167 and visual 58/58 plus uncached 69/69 stand because no renderer path changed. Exact-head Linux and the restarted broad/persistence/concurrency reviews plus mandatory renderer/input containment remain the task completion gate. PR opened/review-ready is intermediate; do not merge or release.
