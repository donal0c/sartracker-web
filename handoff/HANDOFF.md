...
- **PR5 replacement candidate is locally qualified after the second Linux rejection (2026-08-28):** pushed head `011a8051` is rejected. Linux run `33185543280` proved indexed startup was fixed, but its 500,000-settled-row liveness gate reached only cursor `450003` of `500000` within the unchanged 1.2-second deadline. Executable fix `123116e4` / binding `2a2c5ba4` retains the 10,000-candidate turn cap and 200 ms startup/heartbeat gates while reducing the inter-turn yield from 10 ms to 5 ms. Fresh proof is unit 289/2,438; backend 55/1 intentional ignore; lint/build/budgets; Chromium 165/165; visual 58/58; uncached visual 69/69 (`visual-review-2026-08-28T15-55-22Z.json`); indexed 960k (65.54 ms seek, 50.10 ms late page, 51.44 ms max current write, 61.09 ms event loop, exact restart); and authentic-v11 960k (5.53 ms open, 604.17 ms seek, 3.35 ms current write, 589.61 ms restart, explicit fallback). Unsigned macOS arm64 packaging and the bound packaged soak passed 6/6 batches, 8,664/8,664 exact positions, one restart, four healthy interactions, SQLite integrity `ok`, 9.04 ms main maximum, zero crashes and zero redundant-event slope. Push/Linux proof and the restarted broad, persistence/completeness, concurrency/finalization and renderer/input-containment exact-head wave remain mandatory. Do not merge or release.
