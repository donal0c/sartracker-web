...
- `DON-57` — S8c.5a: Build Electron Linux artifacts natively in CI. New workflow `.github/workflows/electron-linux-validation.yml` builds on `ubuntu-22.04`, checks packaged `better_sqlite3.node` is ELF x86-64, generates SHA256SUMS, runs a light AppImage launch screenshot under Xvfb, and uploads artifacts/evidence.
...
Most recent local verification in this turn (DON-57 Electron Linux CI workflow):

- Added `.github/workflows/electron-linux-validation.yml` with manual and PR-path triggers.
- Workflow builds Electron AppImage + `.deb` on `ubuntu-22.04`, verifies the packaged SQLite native module using `file`, writes `SHA256SUMS`, runs a light AppImage launch smoke under Xvfb with `--no-sandbox`, and uploads artifacts plus validation evidence.
- Passed: `actionlint .github/workflows/electron-linux-validation.yml`.
- Passed: `npx vitest run tests/unit/vite-config.test.ts`.
