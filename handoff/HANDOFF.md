...
- **Breadcrumb PR-3 final remediation gate (2026-08-26):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and both G2/G3 approvals remain binding. Fresh review invalidated evidence head `862a300` with a P1: query-worker validation covered invalidation analysis only, so a plausible current-sequence manifest could omit a real stale chunk, derive an empty ready claim, and pass the controller's revision test vacuously. Application head `20ca76e` closes it red-first. All six query result kinds now have request-bound envelopes; enumeration/manifest inventory and final claims are independently attested against bounded main-side metadata; and Complete requires exact unique revision coverage of the selected chunks. An adjacent exact-zero recomputation can no longer inherit stale min/max bounds. Full unit is 264 files / 2,143 tests, plus full lint, TypeScript, build/budgets, CommonJS syntax, and 251 focused coverage/participant tests. Exact Linux run [`32930525474`](https://github.com/donal0c/sartracker-web/actions/runs/32930525474) passed x64 AppImage/`.deb` packaging, AppImage launch, and the 8,664/8,664 packaged soak with digest `c074b33b…`, integrity `ok`, one restart, 46.882 ms main maximum, zero redundant slope, zero renderer crashes, and four healthy operator interactions. This backend/controller trust remediation does not change the operator manual or screenshots. The next documentation-only descendant requires deterministic exact-head gates and five fresh independent exact-head reviews from zero before review readiness. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and unmerged. Do not merge or release from this implementation task.
