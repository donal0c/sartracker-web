...
- **PR5 current remediation candidate (2026-08-28):** exact head `a584b2aa` passed Linux but all four independent reviews rejected it with five accepted P2s; fix `12532025` closed them. `d051e4c9` was rejected on conflicting simultaneous Linux runs. Margin head `84702866` was also rejected before review when isolated Linux run `33215754562` measured a 346.54 ms object-backfill heartbeat and timed out the 500,000-event settlement. Executable fix `e9466e54` moves restartable legacy event/object/GPX reconstruction to a joined worker isolate; the main store remains `synchronous=FULL`, worker turns are reconstructible from retained immutable rows, failure reasons are durably surfaced, current positions remain available, and Replay/evidence/finalization stay fail-closed until completion. The five scale/liveness gates passed three consecutive runs; full unit is 295 files / 2,488 tests; lint/build, backend 57/one intentional ignore and CommonJS/Rust checks are green. Fresh indexed 960k retained digest `b7f89681...fdcfc`, 4,069 concurrent writes at 24.27 ms maximum, 42.49 ms event-loop maximum and exact restart equality. The rebuilt unsigned macOS package passed two launches, 6/6 batches, 8,664/8,664 exact positions and a 3.87 ms main maximum. Browser/visual proof remains green from the unaffected parent. Documentation binding, push, isolated exact-head Linux and all four restarted exact-head reviews remain mandatory. PR opened/review-ready is intermediate; do not merge or release.
