...
- **Breadcrumb PR-1 review remediation (2026-08-23):** [PR #1](https://github.com/donal0c/sartracker-web/pull/1) remains unmerged on `codex/breadcrumb-pr1-ingest-safety`. The latest red-first pass closes the three exact-head review threads: renderer rejection acceptance is drained and sealed across the complete main finalization call; the durable outbox rotates its bounded replay slice so terminal records cannot starve later mission evidence; and stationary projection, breadcrumb accumulation, disabled-cache handling, plus MapLibre source diffs keep a one-device update bounded without rescanning/reserializing every retained device trail. The full current safeguards remain: mission-scoped health and identity, finalized-mission projection guard, 4,096-file/64 MiB durable backpressure, bounded background replay, source-device ownership, explicit fix-time warning preservation, disposal draining, corrupt-record quarantine, and no Tauri delivery claim. Proof at behavior head `e9cbad925454`: focused `109 / 109`; full unit `210 files / 1,715 tests`; backend `51 / 1 ignored`; full Chromium `146 / 146`; tracking visual `13 / 13` with fresh critical review `15 / 15`; BCP-10 `10 / 10`; lint/build/bundle/diff checks; and macOS arm64 Electron packaging. The focused packaged stationary-cluster profile used a schema-v8 177,692,672-byte store with 100 devices × 5,000 fixes: two one-device update runs kept renderer gaps at `46.4 ms` and `57.3 ms`, preserved 100 current fixes and 100 attention states, and passed the unchanged `<200 ms` hard gate. This is synthetic macOS arm64 proof, not Windows/Linux field hardware or live-provider proof. The earlier exact `f24906c425e0` 3.704 GB migration/Review and 8,664-position soak remain valid because migration, Review SQL, persistence, and soak instrumentation did not change. DON-251 still owns the cold `~7.5 s` query/index/retention optimization.
