...
- **`DON-260` beta.12.7 release candidate — 36-hour breadcrumb regression (2026-08-10):** The original correction at `98b1b849c0a1` separates current fixes from continuous bounded catch-up, checkpoints durable frontiers, paces anti-entropy, preserves route geometry, bounds renderer/query memory, and cancels stale work safely. Eamonn's slow-speed and 120–145 km/h evidence is covered by exact source/render identity checks. Candidate beta.12.6 passed Mac/local/CI gates but was rejected by the unchanged Ubuntu 60-second proof: every two-hour/device chunk still performed a separate `synchronous=FULL` SQLite transaction, leaving only `268,275/279,936` rows after the deadline. Commit `4896ecd` now persists each fetched wave (up to eight chunks and all checkpoints) in one atomic transaction; batch/fetch failure isolation, empty windows, mission replacement, and publication-after-durability are regression-locked. Full unit is green at `178` files / `1,398` tests; independent review found no remaining P0/P1/P2. Beta.12.7 must still pass the complete no-skip, tag CI, exact CI AppImage/real `.deb` Ubuntu 36-hour + long-soak + operator matrix before publication. Beta.12.6 remains an unpublished draft candidate and must not be handed to the team.
