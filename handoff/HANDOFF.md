...
- **`DON-260` beta.12.9 release candidate — 36-hour breadcrumb regression (2026-08-10):** Continuous bounded catch-up, atomic up-to-eight-chunk wave persistence, durable restart frontiers, error-bounded route retention, cancellation, and bounded memory are implemented. Exact beta.12.8 Ubuntu proof persisted all `279,936` fixes in `46.345 s`, but completed-history restart was borderline (`10.177 s` fail, `9.892 s` pass against `10 s`), so the candidate remains unpublished. Root cause was SQLite rescanning the mission index for each selected-row hydration chunk; `b3a3826` forces rowid primary-key lookup (`4.642 s` to `1.315 s`, identical `103,633` retained rows) and `132281b` makes three signal-safe instrumented restarts the default proof. Full unit is `178 files / 1,402 tests`; lint, type, build, bundle budgets, and independent review are green. Next: cut immutable beta.12.9, then require exact CI AppImage three-restart/36-hour/14-day and real installed `.deb` Ubuntu qualification before publication. Beta.12.6/.12.7/.12.8 remain abandoned and unpublished.
