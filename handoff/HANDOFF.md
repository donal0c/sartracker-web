...
- **Breadcrumb PR-3 final exact-head gates (2026-08-25):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and both G2/G3 approvals remain binding. Review #4 at `9ce75c4` found that initial direct devices lacked fixed-window backfill checkpoints and that the open-outing cooldown reordered rather than suppressed early rebuilds. Both were reproduced red-first and fixed through `691775a`; only explicit operator Retry bypasses the automatic 30-second throttle. The first replacement soak then correctly rejected two newly required backfill-completion audits as undeclared; `69a1096` adds a tested exact event classifier without widening telemetry allowances. Later reviews found the browser proof mirror still omitted the now-correct production direct-device checkpoint and matched the reported packaged benchmark exception to a renderer-destruction race. Commit `928158c` closes both red-first. The first restarted review then found the manual still told operators the G3-approved controls were disabled; red-first corrections through `133cc14` now state the default-on and controlled-rollback posture. Final gates pass 263 files / 2,104 tests, lint/TypeScript/syntax/Dots/build, Chromium 158/158, participant/coverage visual 11/11, fresh critical review 13/13, focused manual/default gates, both exact-head application packages, and a rebuilt macOS benchmark package. The final replacement Ubuntu soak remains standing at 8,664/8,664 exact positions, restart/integrity/WAL clean, zero redundant slope, 9.558 ms main / 83.6 ms renderer maxima, four healthy interactions, and zero unexplained events because no packaged operational path changed; evidence is under `output/pr3-packaged-soak/69a1096bd950270686c8e200da4311a1ab1fb1f5/`. Corrected 960k/2M production qualification, ratified G2 rows, and the 3.704 GB migration remain standing under their explicit invalidation rules. Next: restart five independent exact-head reviews, update Linear, and open one PR to `master` only if all five are clean.
...
- **Breadcrumb coordination workflow:** one implementation task owns complete PR-3 through BCP-07 → BCP-08 → BCP-09. G2/G3 and final exact-head gates are approved/green. Five freshly restarted independent reviews remain before the PR opens.
