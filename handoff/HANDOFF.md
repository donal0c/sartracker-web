...
- **Breadcrumb PR-3 final exact-head gates (2026-08-25):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and both G2/G3 approvals remain binding. Exact-head review found one remaining mission-sized outing scan on accepted writes; `add5639` replaces it red-first with an indexed O(log outings) point lookup. That head passed 262 files / 2,096 tests, lint, TypeScript, CommonJS syntax, exact Dots 10/10, build/budgets, Ubuntu Chromium 158/158, unsigned macOS arm64 and Ubuntu x64 packaging, and the replacement packaged soak: 8,664/8,664 exact positions, restart/integrity clean, zero redundant slope, 7.784 ms main max, and four healthy interactions. The unchanged operator surface retains exact coverage visual 7/7 and fresh critical review 9/9 from `4b740f2`. Corrected 960k/2M production qualification, the ratified G2 rows, and the 3.704 GB migration remain standing under their explicit invalidation rules. Next: commit this evidence binding, restart five independent exact-head reviews, then open one PR to `master` if all five are clean.
