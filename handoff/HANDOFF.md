...
- **Breadcrumb PR-3 final remediation gate (2026-08-26):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and G2/G3 approvals remain binding. Application commit `9276352` retains every earlier completeness/finalization fence and closes the composed native-close/reload race after the renderer-teardown drain work. Harness head `906beb0` makes both active-mission launches prove graceful code-0 `app.quit()` and makes the standalone AppImage click a real Openbox close control. Exact Linux run [`32966604838`](https://github.com/donal0c/sartracker-web/actions/runs/32966604838) passed x64 AppImage/`.deb` packaging, real window close, and the 8,664/8,664 source-exact soak: digest `317a7368…`, integrity/WAL `ok`, one restart, both launches graceful, zero renderer crashes, four healthy interactions, zero redundant slope, and 21.995 ms Electron-main maximum. Rejected harness and 444.843 ms responsiveness evidence is retained in `docs/breadcrumb-pr3-t1-evidence.md`. The existing manual and critical screenshot remain current. Bind this documentation descendant, repeat exact-head deterministic/browser/visual/macOS/Linux gates, then restart five independent reviews from zero. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and unmerged. The 2026-08-26 review-topology amendment is prospective for PR-4 onward and does not relax PR-3. Do not merge or release from this implementation task.
