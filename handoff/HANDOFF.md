...
- **PR5 mission evidence and replay is at final candidate verification:** earlier exact-head review waves on `aaeaeb769b181f9fd9da1d07b2fa6ae9d4e81e19`, `afc4880ab104ea0cbe75a3f139575c6f3b8c52f2` and `7325e4736b954b9a622f0344d1aee9ed43d38723` were not clean; every accepted finding was centrally source-retraced and reproduced before correction. The final remediation adds strict GPX underflow containment, one-WAL-snapshot replay responses, generation plus exact eligible-position-count cursor fences, participant/group and provenance invalidation, and a durable finalization fence that prevents finished-mission bookkeeping or stale async acknowledgement/archive writes after the immutable snapshot. The fence survives archive-success interruption for safe retry and clears only on finalized commit or pre-success archive failure. Earlier accepted findings include 8 MiB exact-source containment, serialized content dedupe, raw-byte preload removal, explicit paged-issue visibility, truthful retirement copy, finalized Search Operations containment and six-PR/current-baton corrections. The candidate passes the full unit suite with eight workers (`288 files / 2,388 tests`), affected timing tests as a focused `178/178`, backend `51 passed / 1 ignored`, lint/build/budgets, existing Chromium `162/162`, visual Playwright `58/58` and fresh uncached visual review `69/69`; exact-current browser/package gates still follow the commit. Final 960k qualification passed seek/deep/restart `60.34/55.76/53.85 ms`, current-write maximum `39.83 ms`, event-loop maximum `48.33 ms`; 2m headroom passed `72.59/86.60/79.01 ms`, current-write maximum `42.81 ms`, event-loop maximum `47.31 ms`. A freshly derived authentic-v11 960k open passed in `13.02 ms` with `13.32 ms` maximum heartbeat gap, added only provenance/generation/finalization metadata, and built no replay index/read model. Commit/push one replacement exact head, complete exact package/Linux proof, then obtain clean broad, persistence, renderer and concurrency/finalization exact-head reviews. Their immutable-head attestations belong in the PR/final coordinator handoff. Do not merge or release.
