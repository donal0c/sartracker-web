...
- **Breadcrumb PR-3 review ready (2026-08-25):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and both G2/G3 approvals remain binding. Review remediations through `133cc14` close the direct-device checkpoint, open-outing cooldown, audit-classifier, browser proof, benchmark teardown, and stale manual-posture findings red-first. Final gates pass 263 files / 2,104 tests, lint/TypeScript/syntax/Dots/build, Chromium 158/158, participant/coverage visual 11/11, fresh critical review 13/13, focused manual/default gates, both exact-head application packages, and a rebuilt macOS benchmark package. The final replacement Ubuntu soak remains standing at 8,664/8,664 exact positions, restart/integrity/WAL clean, zero redundant slope, 9.558 ms main / 83.6 ms renderer maxima, four healthy interactions, and zero unexplained events because no packaged operational path changed; evidence is under `output/pr3-packaged-soak/69a1096bd950270686c8e200da4311a1ab1fb1f5/`. Corrected 960k/2M production qualification, ratified G2 rows, and the 3.704 GB migration remain standing under their explicit invalidation rules. Five independent reviews are CLEAN at exact pushed application/evidence head `0455f41` with no P1/P2 findings. Linear `DON-273`/`DON-276`/`DON-275` are Done with final evidence comments. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) is open to `master`. Next: normal external PR review; do not merge or release from this implementation task.
...
- **Breadcrumb coordination workflow:** one implementation task owns complete PR-3 through BCP-07 → BCP-08 → BCP-09. G2/G3 and final exact-head gates are approved/green; the five independent reviews are clean and PR #3 is open for external review.
