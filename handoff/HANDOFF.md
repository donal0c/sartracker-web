...
- **WAR-04B release-integrity audit is review-ready in [PR #9](https://github.com/donal0c/sartracker-web/pull/9) (2026-08-30):** exact-base investigation at `3d0d36b3874947d3d620bdb5262d9cd2d7233fcf` confirms two current release blockers: shipped Electron `40.10.0` is EOL, and `electron-builder@26.0.12 -> app-builder-lib@26.0.12` generates an affected AppImage launcher. The production-only npm audit is clean but omits those release-bearing dev dependencies. Live GitHub also has no enforced master/tag review/check boundary, immutable release, attestation/SBOM, or enabled secret/Dependabot/CodeQL visibility. No dependency, workflow, release, package/config, product, or GitHub-setting change was made. Follow `docs/assurance/findings/WAR-04B.md` for controlled remediation and the mandatory exact merged-PR6 refresh; exact-head review/recheck evidence is retained on PR #9. `better-sqlite3` PR #1475 shipped in `12.10.1`, so `DON-146` is no longer upstream-blocked.
...
- **Programme branch state (2026-08-30):** breadcrumb PR-1 through PR-5 and WAR-01 PR #6 are merged on `master`. PR5 landed at `eec92812b783a795c093f37268b295dd2179a3af` from final head `f5ba8647131950dde457e50ce36fe0b8ded7337d`; exact-head Linux run `33260131951` is green, while earlier independent reviews remain prior-head evidence. Breadcrumb programme PR-6, the `DON-248`/`DON-252`/`DON-253` archive lifecycle, is actively being implemented in a separate dirty worktree and remains unmerged/mutable; it is not release proof. WAR-13A PR #7 is docs-only; it does not release a candidate or provide field proof.
