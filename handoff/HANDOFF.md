...
- **Breadcrumb PR-3 frozen review-head preparation (2026-08-25):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and both G2/G3 approvals remain binding, and the v9→v10 migration proof is unchanged. Three fresh reviewers found five additional lifecycle/evidence blockers at `472826d`; each was reproduced red-first and is now remediated: renderer-owned stages are discarded on renderer loss, settlement is sender-scoped, obsolete mission activations cannot commit/reject the replacement renderer, cancelling a build leaves the active tile worker serviceable, renderer-held rejection evidence blocks Complete, and one mission ACK cannot clear another mission's warning. Current remediation verification is 29 focused files / 156 tests, full unit 259 files / 2,026 tests, lint, TypeScript, CJS syntax, build/budgets, and focused Chromium 6/6. Bindings: `docs/breadcrumb-coverage-renderer-decision.md`, `docs/breadcrumb-pr3-t1-evidence.md`, and matching `output/` directories. Next: commit the bounded fixes, rebind exact-production and packaged hot-path evidence on Ubuntu, repeat exact-head gates, and restart all five independent reviews from scratch.
