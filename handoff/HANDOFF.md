...
- **Breadcrumb PR-3 external-review remediation (2026-08-25):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and both G2/G3 approvals remain binding. A later independent review invalidated the earlier clean-review claim: missing manifest inventory could be inserted as falsely fresh, renderer fields could replace worker `type`/`requestId`, and an unvalidated tile `x` could escape the owned cache path. Commit `259fd74` fixes all three red-first: new inventory is pending until built, the runner owns its envelope, the main/worker share bounded integral tile validation, and the worker asserts cache-root containment. The focused regressions pass 4 files / 42 tests; an integration case with two real fixes and no ledger row remains blocked by `chunk_not_fresh`; full unit passes 264 files / 2,110 tests, plus lint, TypeScript, build/budgets, and focused participant/coverage Chromium 10/10. The standing G2, decoded qualification, v9→v10 migration, and prior soak remain historical proof for unchanged workload/schema surfaces; the live PR exact-head Linux package/soak check and five fresh independent reviews must be green before review readiness. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and unmerged. Do not merge or release from this implementation task.
