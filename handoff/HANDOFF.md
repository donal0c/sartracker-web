...
**B7 follow-through** (`DON-24`). The release workflow now has live dependency preflight plus Linux/Windows launch-smoke jobs. First run `26040183978` proved the dependency preflight and exposed two harness gaps: Linux must run with Secret Service and must inspect the screenshot for the fault shell; Windows path discovery had a PowerShell comma bug. Both are fixed locally. Next release action: push the workflow fix, rerun `gh workflow run release.yml --repo donal0c/sartracker-web -f tag=v0.1.0-beta.3 -f smoke_existing_release=true`, review the launch screenshots/logs, then do a real Linux machine smoke before sending artifacts wider. If smoke fails, cut `v0.1.0-beta.4` per the immutability rule. B5 feedback triage (`sartracker-web-s8m` / `DON-11`) follows once the artifact gate is clean.
