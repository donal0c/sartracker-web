...
- **Breadcrumb PR-3 architecture remediation gate (2026-08-26):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and G2/G3 approvals remain binding. Donal paused the repeated review loop and explicitly applied the bounded topology to PR-3. Fresh Fable session `a5493464-eb53-4231-a686-1aed764e3cb7` attacked exact head `397adc8`; central retrace then closed the whole seam coherently: active-session and loss markers are file-and-directory durable, quit/restore joins failed-renderer fencing, known loss permanently blocks Complete/100%, and only a roster-authorized exact-loss acknowledgement can permit archive/read-only finalization. Later loss invalidates it; pending, corrupt, global, or other degraded evidence cannot be acknowledged. The final local code-tree gates pass: 273 files / 2,227 tests, lint, TypeScript/build/budgets, exact Dots 10/10, Chromium 14/14, critical visual 1/1 plus fresh uncached PASS, backend 51 passed / 1 intentional ignore, and changed CJS syntax. Replacement exact-head macOS/Linux package/soak evidence and one fresh broad plus targeted persistence/completeness, concurrency/finalization, and renderer/input-containment rechecks remain. False Complete/100%, lost mission evidence, unbounded Electron-main work, and unresolved safety failures remain absolute blockers. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and unmerged. Do not merge or release from this implementation task.
