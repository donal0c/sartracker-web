...
- **Breadcrumb PR-1 review candidate (2026-08-22):** `codex/breadcrumb-pr1-ingest-safety` contains BCP-10/01/02/05. Schema v8 adds exact accepted-position provenance, immutable first-accepted-wins conflict evidence, a crash-safe rejection outbox, persistent evidence health, and accuracy-aware stationary attention without replacing current map markers. The 3.7 GB packaged gate attributed a pre-existing startup stall to synchronous Mission Review `listAuditEvents` (about 7.5 s) and `countPositions` (about 1.1–1.3 s), materially equivalent at exact base `496d450` and candidate `586b826`. The pulled-forward `DON-251` isolation now leaves Review closed at startup and runs one bounded audit/count snapshot in a cancellable read-only worker; no v8 index/retention work or 200 ms waiver was added. Focused worker/lifecycle/oracle/cancellation/current-position tests, BCP-01 regression, and BCP-10 are green. Exact-head packaged migration/Review responsiveness, final deterministic gates, soak, push, and PR creation remain. No live-provider, field, deployment, merge, or release proof has been run.
