...
- **`DON-261` / `DON-262` / `DON-263` beta.12.5 qualified for guarded publication:** exact commit `042d77ad6158`, tag run `30497318233`, and clean no-skip local verification are green. Exact CI checksums are AppImage `41124632…` and `.deb` `0c876bf7…`. The AppImage and real installed `.deb` passed lifecycle/recovery/finalize/archive, coordinate rejection, sanitized exports, corrupt-secret startup, duplicate launch, live Traccar (33 devices), pending/degraded-basemap overlays, and 13 stable empty-history polls with Open Devices fixed at Y `598`. Deterministic five-/fourteen-day soaks stored 691,224/1,935,384 exact positions with zero redundant rows and an identical prefix digest. Independent code and Fable 5 reviews returned `RELEASE` with no P1/P2. The draft prerelease is ready for the guarded publish step; persistent sync diagnostics remain the non-blocking `DON-264`.
...
- **Latest published team beta:** [`electron-v0.1.0-beta.12.3`](https://github.com/donal0c/sartracker-web/releases/tag/electron-v0.1.0-beta.12.3), pending guarded promotion of the fully qualified beta.12.5 draft. Original-machine confirmation remains `DON-247`.
