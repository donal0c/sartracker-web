...
- **Breadcrumb PR-1 is review-ready (2026-08-22):** [PR #1](https://github.com/donal0c/sartracker-web/pull/1) on `codex/breadcrumb-pr1-ingest-safety` contains BCP-10/01/02/05 plus the pulled-forward `DON-251` Mission Review safety isolation. Schema v8 preserves immutable first-accepted truth, durable canonical anomaly evidence, crash-safe rejection delivery, persistent evidence health, and accuracy-aware stationary attention without replacing current markers. The 3.704 GB gate attributed the old stall to pre-existing Review scans (about 7.5 s audit; 1.1–1.3 s count), not PR-1. Review now stays closed at startup and reads one bounded audit/count snapshot in a cancellable read-only worker; no migration index, retention change, or 200 ms waiver was added. Exact code head `0cf1c27`: lint/build, unit `208 / 1,650`, backend `51 / 1 ignored`, Chromium `146 / 146`, visual Playwright `13 / 13`, fresh visual review `15 / 15`, BCP-10 `10 / 10`, Electron packaging, and CI soak `8,664 / 8,664` pass. Field fixture v7→v8 preserved integrity and the original 2,040,000-position digest; startup max was `48.6 ms`, Review-open max `30.8 ms`, and a live fix published while Review was loading. The cache-warm proof query took `1.564 s`; do not relabel the accepted cold `~7.5 s` measurement as fast—index/retention optimization remains `DON-251`. Donal owns PR review coordination. No live-provider, field-machine, deployment, merge, or release proof was run.
