...
- **Breadcrumb PR-3 final remediation gate (2026-08-26):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and both G2/G3 approvals remain binding. Every earlier review wave is superseded. Application head `0e3cf4b` closes the final two verified P2s red-first: renderer-owned map sources now release before the backend predecessor, failed backend release retries the same idempotent stage, recovery uses one full-manifest fresh-source swap, and claim/catalog worker work is bounded to unique current canonical inventory with exact revision and one-to-one result validation before main-isolate ledger writes. Full unit is 264 files / 2,133 tests, plus lint, TypeScript, build/budgets, exact Dots 10/10, changed CommonJS syntax, and participant/coverage Chromium 10/10. Exact Linux run [`32922725575`](https://github.com/donal0c/sartracker-web/actions/runs/32922725575) passed x64 AppImage/`.deb` packaging, AppImage launch, and the 8,664/8,664 packaged soak with digest `d2c0bb78…`, integrity `ok`, one restart, 33.450 ms main maximum, zero redundant slope, and four healthy operator interactions. This documentation-only descendant still requires deterministic exact-head gates and five fresh independent exact-head reviews before review readiness. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and unmerged. Do not merge or release from this implementation task.
