...
- **PR5 current remediation (2026-08-29):** persistence/completeness and concurrency/finalization are CLEAN on exact head `c1ad54b8`; renderer review rejected it because switching Review tabs visibly reset time/filter/search controls while retaining results from the earlier scope. Strict retrace retained this as P2: a deterministic production path made the controls misdescribe the evidence on screen. Executable fix `aea5db0727c8` separates editable drafts from their authoritative replay/page owner, restores accepted time/device/outing/search scope after remount or publication, and remounts Search Operations on mission change. Red-first unit and actual browser remount regressions are green. Local proof: renderer 15 files / 252 tests, full unit 296 / 2,528, backend 58/one intentional ignore, Chromium 167/167, visual 2/2 plus fresh uncached critical review 4/4, lint/build/diff, unsigned macOS arm64 package `sha.aea5db0727c8`, and a 13.817 s two-launch packaged soak with 6/6 batches, exact 8,664/8,664 positions/digest, integrity `ok`, zero redundant slope, zero crashes, four healthy interactions and 10.875 ms maximum main round trip. The theoretical 48-bit archive-name hash collision is P3 because no realistic mission-cardinality mechanism exists and new ingests use UUID-qualified names; Tauri's process-global attachment mutex is qualification-only because one active mission is enforced and current writes bypass it. Exact-head Linux plus fresh broad and renderer rechecks remain; unaffected persistence/concurrency clean verdicts do not restart. Reviewer severities remain proposals under the recorded production-path/requirement/impact/ownership test. PR opened/review-ready is intermediate; do not merge or release.
