...
- **Breadcrumb PR-3 Stage 1 / G2 active (2026-08-24):** isolated branch `codex/breadcrumb-pr3-complete-coverage` started clean from exact base `7021fc1e`. The non-production A/B/C renderer harness now lives under `tools/coverage-renderer-bench/`, with a strict-red-first pure manifest/statistics/budget library (expected red: missing module; later attestation red: a false pane was accepted), shared SQLite query and page-boundary-safe segmentation pipeline, packaged MapLibre drivers, contributor-revision-bound Candidate-B protobuf tile cache, 5,000-fix late-batch probe, current-position isolation, filter/pan/zoom/RSS/stall measurements, and real SIGKILL/resume proof. Each of nine seeded camera panes now records exact worker-vs-rendered segment counts and SHA-256 feature/revision digests. Focused bench green is `8/8`; exact Dots is `10/10`; lint, production build/bundle budgets, syntax, renderer build, unsigned macOS bench packaging, and small-fixture A/B/C packaged smoke pass. A and B also pass local exit-137/restart with all correctness flags true. These are harness checks only, not G2 scale evidence. The reference Ubuntu X11 host `donal@192.168.18.31` timed out twice and is currently down on the LAN, so the prescribed 960k/2M matrix and decision memo remain open. **No BCP-08 production code exists; do not start it until Donal ratifies G2.**
...
- **Breadcrumb coordination workflow:** one implementation task owns the complete PR-3 through BCP-07 → BCP-08 → BCP-09. G2 blocks all BCP-08 production work; G3 blocks the separate final default flip.
