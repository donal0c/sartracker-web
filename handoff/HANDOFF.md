...
- **PR5 current remediation (2026-08-29):** the restarted wave invalidated exact head `ad21f1a8`. Broad accepted two P2s: prior current-schema text cursors could fail open after the row-ID semantic change, and an already-finalized mission lacked governed idempotent archive repair. Concurrency accepted one P2: logically mission-scoped quarantine still scanned 2,000,000 global rows on Electron main, making a clean Finish take 2,061.05 ms. Persistence was clean for its assigned oversized-identity and mission-scope findings; renderer was not started after invalidation. Executable fix `b9c47e0d51c4d76b2dffc1f438a915c62c0847c2` adds a durable cursor-format reset, incomplete-row-only bounded reconstruction, a mission-first primary-key quarantine map, and fenced finalized archive replacement with explicit rejected/replacement audit linkage. Strict red tests reproduced all three; local proof is green through 295 files / 2,498 tests, focused 193/193, 500,000-entry bounded lookup, backend 57/one intentional ignore, lint/build/CommonJS, indexed 960k and unsigned macOS arm64 8,664-position soak. Prior Chromium 167/167 and visual 58/58 plus uncached 69/69 remain standing because no renderer path changed. Exact binding head `c54475009d79327693a41cd8d35274fc725fb002` passed Linux run `33226833222`: lint, 2,498 tests, build/budgets, native x64 packages, 960k qualification, packaged 8,664-position soak and AppImage launch. All four restarted exact-head reviews remain mandatory. PR opened/review-ready is intermediate; do not merge or release.
