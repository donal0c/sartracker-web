...
- **Breadcrumb PR-3 remediated; final exact-head review next (2026-08-25):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`. G2's 18-run Ubuntu matrix at `8eff87b...` selected Candidate B with ratified 1.5 GiB/2.5 GiB budgets; G3 approved the combined mission-model + coverage default. T1 at `d1e884b` passed 257 files / 1,983 unit tests, static/build/Dots, Chromium 157/157, six critical visual reviews, macOS/Linux packaging, the 16.557 ms integrity-`ok` 3.704 GB v9→v10 migration, and the one packaged Ubuntu soak. The first exact-head review wave then found eight production-path blockers. Red-first remediation now fences worker generations, makes renderer activation part of delivery, revokes Complete on runtime/evidence/scope failures, keeps manifest/claim reads ledger-bounded, batches main-side writes, and publishes progressive period catalogs. A targeted real-production-path Ubuntu check reproduced a 2M first-useful failure at `20ee295` (6.438 s); `d39732e` changed lazy enumeration to one indexed pass per participant device and passed 960k at 2.295 s first-useful / 5.819 s complete and 2M at 4.391 s / 11.679 s, with 21.807/23.055 ms main gaps and exact delivered totals. Binding: `docs/breadcrumb-pr3-t1-evidence.md` and `output/pr3-production-qualification/`. Next: restore the approved flip as the final two-boolean commit, run exact-head gates, and restart all five reviews from scratch; any further remediation restarts all five again.
...
- **Breadcrumb coordination workflow:** one implementation task owns complete PR-3 through BCP-07 → BCP-08 → BCP-09. G2/G3 are ratified, T1 is green, and the failed first review wave has been remediated. Only final-head gates plus five newly restarted independent reviews remain before the PR opens.
