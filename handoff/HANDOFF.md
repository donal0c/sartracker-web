...
- **DON-247 field confirmation remains open:** PCLinuxOS AppImage stayed responsive at 218 hours, but the Mint `.deb` stopped accepting application actions around 182 hours while the in-app clock continued. This is not yet proven to be the old main-process freeze signature. The packaged soak now exercises real, non-forced Devices open/close interactions at start, load checkpoints, restart recovery, and final load; it fails slow/blocked controls and screenshots input failures. Local packaged CI proof passed 4/4 interactions, 8,664 exact positions, restart/backup, main/renderer gates, and zero redundant telemetry. Mint AppImage-vs-`.deb`/identical-profile confirmation remains.
