...
- **WAR-04 is review-ready in [PR #8](https://github.com/donal0c/sartracker-web/pull/8) (2026-08-30):** the investigation-only branch is pinned to exact base `3d0d36b3874947d3d620bdb5262d9cd2d7233fcf`. Nine confirmed local failure/privacy defects and six unproven hypotheses are recorded across official-map readiness, settings/credential startup, and sanitized support/incident export; twelve isolated synthetic checks preserve the red states. Shipping code is unchanged and the normal lint, build, unit (`296 files / 2,535 tests`), backend (`58 passed / 1 ignored`), and Playwright (`227/227`) gates are green. Three bounded WAR-11 remediation clusters are required before release qualification; broad packaged/provider/platform/scale/soak proof stays in WAR-12. `DON-264` remains separately owned. Do not merge or release from WAR-04; its two independent exact-head reviews must be recorded on PR #8.
