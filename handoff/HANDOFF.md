...
- **PR5 current remediation (2026-08-29):** exact head `d20339dd` was rejected after concurrency reproduced an operational Electron attachment write committing after Finish and persistence reproduced a standalone archive omitting superseded/retired attachment bytes still referenced by immutable marker versions. Both remain P2 after the stricter coordinator retrace: deterministic production mechanisms breached PR5's finalized-write and no-silent-loss contracts. Executable fix `7dd355704382` serializes attachment custody and Finish per mission without serializing current positions, audits completed custody, joins it at shutdown, and archives every current/version/audit-referenced attachment under collision-safe names while failing closed on a missing file; the Tauri reference adapter keeps equivalent lock/custody/archive semantics. Red-first regressions are green. Local proof: focused Electron 3 files / 185 tests, full unit 296 / 2,526, backend 58/one intentional ignore, Chromium 167/167, visual 2/2 plus fresh uncached critical review 4/4, lint/build/diff, unsigned macOS arm64 package `sha.7dd355704382`, and a 13.649 s two-launch packaged soak with 6/6 batches, exact 8,664/8,664 positions/digest, integrity `ok`, zero redundant slope, zero crashes, four healthy interactions and 11.129 ms maximum main round trip. Exact-head Linux and all four independent reviews restart on one bound code-and-documentation candidate. Reviewer severities are now proposals: only a source-retraced production-path, requirement-owned, realistically consequential defect blocks as P1/P2; speculative, qualification-only, pre-existing, reference-only or out-of-contract findings are P3/out of scope with a recorded reason. PR opened/review-ready remains intermediate; do not merge or release.
