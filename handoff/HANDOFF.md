...
- **Breadcrumb PR-3 final remediation gate (2026-08-26):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and both G2/G3 approvals remain binding. The first final-head review wave was invalidated by five confirmed boundary defects. Application head `f842190` closes them red-first: period digests are derived from request-owned contributors; repeated-effective-from participant re-adds preserve earlier fixed checkpoint edges and add only the uncovered suffix; finalized evidence health leaves the live aggregate but is restored on unlock; normal claim/write sequence drift returns the newer bounded SQLite claim while same-sequence divergence still fails closed; and default-on coverage is gated to operational Electron rather than the historical Tauri store. Full unit is 267 files / 2,147 tests; lint, TypeScript, build/budgets, participant/coverage Chromium 10/10, selected critical visual and uncached review 2/2, and backend 51/51 with one intentional OS-keychain ignore pass. Exact application-head Linux run [`32940999122`](https://github.com/donal0c/sartracker-web/actions/runs/32940999122) passed x64 AppImage/`.deb` packaging, AppImage launch, and the 8,664/8,664 source-exact soak with digest `05d0957d…`, integrity `ok`, one restart, 23.3174 ms main maximum, zero renderer crashes, and four healthy operator interactions. The operator manual now describes repeated-effective-from recovery; existing screenshots remain accurate because controls and layout did not change. The documentation-only descendant still requires deterministic exact-head/Linux gates and five fresh independent exact-head reviews from zero before review readiness. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and unmerged. Do not merge or release from this implementation task.
