...
- **Active release blocker — `DON-260` / `0.1.0-beta.12.1`:** deep breadcrumb correctness hotfix is locally complete and awaiting exact-artifact qualification. Stable upstream identity/device ownership, audited corrections, deterministic whole-route selection, read-only retryable worker restart reconstruction, mission-wide late-fix reconciliation, strict parsing, five-minute stale semantics, runtime-generation isolation, explicit persistence/cache failure states, and display/fix-time/accuracy UI are implemented under TDD. Current local proof: lint/build; full unit `168 files / 1232 tests`; backend `51 passed / 1 ignored` plus clean formatting/strict Clippy; Chromium `132/132`; visual Playwright `36/36`; fresh uncached independent visual review `41/41`; full no-skip beta gate `8/8`; packaged CI profile `8,664/8,664`; packaged five-/fourteen-day profiles `691,224/691,224` and `1,935,384/1,935,384`, exact full/prefix digests, one/two restarts, zero redundant-event slope, healthy SQLite/WAL, main maxima `96.3/193.6 ms`, renderer maxima `143.1/198.8 ms`, and individual operator-action maxima `502.8/808.0 ms`; lifecycle/restart/recovery/finalize/archive, bad-secret, and sanitized export smokes passed. Same-session Fable final disposition is `PASS_TO_FULL_RELEASE_GATES` with no P1/P2 finding. Tag run `30402564688` correctly blocked when the Linux image blocklisted WebGL. Run `30426564770` then rendered the real UI, persisted the exact 8,664-row digest, and completed 4/4 healthy interactions, but correctly blocked because headless background/occlusion throttling produced a 2,772.2 ms action against the unchanged 1,000 ms limit. The harness now disables Chromium background/renderer/occluded-window throttling for foreground-equivalent CI measurement under TDD. Ubuntu/Xvfb passes 8,664/8,664 exact with 4/4 healthy, main max 176.4 ms, renderer max 149.9 ms, action max 646.2 ms, and no desktop-session throttle; a locked physical session remains invalid timing evidence. Neither failed run created a release or uploaded distributable artifacts. Still required: replacement tag-driven CI, exact CI AppImage/`.deb` qualification on Ubuntu, release-note/Linear closeout, and publication. Beta.13 remains `DON-248`-`DON-255`; do not fold map loading or storage/archive redesign into this hotfix. Schema 5 is additive but beta.12 cannot open it, so no in-place downgrade after upgrade.
