...
- **PR5 current remediation (2026-08-29):** exact head `bd0f3f22` was clean in broad, persistence and renderer review, but concurrency/finalization rejected it. Central retrace reproduced a delayed same-mission Search Operations worker result overwriting refreshed evidence and a retained area becoming unreachable after its sort key changed between pages. Executable fix `5cbd93e91d8c` now reads generation, exact count and entries inside one pinned SQLite snapshot; binds the opaque cursor to that generation; rejects changed continuations with an explicit return-to-first-page instruction; invalidates all page tokens when mission refresh begins; and refuses to publish mixed-generation initial pages. The browser harness mirrors the same contract. Both reviewer reproductions and a concurrent-WAL snapshot attack were red then green. Local proof is green through 296 files / 2,513 unit tests, backend 57/one intentional ignore, Chromium 167/167, visual 2/2 plus fresh uncached critical review 4/4, lint/build/CommonJS/diff checks, unsigned macOS arm64 packaging, and a packaged soak with two launches, 6/6 batches, exact 8,664/8,664 positions, integrity `ok`, zero redundant slope and 2.4 ms maximum main round trip. Exact-head Linux and all four independent exact-head reviews must restart on one final code-and-documentation head because this fix crosses runtime, worker and persistence boundaries. PR opened/review-ready is intermediate; do not merge or release.
