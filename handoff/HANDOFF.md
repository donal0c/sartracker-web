...
- **Breadcrumb PR-3 external-review remediation (2026-08-26):** branch `codex/breadcrumb-pr3-complete-coverage` remains based on exact PR-2 merge `7021fc1e`; Candidate B and both G2/G3 approvals remain binding. External and renewed review invalidated the earlier clean verdicts with false freshness/classification, worker/query trust-boundary, tile-cache, and catalog recovery defects. Commits through `6e53ba1` close them red-first. Coverage IPC preserves failed terminal ownership, recovers renderer-lost and alive-renderer stages before Retry, accepts a lost-generation cleanup only when no different stage exists, and prioritizes the retrying renderer's live stage before older stale tokens so a multi-renderer worker restart cannot wedge cleanup order. Focused green is 5 files / 56 tests; full unit is 264 files / 2,117 tests, plus lint, TypeScript, CommonJS syntax, build/budgets, and participant/coverage Chromium 10/10. The `a7cbda3` Linux CI pass is superseded by this operational-path change; a new exact-head Linux package/soak check and five fresh independent reviews must be green before review readiness. PR [#3](https://github.com/donal0c/sartracker-web/pull/3) remains open and unmerged. Do not merge or release from this implementation task.
