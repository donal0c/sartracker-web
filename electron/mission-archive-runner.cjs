...
const DEFAULT_WORKER_PATH = path.join(__dirname, 'mission-archive-worker.cjs')
...
function startMissionArchiveCreateWorker(input) {
  ...
  const cancelGraceMs = normalizeDuration(
    ...
    0,
    30_000,
    'Mission archive cancellation grace period',
  )
  ...
  const completion = new Promise((resolve, reject) => {
    ...
    /** Clears lifecycle resources after the physical worker terminates. */
    const cleanup = () => {
    ...
    /** Rejects the public operation once without claiming the worker has exited. */
    ...
    /** Requests cooperative cancellation and then forces physical termination. */
    const cancelAndReject = (error) => {
      if (Atomics.compareExchange(cancellationFlag, 0, 0, 1) === 0) {
        try {
          ...
          // The shared flag remains authoritative if the message port is already closed.
        }
      }
      rejectOnce(error)
    ...
    const resetWatchdog = (deadlineMs = watchdogMs) => {
      ...
      watchdogTimer = setTimeout(() => {
        cancelAndReject(new Error('Mission archive worker stopped making bounded progress.'))
      }, deadlineMs)
    }
    ...
    worker.on('message', (message) => {
      ...
      if (message?.type === 'progress') {
        if (terminalMessageSeen) {
          cancelAndReject(new Error('Mission archive worker emitted data after completion.'))
          return
        }
        let progress
        try {
          progress = normalizeArchiveProgress(message, request.operationId)
          assertProgressAdvance(progress, lastProgress)
        } catch {
          cancelAndReject(new Error('Mission archive worker emitted invalid progress.'))
          return
        }
        ...
        try {
          ...
          cancelAndReject(new Error('Mission archive progress observer failed safely.'))
        }
        return
      }
      if (message?.type === 'complete') {
        if (terminalMessageSeen) {
          cancelAndReject(new Error('Mission archive worker emitted duplicate terminal data.'))
          return
        }
        terminalMessageSeen = true
        try {
          terminalResult = normalizeArchiveCreateResult(message, request)
        } catch {
          cancelAndReject(new Error('Mission archive worker returned a substituted result.'))
        }
        return
      }
      ...
      cancelAndReject(new Error('Mission archive worker emitted an unsupported message.'))
    })

    worker.once('error', () => {
      cancelAndReject(new Error('Mission archive worker failed unexpectedly.'))
    })
    ...
    worker.once('exit', (code) => {
      ...
      reject(new Error(`Mission archive worker exited without a valid completion (code ${code}).`))
    })
    ...
    try {
      ...
      cancelAndReject(new Error('Mission archive credentials could not be transferred safely.'))
      return
    }
    zeroIfAttached(passphraseBytes)
    zeroIfAttached(recoveryCodeBytes)
...
/** Validates strict progress sequence and prevents phase regression or counter substitution. */
function assertProgressAdvance(progress, previous) {
  if (previous === null) return
  if (progress.sequence <= previous.sequence) throw new Error('Archive progress sequence regressed.')
  ...
  if (currentPhase < previousPhase) throw new Error('Archive progress phase regressed.')
  if (currentPhase === previousPhase && (
    progress.unit !== previous.unit
    || progress.total !== previous.total
    || progress.completed < previous.completed
  )) {
    throw new Error('Archive progress counter changed shape.')
  }
}
...
/** Creates a bounded typed failure without reflecting worker-controlled text or secrets. */
function createClosedWorkerFailure(message, operationId) {
  const code = message?.operationId === operationId && WORKER_FAILURE_CODES.has(message?.code)
    ? message.code
    : 'ARCHIVE_CREATE_FAILED'
  const error = new Error(`Mission archive worker failed safely (${code}).`)
  error.code = code
  return error
}
...
function createAbortError() {
  const error = new Error('Mission archive worker was cancelled.')
...
/** Decorates a promise with physical-exit ownership and explicit cancellation. */
...
module.exports = { startMissionArchiveCreateWorker }
